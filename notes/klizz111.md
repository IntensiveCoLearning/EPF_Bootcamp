...
\\overline{B} \\leftarrow r \\cdot C -r \\cdot e \\cdot A \\quad \\text{（其中 } C = G\_1 + \\sum\_{l=1}^\\ell m\_l \\cdot H\_l\\text{）}
...
（$\\mathsf{ctx}$ 为领域分隔字符串，例如 `"BBS_PoK_1||ChainID"` 或具体应用定义的上下文，防止重放/混淆攻击，例如在Dapp1生成的c用到了Dapp2中）
...
私钥 d 是一个随机整数，满足： 1 \\leq d \\leq n-1

**_\####_ 公钥生成**

公钥 Q 通过标量乘法计算： Q = d \\cdot G

### 密钥交换

Alice 和 Bob 通过以下步骤交换密钥：

NaN.  **Alice** 生成私钥 d\_A 和公钥 Q\_A = d\_A \\cdot G，将 Q\_A 发送给 Bob。
      
NaN.  **Bob** 生成私钥 d\_B 和公钥 Q\_B = d\_B \\cdot G，将 Q\_B 发送给 Alice。
      
NaN.  **Alice** 接收到 Q\_B 后，计算共享密钥： K\_A = d\_A \\cdot Q\_B = d\_A \\cdot (d\_B \\cdot G) = (d\_A \\cdot d\_B) \\cdot G
      
NaN.  **Bob** 接收到 Q\_A 后，计算共享密钥： K\_B = d\_B \\cdot Q\_A = d\_B \\cdot (d\_A \\cdot G) = (d\_B \\cdot d\_A) \\cdot G
      
NaN.  最终得到相同共享密钥 K = (d\_A \\cdot d\_B) \\cdot G。
...
NaN.  **生成随机数** k，满足 1 \\leq k \\leq n-1。
      
NaN.  **计算点** P = k \\cdot G = (x\_1, y\_1)。
      
NaN.  **计算公钥** K = k \\cdot Q = k \\cdot d \\cdot G。
      
NaN.  **生成对称密钥** K\_{sym} = H(K)，其中 H 是哈希函数。
      
NaN.  **加密消息** C = M \\oplus K\_{sym}。
...
NaN.  **计算共享密钥** K = d \\cdot P = d \\cdot k \\cdot G。
      
NaN.  **生成对称密钥** K\_{sym} = H(K)。
      
NaN.  **解密消息** M = C \\oplus K\_{sym}。
...
生成一个随机数 k，满足： 1 \\leq k \\leq n-1

**!!!**：这里是简化的，具体参照[**这里**](https://datatracker.ietf.org/doc/html/rfc6979)，简单说就是Hash(h || d)

步骤2：计算点

计算椭圆曲线点： P = k \\cdot G = (x\_1, y\_1)
...
验证 r 和 s 在有效范围内： 1 \\leq r \\leq n-1 1 \\leq s \\leq n-1

步骤2：计算逆元

计算 s 的乘法逆元： w = s^{-1} \\; mod \\; n

步骤3：计算标量

u\_1 = h \\cdot w \\; mod \\; n u\_2 = r \\cdot w \\; mod \\; n

步骤4：计算验证点

P^{'} = u\_1 \\cdot G + u\_2 \\cdot Q = ({x\_1}^{'}, {y\_1}^{'})

步骤5：验证结果

如果 {x\_1}^{'} \\; mod \\; n = r，则签名有效；否则无效。

### 数学原理

从签名生成有： s = k^{-1}(h + r \\cdot d) \\; mod \\; n

重排得到： k = s^{-1}(h + r \\cdot d) \\; mod \\; n \\quad (\\alpha)

在验证过程中： P^{'} = u\_1 \\cdot G + u\_2 \\cdot Q

替换 u\_1 和 u\_2： P^{'} = (h \\cdot s^{-1}) \\cdot G + (r \\cdot s^{-1}) \\cdot Q

由于 Q = d \\cdot G： P^{'} = (h \\cdot s^{-1}) \\cdot G + (r \\cdot s^{-1}) \\cdot (d \\cdot G) P^{'} = s^{-1}(h + r \\cdot d) \\cdot G

根据签名生成中(式\\alpha)的关系： P^{'} = k \\cdot G = P
...
在以太坊中，需要一个 v（也叫 _recovery id_，记作 \\mathrm{recId}）与 (r,s) 一起传输，使得验证者可以从签名中恢复出公钥 Q（进而得到地址）。

1) v / \\mathrm{recId} 到底在表达什么

ECDSA 签名里有中间点： R = k\\cdot G = (x\_R, y\_R) 而签名的 r \\equiv x\_R \\pmod n

因此仅凭 r 并不能唯一确定 R：

-   同一个 x 往往对应两个点（y 与 -y），需要 **1 bit** 表示 y 的奇偶性（或“是否取较大/较小根”）。
    
-   由于 x\_R 是在有限域 \\mathbb{F}\_p 上的坐标，而 r 是模 n 的结果，可能存在 x\_R = r + jn \\quad (j \\in \\{0,1\\}) 需要再用 **1 bit** 表示 j。
    

所以常见的恢复标识是： \\mathrm{recId} \\in \\{0,1,2,3\\} 其中通常可理解为：

-   \\mathrm{recId} \\bmod 2：选择 y 的奇偶性（对应两种 R）
    
-   \\left\\lfloor \\mathrm{recId}/2 \\right\\rfloor：表示是否使用 x=r+n（即 j）
...
历史上（黄皮书/早期实现）以太坊把 \\mathrm{recId} 平移为： v \\in \\{27, 28\\},\\quad v = \\mathrm{recId} + 27\\quad(\\mathrm{recId}\\in\\{0,1\\})

引入 EIP-155（防重放）后，交易签名里常见： v = \\mathrm{recId} + 35 + 2\\cdot \\mathrm{chainId}
...
ECDSA 有： s \\equiv k^{-1}(h + r d)\\pmod n 两边乘以 k： sk \\equiv h + rd \\pmod n

又因为 R=kG，于是： sR = s(kG) = (sk)G \\equiv (h+rd)G = hG + r(dG) = hG + rQ

移项得到： rQ \\equiv sR - hG \\pmod n 从而： Q \\equiv r^{-1}(sR - hG)\\pmod n

这就是“公钥恢复”的核心：**只要能确定 R，就能算出 Q。** 而 v/\\mathrm{recId} 的作用，就是帮助从 r 选出正确的 R。
...
NaN.  **范围检查** 1\\le r,s \\le n-1
      
NaN.  **由 r 构造候选 x** 令 j=\\left\\lfloor \\mathrm{recId}/2 \\right\\rfloor \\in \\{0,1\\}，取 x = r + jn 若 x \\ge p 则该候选无效（这一分支失败）。
      
NaN.  **在曲线上解出点 R** 计算 y^2 \\equiv x^3 + ax + b \\pmod p 若无解则该候选无效；若有两解 \\pm y，用 \\mathrm{recId}\\bmod 2 决定取哪个（例如选与奇偶性匹配的那个），得到 R=(x,y)
      
NaN.  **计算公钥** Q = r^{-1}(sR - hG) 得到候选公钥 Q。
      
NaN.  **校验** 用恢复出来的 Q 对 (r,s) 做一次标准 ECDSA 验签；通过则恢复成功，否则尝试其他候选（在实现里就是不同的 \\mathrm{recId} 分支）。
...
[**论文原文**](https://www.iacr.org/archive/asiacrypt2010/6477178/6477178.pdf)

### 预备知识

双线性配对

<G src="/img/zkp/双线性.png" alt="双线性"></G>

**这里要注意，配对的 \\mathbb{G}\_1 和 \\mathbb{G}\_2 是两个不同的群**

可信设置 (Trusted Setup)

需要预先生成一组公共参数字符串（CRS - Common Reference String）。这个过程通常涉及一个或多个参与者共同生成一个秘密值（_MPC_） \\alpha，然后计算并公开一系列点： \\vec{CRS} = \\{g, \\alpha g, \\alpha^2 g, \\alpha^3 g, ..., \\alpha^d g, h, \\alpha h\\} = \\{G\_0,G\_1,G\_2 ... G\_d, H\_0, H\_1\\}, 以下简记为 \\vec{G} 和 \\vec{H}, 其中 d 支持是你希望承诺的多项式的最大次数, g \\in \\mathbb{G}\_1, h \\in \\mathbb{G}\_2,既生成元。秘密值 \\alpha 必须在生成后被彻底销毁（Toxic waste）。如果 \\alpha 被泄露，任何人都可以伪造任何证明，整个方案的安全性就崩溃了。

根据椭圆曲线离散对数的困难性定理，在嵌入到多项式后，给定 g 和 \\alpha g，计算 \\alpha 是不可行的。
...
$$  
C = c\_0 G\_0 + c\_1 G\_1 + ... + c\_n G\_n = f(\\alpha) g  
$$
...
这个 C 就是 f(x) 的_KZG_承诺。它是一个椭圆曲线上的点，大小是固定的（一个群元素），无论 f(x) 有多复杂。
...
NaN.  证明者计算这个商多项式 q(x) = \\frac{f(x) - y}{x-z}，记为 q(x) = b\_0 + b\_1 x + b\_2 x^2 + ... + b\_{m} x^{m}。
      
NaN.  证明者计算证明 \\omega（也称为_witness_ 或者 \\pi ）： w = b\_0 G\_0 + b\_1 G\_1 + ... + b\_m G\_m = q(\\alpha) g 。
      

验证阶段_Verify_

验证者拥有：(承诺 C = f(\\alpha) g，要验证的点 z 和声称的值 y，证明 \\omega = q(\\alpha) g，CRS中的 G\_0 、H\_0 和 H\_1 = \\alpha h，双线性配对 e)

验证者执行以下检查：

e(C - y \\cdot g, h) \\equiv e(\\omega, H\_1 - z \\cdot h)
...
e(C - y \\cdot g, h) = e(f(\\alpha) g - y \\cdot g, h) = e((f(\\alpha) - y)g, h)

-   右边：
    

e(\\omega, H\_1 - z \\cdot h) = e(q(\\alpha)g, (\\alpha - z)h)

-   根据双线性性质和 f(\\alpha) - y = (\\alpha - z) \\cdot q(\\alpha)：
    

-   左边： e((f(\\alpha) - y)g, h) = e(((\\alpha - z) \\cdot q(\\alpha))g, h) = e((\\alpha - z)q(\\alpha)g, h) = e(g, h)^{q(\\alpha)(\\alpha - z)}
...
$$  
e(C + z \\cdot \\omega - y \\cdot g, h) \\equiv e(\\omega, H\_1)  
$$
...
对于攻击者，假设已知( \\alpha , C = f(\\alpha) \\cdot g，W = q(\\alpha) \\cdot g )，不知道多项式f(x)

NaN.  选择任一点 z' ，假设需要声称 f(z') = y\_{fake} 。
      
NaN.  构造证明 W\_{fake} , 从 W 中可以知道 W = \\frac{f(\\alpha) - y}{\\alpha - z} \\cdot g , 重写为 (\\alpha - z) \\cdot W = (f(\\alpha) - y) \\cdot g , 展开得到 (\\alpha - z) \\cdot w = f(\\alpha) \\cdot g - y \\cdot g , 又因为 C = f(\\alpha) \\cdot g , 则又可以得到 (\\alpha - z) \\cdot w = C - y \\cdot g 。 显然攻击者可以计算出 (\\alpha - z') \\cdot W\_{fake} = C - y\_{fake} \\cdot g , 这里由于 \\alpha 和 C 已知，z' 和 y\_{fake} 自选。从而可以计算出 W\_{fake} = \\frac{C - y\_{fake} \\cdot g}{\\alpha - z'}
      
NaN.  这样，攻击者提交( z' , y\_{fake} , w\_{fake} )即可欺骗验证者。
...
假设 _P_ 有两个多项式 f\_1(x) 和 f\_2(x)，它们的承诺分别是 C\_1 和 C\_2。需要证明 f\_1(z) = y\_1 和 f\_2(z) = y\_2。以下给出一个聚合证明。

NaN.  _V_ 发送随机数 r \\rightarrow _P_
      
NaN.  _P_ 计算 q(x) = f\_1(x) + r \* f\_2(x) 与 \\omega = q(\\alpha) g
      
NaN.  _V_ 计算 C\_g = C\_1 + r _C\_2, y\_g = y\_1 + r_ y\_2
      
NaN.  _V_ 验证 e(C\_g - y\_g _g, h) \\equiv e(\\omega, H\_1 - z_ h)
...
-   **防止网络分裂**：在以太坊网络中，所有客户端必须对交易结果达成一致。如果单个状态转换出现差异，就会导致网络分裂（Network Split）和最终确定失败（Finalization Failure）。
    
-   **统一行为**：测试的核心目标是确保所有不同的客户端实现（Implementations）能够以完全相同的方式解释和执行交易，从而维护协议定义的规则。
...
| **通用测试** | Ethereum Tests | 所有实现通用的测试用例 |

| **通用测试** | Hive | 以太坊端到端测试框架 |

| **执行层** | Execution Spec Tests | 针对执行客户端的测试用例 |

| **执行层** | FuzzyVM | EVM 的差异模糊测试工具 |

| **共识层** | Consensus Spec Tests | 共识层规范测试 |
...
\* **Walkthrough**：测试与安全概述指南。

\* **EVM 实验室工具**：

\* **Go evmlab**：受 EVMLAB 启发的 EVM 实验室。

\* **Collection of execution APIs**：执行 API 集合。

\* **测试链工具**：

\* **Assertoor**：以太坊测试网测试工具。

\* **Ethereum package**：测试网部署器（推荐新手使用）。

\* **仪表盘**：

\* **Hive test results**：Hive 测试结果展示。

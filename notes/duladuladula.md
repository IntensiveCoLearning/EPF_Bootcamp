...
多项式承诺方案和后量子密码方向学习,AI总结

* * *

# 一、什么是多项式承诺（Polynomial Commitment）

多项式承诺方案（PCS）的目标是：

> 证明者先对一个多项式 (f(x)) 做承诺；  
> 之后只公开某一点 (f(z)=y) 的值，并给出一个简短证明，验证者无需看到整个多项式。
...
用于 blob 数据承诺。([以太坊](https://ethereum.github.io/consensus-specs/specs/deneb/polynomial-commitments/?utm_source=chatgpt.com))
...
必须有可信初始化参数（structured reference string）。
...
量子计算可破坏其安全假设。([ZKDocs](https://www.zkdocs.com/docs/zkdocs/commitments/kzg_polynomial_commitment/?utm_source=chatgpt.com))
...
| 方案 | proof 大小 | verifier 成本 | trusted setup | 后量子友好 |
| --- | --- | --- | --- | --- |
| KZG | 最小 | 最低 | 是 | 否 |
| IPA | 中等 | 中等 | 否 | 否（通常仍依赖离散对数） |
| FRI | 较大 | 中等 | 否 | 相对更友好 |
...
## 3\. STARK / Hash-based Commitment

以太坊当前一个重要方向：

-   用 **STARK** 风格承诺替代 KZG。([ethereum.org](http://ethereum.org))
...
-   KZG → post-quantum-safe commitment
...
这已经成为以太坊明确研究方向之一。([ethereum.org](http://ethereum.org))
...
聚合签名原理和以太坊常用哈希函数学习,AI总结

* * *

## 一、 Web3 中的聚合签名 (Aggregate Signature)

聚合签名的核心目标是“多合一”：将多个不同的签名（甚至来自不同的人对不同的消息）压缩成一个极小的签名，且验证者只需进行一次验证。

### 1\. 核心原理：BLS 签名与双线性映射

目前 Web3 中最主流的聚合签名方案是 **BLS (Boneh-Lynn-Shacham)** 签名，它基于**双线性映射 (Bilinear Pairings)** 的数学特性：

-   **基本公式**：假设 $G\_1, G\_2$ 是两个椭圆曲线群，$e$ 是映射函数。双线性性质保证了：
    
    $$e(P, aQ) = e(aP, Q) = e(P, Q)^a$$
    
-   **聚合过程**：
    
    1.  **各司其职**：$n$ 个签名者分别用自己的私钥 $sk\_i$ 对消息 $m\_i$ 签名，得到签名 $\\sigma\_i$。
        
    2.  **无损压缩**：任何节点都可以将这些签名直接相加（群加法）：$\\sigma\_{agg} = \\sum \\sigma\_i$。
        
    3.  **单次验证**：验证者只需使用聚合后的公钥 $PK\_{agg}$ 和聚合签名 $\\sigma\_{agg}$，通过一个配对方程即可确认所有人的签名是否都合法。
        

### 2\. 为什么 Web3 需要它？

-   **节省空间**：在以太坊 POS（权益证明）中，每 12 秒就有成千上万名验证者进行投票（Attestation）。如果不聚合，区块头将无法装下这么多签名。
    
-   **降低通信开销**：节点之间传递一个聚合签名比传递几千个独立签名快得多。
    
-   **非交互性**：与 Schnorr 签名（比特币使用）不同，BLS 聚合不需要签名者之间反复通信确认，任何人拿到签名后都能手动聚合。
...
以太坊并不只使用一种哈希函数，而是根据安全性、硬件效率和零知识证明（ZK）友好性在不同场景切换。

### 常用哈希函数一览表

| 哈希函数 | 主要用途 | 特点 |
| Keccak-256 | 生成地址、合约代码哈希、交易/区块 ID、MPT 树 | 以太坊的“灵魂”。安全性极高，硬件抗性强。 |
| SHA-256 | 预编译合约 (0x02)、与比特币等外部链交互 | 工业标准，但在 EVM 中比 Keccak 贵（消耗 Gas 更多）。 |
| RIPEMD-160 | 预编译合约 (0x03)、生成兼容性地址 | 主要为了兼容比特币/莱特币风格的地址格式。 |
| Poseidon | ZK-Rollup (L2)、Merkle Tree (ZK 场景) | ZK 友好型。在算术电路中约束极少，大幅降低证明生成成本。 |
| BLAKE2b | 预编译合约 (0x09)、跨链数据校验 | 速度极快，常用于需要处理海量数据的场景（如 Zcash）。 |

### 核心解析：Keccak-256 vs. SHA-3

这是一个常见的误区：**以太坊使用的不是标准 SHA-3**。

-   在 SHA-3 标准最终确定前，以太坊就采用了 Keccak-256。
    
-   后来 NIST 在标准化 SHA-3 时更改了填充（Padding）算法。
    
-   **结论**：如果你在代码中使用标准库的 `SHA3` 算出的结果，会和以太坊的 `keccak256` 结果完全不同。
    

### 现代演进：ZK 友好型哈希 (Poseidon/MiMC)

传统的 Keccak 或 SHA-256 在零知识证明（ZKP）的电路中非常繁琐（涉及大量位运算）。

为了让 L2（如 StarkNet, Scroll, Polygon zkEVM）运行更快，开发者开发了 **Poseidon**。它使用模运算而非位运算，使得在 ZK 证明中验证一个哈希的成本降低了 10 到 100 倍。

* * *

> **技术小贴士**：
> 
> 如果你在编写智能合约，除非有特殊的跨链需求，否则请始终优先使用 `keccak256`，因为它是 EVM 的原生指令，Gas 消耗最低。

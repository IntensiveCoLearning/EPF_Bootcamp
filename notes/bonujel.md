...
执行层把“本地已有的链状态”当作输入，再拿一个“候选区块”进来，跑一次 **区块级状态转换函数**。如果这个区块在协议规则下是有效的，就把它执行并接到本地链尾，同时把状态更新成新状态；如果无效，就直接拒绝。这个思路正对应公式：σt+1≡Π(σt,B)\\sigma\_{t+1}\\equiv \\Pi(\\sigma\_t,B)σt+1​≡Π(σt​,B)。

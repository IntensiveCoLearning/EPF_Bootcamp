...
进入协议层时，有着最容易被忽略的两个的问题。一是，为什么已经有了比特币，还需要以太坊这样一条新链？二是，如果讨论对象明明还是 “以太坊”，为什么还同时谈到了执行层、共识层、validator client、Engine API 这些分层的概念？这两个问题连在一起。前面的决定了以太坊试图成为什么，后面对的决定了它在演进之后为什么必然长成今天这种结构。

...
「10 Account abstraction 账户抽象」章节，EIP-4337 完整规范、Yoav Weiss 在 EIPs Magicians 上的实现讨论、EIP-7702（set EOA code）的最新设计、Vitalik 2023 年 "Smart Account, Bad Account" 的反思帖。

### 串联逻辑

账户抽象的脉络是从 EIP-86 → EIP-2938 → EIP-3074 → EIP-4337 → EIP-7702 的六七年迭代。每一版都是在「协议侵入性」与「功能完备性」之间重新选择。EIP-4337 选择了零协议侵入（完全在 mempool 层实现）的极端，而 EIP-7702 又把「让 EOA 临时拥有合约代码」的能力写进协议层。要理解 AA 当前的设计空间，必须把这条历史轨迹看清楚。

### 重点研究

-   **EIP-4337 的「零侵入」是一种代价**：4337 通过引入 UserOperation mempool、bundler、EntryPoint 合约等 off-protocol 组件，实现了不修改协议的账户抽象。这是一个工程上的精巧解法，但它的代价是把大量复杂度推到了应用层——bundler 必须自己实现 reputation、anti-DoS、gas estimation 等本应由协议提供的服务。结果是 4337 的实际部署严重依赖 Pimlico、Stackup、Alchemy 等少数 bundler 服务商，「账户抽象」在用户视角下变成了「另一种 RPC 依赖」。这种「协议简洁性 vs 应用复杂性」的权衡在 EIP-1559 上有相反的案例（1559 选择了协议侵入但带来了 UX 简化），4337 是另一个极端。
    
-   **EIP-7702 是对 4337 路径的部分回撤**：7702 让 EOA 可以临时附加合约代码（在某个 transaction 里临时变成 smart account），这其实是承认了 4337 的零侵入路径在 UX 上不够好。7702 在 Pectra 升级中被采纳，意味着以太坊在 AA 路径上回到了「适度协议侵入」的中间地带。这个反复说明协议设计有时需要先走极端、再回归中间——纯学术分析很难在第一次就找到最优点。Vitalik 在 "Smart Account, Bad Account" 中坦承了这一点，难得地公开承认「早期判断有偏差」。
    
-   **AA 与 MEV 的新交叉**：UserOperation 在 mempool 中是可见的，bundler 可以重新排序它们以获取 MEV。这就形成了一个 AA-specific 的 MEV 形式——bundler 可以选择性 include UserOp，可以在 UserOp 之间插入自己的交易。当前 4337 通过 EntryPoint 的 atomic 执行减少了部分 sandwich 风险，但 bundler-level MEV 仍然是开放问题。这与传统 mempool 的 MEV 有结构相似，但参与者集合（bundler）远小于 builder 集合，集中度更高。如何把 PBS 的拍卖机制移植到 UserOp mempool 是一个尚未充分研究的方向。
    
-   **AA 与 social recovery 的安全经济学**：账户抽象的卖点之一是 social recovery（用 N-of-M guardian 替代 seed phrase）。但 social recovery 的安全模型需要 guardian 之间不互相串通——这是一个反复出现的多签安全假设。Vitalik 提出的 ZK-based social recovery（用零知识证明 guardian 是 "live human" 而不暴露身份）能减少串通风险，但实现难度很高。这个方向上的开放问题包括：guardian 的法律责任、丧失能力时的 recovery 是否需要 sunset clause、跨链 social recovery 的一致性保证。这些问题已经超出了纯协议设计，进入了法律与制度设计的领域。
    

### Insight

**账户抽象的本质是把以太坊的「用户」重新定义为「任意可验证的状态机」。** 传统 EOA 模型把用户绑定为「控制某个私钥的实体」，这是密码学对身份的最简化模型；而 AA 模型把用户视为「任意可被 EVM 验证 valid 的状态转换发起者」——可以是私钥签名、可以是多签、可以是 ZK 证明、可以是社交恢复，甚至可以是另一个合约的输出。这种重定义的影响远不止于钱包 UX——它意味着以太坊上的「身份」从一个 atomic primitive 变成了一个 programmable primitive。在 web2 世界，类似的转变在 OAuth 2.0 之后才发生（身份从「用户名密码」变成「federated identity」）。Web3 的 AA 可以看作 web2 OAuth 的平行演化，但因为 EVM 的可编程性，它的最终形态可能更激进——每个用户都是一个独立的合约，每次交易都是合约对自身状态机的一次状态转换。这条思路如果完全成熟，「账户」这个概念可能会从协议层消失，被更通用的「可验证的意图（intent）」取代。这与近期 SUAVE、Anoma 的 intent-centric architecture 趋势是同一条线索。
...
「09 Light clients 轻客户端」章节，Helios 客户端代码、Sync Committee 设计稿（EIP-2982、Altair upgrade），以及 Portal Network 的 history / state / beacon 子网设计文档、Vitalik 关于 stateless client 与 Verkle tree 的最新讨论。

### 串联逻辑

light client 看似边缘，但它是以太坊「普通用户独立验证」承诺的最终落脚点。从概念上分四层：① pure light client（只验签，依赖共识委员会）② stateless client（验签 + 通过 witness 验执行）③ Portal Network 节点（贡献历史数据存储）④ ZK light client（用 SNARK 压缩共识验证）。每一层对应不同的 trust assumption 与硬件预算。

### 重点研究

-   **Sync Committee 的安全假设比一般描述弱**：Altair 升级引入的 sync committee 让 light client 只需验证 512 个轮换的 validator 签名，而不是全部。这把 light client 的工作量从 O(N) 降到 O(1)，代价是引入了一个委员会层的 honest majority 假设。但 sync committee 的 stake 远低于全网（512 个 validator 控制大约几十万 ETH），这意味着攻击 light client 的成本远低于攻击主网。Justin Drake 等人讨论过提高 sync committee 大小或引入 ZK proof 替代签名验证，但这些改进都没有进入主网。这是一个被技术圈广泛认知但被产品圈低估的安全 gap。
    
-   **Stateless client 对 Verkle 的深度依赖**：stateless client 的核心是让节点不需要保存全部状态，而是依赖每个区块附带的 witness 来验证执行。当前 Merkle Patricia Trie 的 witness size 太大（每笔交易 access 一个 storage slot 就需要约 3KB 的 proof），不可能放进区块。Verkle tree 通过 vector commitment 把单 leaf proof 压缩到约 200 bytes，使 stateless 在带宽预算内可行。但 Verkle 迁移本身是以太坊历史上最大的状态格式变更，需要在不停机的情况下重新生成全部状态根，工程难度堪比 The Merge。Guillaume Ballet 团队的 transition strategy（双 trie 并行运行 + 渐进迁移）是当前的主线，但完整时间表已经多次推迟。
    
-   **Portal Network 作为「分布式以太坊存档」**：Portal Network 把历史数据、状态、beacon chain 数据分别拆成独立的 P2P 子网，让普通用户可以选择性贡献存储（例如只存 1GB 历史数据）。从分布式系统角度看，这是把以太坊的「single archive node」模型转换为「federated archive」模型，与 IPFS 的 sharded storage 接近，但加入了以太坊特定的可验证性（每片数据都可以独立验证 proof）。Portal 的最大未解问题是 incentive——altruistic node 能撑多久？目前没有 token incentive，全靠 ideology + reputation，长期可持续性存疑。
    
-   **ZK light client 的路径潜力**：用 SNARK 把 Casper FFG 的 finality 证明压缩成几 KB 的 proof，让任何设备（包括手机、IoT、跨链桥）都能在毫秒级验证以太坊状态，这是 Succinct Labs（Telepathy）、Polyhedra（zkBridge）、RISC Zero 等团队在做的事。技术上 BLS12-381 签名的 SNARK 化已经基本可行（Halo2 实现），但成本仍然偏高（生成一个 finality proof 需要十几秒 + 几 GB RAM）。一旦这个成本降到秒级 + MB 级，跨链桥的安全模型会从「多签 multisig」彻底转换为「密码学验证」，这会重写整个跨链架构。这是 light client 研究最有变革性的方向。
    

### 收获

以太坊的去中心化承诺最终要靠 light client 的可用性兑现。如果只有专业节点能验证链状态，那 stake 再分散也不算真正的去中心化。Sync Committee、Verkle、Portal、ZK light client 形成的组合是这个承诺的技术保障，但每一项都面临工程或经济学的卡点。

### Insight

早期 light client 信任 sync committee 的 honest majority；stateless client 信任 builder 提供的正确 witness；Portal Network 信任 federated storage 的 redundancy；ZK light client 只信任数学。每一步都把信任的根从「社会层」下移到「密码学层」。这与传统 PKI 信任链的演化路径相反——传统 PKI 是从密码学根逐渐叠加治理层（CA、policy、audit），而以太坊 light client 是从治理根逐步替换为密码学。这种「逆向去信任化」在分布式系统设计史上是新现象，它的最终形态可能是一个「完全用 SNARK 表达的协议」——任何参与者只需验证一个 succinct proof 就能确认整个协议的正确执行。
...
「08 Preconfirmations 预确认」章节，Justin Drake 的 based preconfirmations 原始帖子、mteam（mteam.eth）的 preconf market design 系列文章、Espresso / Astria 关于 shared sequencing 的 whitepaper。

### 串联逻辑

preconfirmation 是 UX 痛点（12 秒出块对 dApp 来说太慢）、L2 sequencer 中心化（当前 rollup sequencer 是 single point）、based rollup 复兴（让 L1 proposer 直接 sequence L2）三个独立诉求的交叉点。它本质上是把「finality」从一个共识层概念变成一个「市场化承诺」——proposer 收取 fee，对外承诺某笔交易在某个 slot 内被包含。

### 重点研究

-   **preconf 的「承诺」是 cryptoeconomic 而非 cryptographic**：用户拿到的 preconf 不是数学证明，而是 proposer 的签名 + 它质押的 ETH 作为违约抵押。如果 proposer 违约（没有把交易包含进区块），preconf 持有者可以提交证据触发 slashing。这种设计与传统金融的 escrow / bonded performance contract 结构相似。但有一个不好处理的缝隙：proposer 在 preconf 之后被「运气性」地 missed slot（因网络延迟或客户端 crash），这种情况下应该 slashing 还是赦免？目前的设计倾向于严格 slashing 以保持承诺的可信度，但这会增加 solo staker 参与 preconf 市场的风险溢价，进一步推动专业化。
    
-   **Based rollup 与 shared sequencing 的冲突**：based rollup 让 L1 proposer 直接对 L2 进行 sequencing，好处是 L2 继承 L1 的去中心化与活性，代价是 sequencer 必须知道每个 L2 的执行规则（否则 preconf 没法对 L2 状态做承诺）。shared sequencing（Espresso、Astria）则是把多个 L2 的 sequencing 委托给一个独立的 sequencer 网络，可以做跨 L2 的原子操作。这两条路径在 preconf 上有结构性冲突——based 派强调 L1 包容性，shared 派强调跨 L2 协同。两者无法简单合并，因为前者要求 sequencer 集合等于 L1 validator 集合，后者要求 sequencer 集合是独立的。
    
-   **preconf 的市场结构与「垄断性流动性」风险**：如果某些 sophisticated proposer 能可靠地提供 preconf 服务，而普通 solo staker 不能（因为缺少 builder 关系、缺少 collateral 池），那么 preconf 市场会自然集中到少数 entity。这与早期 MEV-Boost 的中心化路径完全相同。Justin Drake 提出的「preconf 必须是 permissionless」原则在实操上很难执行——permissionlessness 与 reliability 是 trade-off。一个可能的中间方案是 preconf delegation：solo staker 把自己的 slot preconf 权委托给专业 preconf provider，自己分享 fee。这与 LST 的委托结构很像，因此也会继承 LST 的所有治理与中心化问题。
    
-   **preconf 与 SSF 的相互关系**：很多人以为 SSF 之后就不需要 preconf 了——既然 12 秒就 finalize，何必再搞预确认？但事实上 SSF 的 12 秒是「最终性确认」，而 dApp 的 UX 期望是 100ms 级别的「确定可包含」。这两个时间尺度差了两个数量级，preconf 填补的正是这段空隙。SSF 与 preconf 不是替代关系而是互补关系。这个理解的偏差经常出现在媒体讨论里，但研究层应该清楚两者的目标函数完全不同。
    

### 收获

preconfirmation 章节展示了一个重要趋势：以太坊正在把「finality」概念分层——从 inclusion preconf（100ms）→ execution preconf（1s）→ soft confirmation（single slot）→ finalized（SSF 12s）→ super-majority finalized（multi-epoch）的层次结构。每一层都对应不同的安全 / UX 权衡，应用层可以按需选择。这种 multi-tiered finality 在传统数据库里有对应物（read uncommitted / read committed / serializable），但在区块链协议中是首次系统性引入。

### Insight

**preconfirmation 让以太坊从「binary finality」演化为「continuous finality spectrum」，这是分布式系统对一致性模型的一次全新表达。** 经典 distributed systems 教科书把一致性划分为 atomic / sequential / causal / eventual 等离散级别，但 preconf 引入了一个新的维度——economic certainty。在这个维度上，「某交易已被确认」不是一个 boolean，而是一个「在 X 元 collateral 担保下 Y 时间内的 P 概率确认」的复合声明。这种 quantitative finality 可以直接被 smart contract 消费——例如借贷协议可以根据 collateral 的 finality 等级动态调整 LTV，跨链桥可以根据 source chain 的 finality 等级延迟 fund release。
...
fork choice reorg 相关研究，Vitalik 与 Aditya Asgaonkar 关于 ex-ante reorg 的论文、Barnabé 关于 LMD-GHOST + Casper FFG 组合下 reorg attack surface 的图解，以及最近的 attacker model 分析。

### 串联逻辑

fork choice 是共识层最核心也最被忽视的部分。一般教程会停在「GHOST 选择最重子树」，但 LMD-GHOST + Casper FFG 在面对 adversarial attester 时的表现远比纯算法描述复杂。要理解 FCR 这章需要先把 reorg 分类清楚——ex-post vs ex-ante、short vs long、selfish-mining 派生 vs MEV 派生——再看每一种 reorg 在新的 fork choice 改进（proposer boost、view-merge、Goldfish、RLMD-GHOST）下的攻击成本变化。

### 重点研究

-   **Proposer Boost 是一个临时修补而非根本解**：2021 年的 ex-ante reorg attack 发现后，社群快速引入 proposer boost（当前 slot 的提案获得 40% 的 attester weight 加权）。这个 patch 把短 reorg 的攻击成本提高了，但它本身破坏了 GHOST「基于多数 attestation 选择」的简洁性，引入了一个被研究者称为 "balancing attack with boost-aware adversary" 的新攻击面。Neu 等人证明在特定网络延迟模型下，proposer boost 仍然存在 1-block reorg 漏洞。这是典型的 patch on patch 工程债——下一代 fork choice（Goldfish、RLMD-GHOST）的目标就是从根本上重新设计这个权重机制，但 backward compatibility 与 client 实现成本是巨大门槛。
    
-   **MEV-driven reorg 的经济学下界**：reorg 攻击的可行性传统上用「控制多少 stake」衡量，但在 MEV 时代必须重新表述为「reorg 收益超过 reorg 成本」。如果某个 slot 包含一笔 1000 ETH 的 MEV opportunity，攻击者愿意承担多少 slashing 风险？Daian 等人提出 time-bandit attack 框架，证明只要 MEV 足够大，即使是 honest majority 也有动机协同 reorg。这个下界依赖于 MEV 的分布，而 MEV 的尖峰（NFT mint、大型 oracle update、CEX-DEX divergence）在最坏情况下可以达到 5000+ ETH。理论上的 economic security 与实际 MEV 的对比是以太坊安全模型中最不舒服的开放问题。
    
-   **Single Slot Finality 对 fork choice 的影响**：SSF 实现后，reorg 在概念上不再可能（一旦 finalize 就不可逆），但在 finality 形成之前的「pre-finality 窗口」仍然存在 reorg 风险。这个窗口在当前 Casper FFG 下是 2 epoch（12.8 分钟），SSF 后会缩小到 1 slot（12 秒），但这 12 秒内的 reorg 风险并不消失，反而会被压缩到更短的时间窗口里。这意味着 short reorg attack 的研究在 SSF 之后仍然重要，只是攻击模型从「epoch-level」变成「slot-level」。这是一个被乐观情绪掩盖的细节——很多人以为 SSF 直接消灭了 reorg 问题。
    
-   **fork choice 的形式化验证缺口**：以太坊有 4-5 个独立的 consensus client（Prysm、Lighthouse、Teku、Nimbus、Lodestar），每个都独立实现 fork choice。spec 是 Python 的执行可读规范，但 Python spec 与 Go / Rust / Java / Nim / JS 的实际实现之间的等价性依赖大量 differential testing 而非形式化证明。最近 Runtime Verification 与 ConsenSys 联合的 K-framework 项目在尝试 mechanize fork choice，但工作量巨大。这是一个长期被低估的安全风险——历史上 Bitcoin 也曾因 client 实现不一致触发过链分叉（2013 BDB lock issue）。以太坊的 multi-client 多样性在抗 client bug 上是优势，但在 consensus correctness 上是风险。
    

### 收获

FCR 这一章的真正价值是让一个不太直观的事实浮出水面：fork choice 不是一个「已经被解决的算法问题」，而是一个持续受到 MEV、网络异步、客户端多样性多重压力的活跃研究领域。SSF 不是终点，而只是把问题从 epoch-scale 压缩到 slot-scale。任何认为「PoS 已经成熟」的判断都低估了这一层的复杂度。

### Insight

**以太坊 fork choice 的设计正在从「理论最优」转向「对抗性现实最优」，这里其实是分布式系统理论与博弈论混在一起在做的事。** 经典论文（Zamani、Goldfish、Pass-Shi）追求的是在某种对抗模型下的安全性证明，但实际的 fork choice 必须同时应对协议外的 MEV 激励、协议内的 attester 串谋、以及 P2P 层的网络分区，这几类因素纠缠在一起，使得没有任何单一的形式化模型能完整刻画安全性。最现实的研究路径可能不是追求 "provably secure" 的单一算法，而是构造一个 multi-attacker 的仿真框架（类似 fuzzing 但针对共识协议），在这个框架里持续测试每个新提案。Ethereum Foundation 近期资助的 Antithesis 仿真平台就是这条路径的体现，它意味着共识研究正在从「定理证明」模式转向「系统科学」模式。
...
「06 PeerDAS」章节、EIP-7594 完整规范、Dankrad 的 Data Availability Sampling、Francesco D'Amato 关于 1D vs 2D DAS 的设计讨论、最近的 PeerDAS devnet 测试报告。

### 串联逻辑

PeerDAS 不能孤立理解，它是 EIP-4844 与 full Danksharding 之间的关键过渡。从 4844 的「所有节点下载所有 blob」→ PeerDAS 的「每个节点只下载部分 column」→ full Danksharding 的「通过 cell sampling 在 2D 网格上验证 DA」，每一步都在松弛网络层假设。理解这条松弛过程才知道 PeerDAS 解决的是哪一段。

### 重点研究

-   **DAS 的安全模型依赖一个不严密的「诚实少数」假设**：PeerDAS 假设网络中存在一定比例的 honest sampler，使得「所有数据都被采样到」在概率上成立。但这个假设的形式化其实并不严密——它要求 sampler 的采样请求既 unpredictable 又 unlinkable，否则 adversarial blob proposer 可以有选择性地公布「被采样到的那些 cell」，制造 partial release 攻击。当前 PeerDAS 通过 subnet 随机分配 + 私有 sampling 来缓解，但 unlinkability 在 P2P 层并不容易保证（IP 地址、连接模式都是 fingerprint）。学术上对 DAS 的形式化分析（Al-Bassam 2018、Hall-Andersen 2024）已经有进展，但仍然假设了理想化的网络模型，与 libp2p 的实际行为还有距离。
    
-   **1D vs 2D 编码的工程取舍**：PeerDAS 用 1D Reed-Solomon 编码（按 column 划分），full Danksharding 用 2D 编码（按 row 与 column 双向编码）。1D 的优势是实现简单、编码开销低，劣势是单个 column 的 unavailability 会导致整个 blob 不可重建；2D 通过冗余可以在更高失败率下重建数据。从 4844 走到 1D PeerDAS 已经是巨大工程，再过渡到 2D 需要重写大量 networking 代码。这是一个典型的 incremental rollout 策略——先用 1D 在主网验证 DAS 的端到端可行性，再升级到 2D。这种 progressive deployment 思路其实可以反过来借鉴到其他 EIP（例如 Verkle 迁移、SSF rollout）。
    
-   **KZG vs IPA vs FRI 的长期路径**：PeerDAS 当前用 KZG 承诺，因为它的 proof 大小最小、验证最快，但它依赖 trusted setup（KZG ceremony 已完成 14 万人参与）。长期来看，trusted setup 是一个 protocol smell——任何依赖 trusted setup 的组件都不是真正的 trustless。所以 roadmap 里有「quantum-safe and trusted-setup-free commitments」这一项。FRI-based 方案（如 STARK）能去掉 trusted setup，但 proof 大几个数量级；IPA 也无 trusted setup，但 verification 时间长。这个三难选择（succinctness / setup-freeness / verification cost）还没有完美解。最近 Binius 等基于 binary field 的方案给出了新的 trade-off 点，值得跟进。
    
-   **「网络瓶颈」vs「密码学瓶颈」的研究分工**：PeerDAS 的瓶颈在 networking 而不是密码学，这一点在 EPF cohort 之间有共识，但社群对此重视不足。当前 libp2p / GossipSub 的稳定性已经接近其设计极限，再加大 blob 数量需要新的 P2P 协议族——例如 episub、SyncCommittee-based dissemination、甚至基于 network coding 的 random linear network coding（RLNC）。这是一个被低估的研究方向。Cosmos 的 ABCI 抽象在这个层面其实有可借鉴的设计——把网络层与共识层完全解耦，允许独立演进。
    

### 收获

PeerDAS 章节最大的新的理解是：以太坊扩容的下一阶段瓶颈不在密码学也不在共识，而在 P2P 网络。这意味着 EPF / EF 的研究人员配置可能需要再平衡——目前 networking 工程师比 cryptographer 少得多。同时，DAS 的安全模型依赖「诚实采样者」假设的脆弱性，是一个被讨论但没有得到充分形式化的问题，潜在的研究价值很大。

### Insight

**DAS 真正解决的是一个分布式系统中的 fundamental impossibility——如何在不下载全部数据的情况下确认数据已被全网保存。** 这与 CAP 定理一样属于「看似可以工程化解决但其实有数学下界」的问题。DAS 的解法本质上是用概率论换一致性——通过随机采样把「所有 sampler 都能拿到 sample」与「数据已经全网公开」建立概率上的等价。这个等价不是绝对的，而是一个 (1-ε) 概率保证，其中 ε 取决于采样数与冗余度。这意味着以太坊的 DA 安全性在形式上不再是 binary 的，而是 quantitative 的——某个 blob 在某个 epoch 后的「可用性置信度」是 99.9999% 而不是 100%。这种从 binary 到 probabilistic 的转变在区块链协议中是首次出现，对未来的安全证明、insurance pricing、L2 fraud proof 设计都有方法上的意义。
...
「05 eODS」（economic objective of decentralized staking 相关讨论），EIP-7251（MAX\_EFFECTIVE\_BALANCE）、EIP-7514（max epoch churn），以及 Vitalik 关于 enshrined LST 的 H1\_5auGQd 笔记、Mike Neuder 的 two-tiered staking 与 Dankrad 的 liquid staking maximalism。

### 串联逻辑

staking 经济学不是一个独立模块，而是 The Merge / The Scourge / SSF 的交叉点。要理解 eODS 的研究方向，必须先把「为什么 32 ETH」这个 historical accident 拆解清楚——它是 BLS 签名带宽 + slashing 经济门槛 + UX 简化三个约束的妥协结果。所有当前提案（MAX\_EFFECTIVE\_BALANCE、orbit SSF、rainbow staking）都是在重新平衡这三个约束。

### 重点研究

-   **EIP-7251 改变的是签名经济学而不仅是 UX**：把单 validator 上限从 32 ETH 提到 2048 ETH，最直接的效果是大型 staker 不再需要维护成千上万的 validator 实例。但更深层的影响是签名总量下降——这正是 SSF 设计能够把 8192 签名/slot 当成假设的前提。换句话说，7251 是 SSF 的 enabling EIP，不只是用户体验改进。这个连锁反应在 roadmap 文档里没有明确标注，但读 Drake 的 SSF 设计稿能反推出来。理解这种隐性依赖对预测 Pectra / Fusaka 之后的 EIP 排序至关重要。
    
-   **流动性质押的「看不见的边界条件」**：Lido 等 LST 占据 stake 33% 以上是被反复讨论的中心化风险，但更微妙的问题是 LST 的 governance 机制本身——LDO 持有者（不是 stETH 持有者）实际上控制了 node operator 的选择。这就形成了一个 "stake without skin in the game" 的代理问题，与传统公司金融中的 separation of ownership and control（Berle-Means 1932）非常相似。Vitalik 提出的 enshrined LST 想从协议层切断这个代理链——把 staking 拆成「提供经济安全」与「运营 validator」两层。但这种 unbundling 也会消解 LST 的现有商业模式，因此利益阻力极大，技术上是否可行只是次要问题。
    
-   **rainbow staking 与 risk tranching 的金融属性增强**：Barnabé 等提出的 rainbow staking 把 staker 按「愿意承担的协议责任」分层——重质押者承担 attester / proposer / builder 等高责任高回报角色，轻质押者只承担 finality voting 等低责任低回报角色。这本质上是把 staking 做成了 risk-tranched product，与 CDO 的分层结构在形式上很像。要当心的是 CDO 历史上展示的 systemic risk 传递机制——当底层资产（即 staking yield + MEV）相关性突然上升时，最高层 tranche 也无法幸免。把这段金融史的教训搬到 rainbow staking 的设计阶段是有价值的。
    
-   **issuance policy 作为隐含的货币政策**：以太坊的 issuance curve（当前 stake ratio 提升时 issuance 自然下降）实际上是一种 algorithmic monetary policy，它的目标变量是「stake ratio 不要超过某个阈值」。Anders Elowsson 等人提出了 reduced issuance curves 的具体方案，争论焦点在于「应该把 stake ratio 稳定在 25% 还是 50%」。这个问题本质上跟央行选择目标利率是同一性质——它涉及对 (1) 网络安全所需的最低 stake、(2) ETH 作为 sound money 的 narrative、(3) staking 对 DeFi 流动性的挤占之间的权衡。把它当成宏观经济学问题而非协议参数问题来处理，能看到更多被忽视的政策维度。
    

### 收获

eODS 这一章把 staking 从「运维问题」提升到「协议层货币政策」的高度。一个收获是：以太坊的 issuance、slashing、validator size、LST 政策共同构成了一套 implicit monetary regime。这套 regime 没有独立的「理事会」也没有可解释性报告，但它对 ETH 的供给曲线、安全预算、机构参与门槛都有决定性影响。这种「治理空白」在传统央行视角下是不可想象的，但也正是以太坊的实验性所在。

### Insight

**以太坊的 staking 系统正在演化成一个 algorithmic central bank，而 EPF 社区其实是它隐形的 "monetary policy committee"。** issuance curve 决定了 ETH 的通胀率，slashing 决定了「惩罚性利率」，MAX\_EFFECTIVE\_BALANCE 决定了「准入门槛」，enshrined LST 的去留决定了「商业银行的角色」。把这些参数视为孤立的 EIP 是技术官僚视角，而把它们视为一个连贯的货币体系才是研究层应有的视角。值得思考的是：如果以太坊真的是一个 monetary regime，那它需不需要类似 Taylor rule 那样的可证明的 policy reaction function？目前的 issuance curve 只是一个静态曲线，对外部冲击没有反应能力。这可能是下一代 staking economics 研究的真正前沿。
...
「04 PBS」章节，配合 Mike Neuder 的 "Why enshrine proposer-builder separation"、Francesco D'Amato 关于 ePBS attribution 的分析、Barnabé Monnot 的 "More pictures about proposers and builders"。

### 串联逻辑

PBS 的研究路径是从「为什么需要分离」→「怎么分离」→「分离后如何防止新的中心化」三个层次推进的。第一层是制度问题（block building 已经过度专业化，validator 不再是合理的 builder），第二层是机制问题（commit-reveal、payment 时序、fork choice 影响），第三层是博弈论问题（builder set 的退出与进入门槛、relay 的去信任化、attester 的 conditional commitment）。

### 重点研究

-   **builder-proposer 分离的真正动因是 economy of scale 不可逆**：block building 的 sophistication 已经远超普通 validator 能维持的水平——backrunning 一个 oracle update 需要监听 50+ DEX 池子、维护 sub-millisecond 报价系统、运行专业的搜索者网络。这种专业化不是政策选择，而是市场必然。如果不在协议层做 PBS，结果就是少数大型质押池吞下所有 MEV，导致 stake 集中。从这个角度看，PBS 不是为了「让某些人赚 MEV」，而是为了「让普通 solo staker 能与机构 staker 拿到相同的预期收益」。这个视角下，ePBS 是 anti-centralization 政策工具，而不是 throughput 优化。
    
-   **ePBS 的核心难题是「honest builder problem」**：当 proposer 只承诺一个 builder 的 bid（而不是直接揭示 payload）后，builder 可以选择 reveal 也可以选择不 reveal。后者会让 slot 出空块，但 builder 可能因此规避了不利的市场变化（例如 reveal 之前价格反向）。这就给 builder 留下了一个 free option。当前研究（Vitalik、Justin Drake）的方向是用 conditional payment（builder 必须先 burn ETH 才能进入 bid）和 view-merge fork choice（attester 优先确认包含 payload 的 chain head）来惩罚 non-reveal。这个机制的均衡分析涉及 option pricing 与博弈论的交叉，目前还没有完整证明它能在所有市场波动条件下保持 incentive compatible。
    
-   **Execution Tickets 的市场设计意义**：ET 把「执行权」抽象成可交易的 ticket，这其实是把以太坊的 slot 从「周期性义务」转变成「可拍卖的稀缺资源」。这种设计与传统拍卖理论中的 spectrum auction、pollution permit market 有结构相似。值得探讨的问题是：ET 的二级市场会不会变得像电力市场那样过度金融化——做市商、对冲基金主导 ticket 流动性，反而把 sophisticated proposer 排挤出去？历史上美国 1990s 的电力市场化就出现过类似问题（Enron 危机的部分根源）。把这些教训纳入 ET 的设计是一个被忽视的跨学科切入点。
    
-   **Inclusion List 的机制限度**：IL 让 proposer 强制 builder 必须包含某些交易，是 anti-censorship 的关键工具。但 IL 本身会被博弈：恶意 proposer 可以提交一个故意冲突的 IL（例如包含一笔会使 builder 收益归零的 sandwich-killer 交易）来勒索 builder 分润。COMIS（committee-enforced inclusion sets）通过让多个 attester 共同投票形成 IL，能减少单个 proposer 的勒索能力，但又引入了 committee coordination 的攻击面。这是一个典型的「机制设计上的此消彼长」——任何强化某一方权力的设计都会催生新的攻击向量，最终需要在多个攻击模型之间做权衡。
    

### 收获

PBS 这一章揭示了一个被低估的事实：以太坊的协议演化越来越像微观经济学的应用学科。从 EIP-1559 的费用市场，到 ePBS 的双重拍卖，再到 ET 的二级市场，每一项升级都在协议层引入新的市场机制。这意味着未来的 EIP 评审可能需要博弈论与机制设计的专业能力，而不只是密码学与分布式系统。Ethereum Foundation 内部已经在设立 "incentives team"，这是这种趋势的组织层面的信号。

### Insight

**ePBS 的本质是把以太坊从「基于义务的协议」重构为「基于市场的协议」。** PoW 时代矿工「自由竞争出块」，PoS 早期 validator「按周期履行义务」，ePBS 之后 proposer 的核心职能退化为「主持一场拍卖」，真正的执行权通过市场机制流向最高效的 builder。这种转变在传统系统里有先例——从国营经济到拍卖式的政府采购、从公共事业垄断到电力批发市场都走过类似的路。值得借鉴的是：所有走过这条路的传统市场最终都需要一个独立的 market regulator 来监督拍卖公平性、防止串谋、处理异常事件。在放到以太坊这里，这个 regulator 角色由谁承担——是 social layer（Twitter、EIP discussion）、是 client team、还是某种 protocol-enforced anomaly detector——尚未有共识。这可能是 ePBS 之后最关键的开放问题。

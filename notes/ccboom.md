...
-   它是一个 cryptographic hash function。
    
-   在 Ethereum 里大量使用，比如地址、区块头、MPT 节点、storage key、交易/收据 trie 等。
    
-   它的作用不是加密可逆数据，而是生成“数据指纹”：输入稍微变一点，输出就完全不同。
    

**2\. 历史背景**

-   Keccak 最早是提交给 NIST 新哈希算法竞赛的方案。
    
-   这个竞赛目标是找一个替代 SHA-1 的安全哈希算法。
    
-   Keccak 的设计团队是 Guido Bertoni、Joan Daemen、Michaël Peeters、Gilles Van Assche。
    
-   后来 Keccak 赢了 SHA-3 竞赛，但 Ethereum 使用的是标准最终定稿前的 Keccak256 版本。
    

**3\. Sponge Construction**  
Keccak 的核心设计叫 sponge construction，海绵结构。
...
-   每块和内部 state 的 bitrate 部分做 XOR。
    
-   每次吸收后，都对整个 state 做 permutation，让输入充分混合。
    
-   bitrate r 决定一次能吸收多少数据，也影响吞吐效率。
...
-   从 state 的 bitrate 部分取出结果。
    
-   理论上 sponge 可以输出任意长度；Keccak256 固定取 256 bit。
    

**4\. EVM 里的实现**  
原文特别提醒一个容易混的点：

-   EVM 里没有叫 KECCAK256 的 opcode。
    
-   但有一个 opcode 叫 SHA3。
    
-   这个 SHA3 opcode 实际输出的是 Keccak256 hash。
    

也就是说，在 EVM 语境里看到 SHA3，你要知道它历史命名有点旧，实际是 Ethereum 用的 Keccak256，不是标准化后的 SHA3-256。

**5\. Ethereum 里用在哪里**  
页面列了几个关键位置：

-   Block Header：比如 parentHash、stateRoot 等字段依赖 Keccak 256-bit hash。
    
-   Merkle Patricia Tree：MPT 节点通过“节点内容的 Keccak256 hash”来标识。
    
-   Storage Encoding：合约 storage slot key 会被 Keccak256 处理，再映射到 RLP 编码后的 value。
    
-   Transactions / Receipts Trie：相关 trie 的 root 也依赖这种哈希承诺结构。
    

一句话：Keccak256 是以太坊数据结构的“指纹机器”，让节点能高效验证数据有没有被改。

**6\. Keccak256 vs SHA3-256**  
这是最重要的坑。

原文引用 Nick Johnson 的说法：

-   SHA3-256 基本上就是 Keccak256，但 padding 方式不同。
    
-   Ethereum 在 SHA-3 标准最终 padding 变更前就已经定义了协议，所以继续使用旧 padding 的 Keccak256。
...
-   以太坊里说的 hash，通常是 Keccak256
<!-- DAILY_CHECKIN_2026-04-29_END -->

# 2026-04-28
<!-- DAILY_CHECKIN_2026-04-28_START -->

BLS 是 Ethereum 共识层 validator 使用的数字签名方案，全称是 Boneh-Lynn-Shacham signature。它和执行层普通交易使用的 ECDSA 不同，BLS 的最大优势是可以做签名聚合：很多 validator 各自签名后，这些签名可以合成一个聚合签名，从而显著降低 Beacon Chain 在处理大量 attestations、blocks、sync committee messages 时的带宽和验证成本。这也是为什么 BLS 对 PoS 很重要，因为 Ethereum 有大量 validators，如果每个签名都单独存储和验证，共识层会非常沉重。

BLS 的数学基础是椭圆曲线上的双线性配对 pairing，形式是 e: G1 × G2 -> GT。validator 的私钥 sk 是一个标量，公钥是 pk = sk × g1，其中 g1 是 G1 上的生成点。签名时，Beacon Chain 不会直接签任意原始消息，而是先对 SSZ 对象计算 hash\_tree\_root，再通过 compute\_signing\_root() 加入 domain，得到 32 字节的 signing root；然后把这个 signing root 通过 hash-to-curve 映射到 G2 上，得到 H(m)，最终签名就是 σ = sk × H(m)，压缩后通常是 96 bytes。

验证签名时，不需要私钥，只需要消息、公钥和签名。验证者检查 pairing 等式 e(g1, σ) = e(pk, H(m)) 是否成立。如果成立，说明这个签名确实是由对应公钥背后的私钥对该消息产生的；如果消息被改、公钥不对或签名损坏，等式就不成立。BLS 不像 ECDSA 那样每次签名都依赖随机 nonce，因此避免了一类 nonce 复用或生成不当导致私钥泄露的问题，但生成私钥本身仍然必须依赖安全随机数。

在 Ethereum 里，BLS 主要服务共识层：validator 用它签 block、attestation 和其他共识消息；普通执行层交易仍然使用 ECDSA。BLS 的聚合能力让大量 validator 的投票可以被压缩处理，使 Beacon Chain 能支撑几十万甚至更多 validator 参与共识。因此，BLS 的核心价值可以概括为：用 pairing 验证签名正确性，用聚合降低大规模共识投票的成本。
...
当ethereum交易时，系统要确认交易是账户持有人授权的

ECDSA是一种让别人验证，这确实是你授权的消息的工具

它有一对key

public key是给别人公开的，用来验证身份和验证签名

private key只能自己知道，用来签名和解密

在Ethereum交易中，你应该用private key来签名，然后别人用public key去解密

ECC是公钥密码学的一种，使用椭圆曲线数学，可以用更小，更高效的key，达到很强的安全性

ECDSA = Elliptic Curve Digital Signature Algorithm

他的作用是验证提交到 Ethereum的交易是否合法，是否由账户持有人授权

先看看椭圆曲线 y² = x³ + ax + b

并要求 4a³ + 27b² ≠ 0

这个条件是保证曲线没用尖点交叉点等特殊情况

以太坊使用的是 secp256k1 曲线，它的参数是：
...
椭圆曲线有趣的地方在于，曲线上的点可以形成一个group

也就是说，两个曲线上的点做加法，结果仍然是曲线上的点
...
这就是ECC的核心运算，私钥是数字，公钥是这个数字乘以某个固定曲线点得到的曲线点

P = K \* G

G 是公开的 base point

K 是 secret key，也就是私钥

P 是 public point，也就是公钥点

从k和G算出p很容易

但是从p和G反推k非常困难

ECDSA 的安全性来自 scalar multiplication 的单向性：正向容易，反向极难

有限域椭圆曲线：真实 ECDSA 不是在连续实数曲线上运算，而是在有限域里做确定性模运算。Ethereum 的 secp256k1 使用巨大素数域，保证计算可重复、离散且安全。

生成密钥对：私钥 K 是一个随机数字，公钥点是 P = K \* G。G 是公开基点，P 可以公开，但从 P 和 G 反推 K 在现实中不可行。

签名过程：签名会先 hash 消息得到 m，再生成一次性临时密钥 eK，算出临时公钥点并得到 r，最后结合私钥算出 s。签名结果是 (r, s)，其中临时密钥不能泄露或复用。

验证过程：Bob 用消息 hash、Alice 公钥 P 和签名 (r, s) 重新计算出一个点 R，检查 R 的 x 坐标是否等于 r。如果消息被篡改，hash 会变，验证结果就会失败。

Ethereum 里的 ECDSA：Ethereum 账户用 ECDSA 签交易，实际环境使用 secp256k1、keccak256 和巨大素数域。页面也提醒不要自己实现密码学，应使用可信库。
...
今天学习的是 FCR = Fast Confirmation Rule，中文可以先记成“快速确认规则”。

**核心问题**

-   以太坊现在最硬的确认方式是 FFG finalization，安全但慢：最好约 13 分钟，平均约 16 分钟。
    
-   很多场景等不了这么久，比如钱包显示确认、CEX 入金、日常支付。
    
-   但“交易进块就算确认”或者“等 N 个后续块”都只是 heuristic，不满足严格安全性质。
    

**FCR 是什么**

-   FCR 是一个算法，用来判断某个 block 是否“不会离开 canonical chain”。
    
-   它输出很简单：confirmed 或 not confirmed。
    
-   它的目标是在 SSF 还没实现之前，提供比 finalization 快很多、但仍有形式化保证的确认信号。
    
-   最理想情况下，它可以做到约 12 秒，也就是 1 个 slot 级别的确认。
...
-   它依赖 synchrony，也就是网络健康：诚实验证者在某个 slot 发出的 attestations，其他诚实验证者能在该 slot 结束前收到。
    
-   它还假设任意 committee 里最多有一部分 stake 是 dishonest，文中给的可配置范围是 β = 20-25%。
    

**它保证什么**

-   Safety：被 FCR 确认的 block，在诚实节点视角里不会被 reorg 掉。
    
-   Monotonicity：confirmed block 不会倒退，除非触发 reset-to-finalized，这表示 FCR 的假设可能失败，需要退回到 finalized checkpoint。
    
-   原文最重要一句是：FCR never confirms a block that is not canonical.
    

**为什么不能用简单替代方案**

-   Block depth heuristic：比如等 6 个块、12 个块。这只是经验，不知道当前 fork choice 和投票支持是否真的稳。
    
-   Justification-based heuristic：比纯 block depth 好一点，但仍然缺少真正的 confirmation 信息。
    
-   这些方法都不满足 FCR 想要的形式化 safety property。
    

**Gasper 背景**

-   Ethereum PoS 共识叫 Gasper。
    
-   LMD-GHOST：fork-choice 算法，决定当前 canonical head。
    
-   FFG-Casper：finalize checkpoints，提供最终性。
    
-   时间单位是 slot，当前 12 秒；epoch 是 32 个 slots。
    

**FCR 怎么挂到 LMD-GHOST 上**

-   FCR 是给 LMD-GHOST 增加一个快速确认规则。
    
-   它从上一个 confirmed block 开始，看当前 canonical chain 上后续哪些 block 可以继续确认。
    
-   入口函数叫 get\_latest\_confirmed，分两步：
    
-   第一，检查假设是否还成立。
    
-   第二，沿 canonical chain 推进确认位置，找最新 confirmed descendant。
    

**isOneConfirmed**

-   原文提到一个核心谓词：isOneConfirmed。
    
-   算法会沿着 canonical chain 找最长前缀，只要每个 block 的 isOneConfirmed 为 true，就可以继续往前确认。
    
-   如果某个 block 通过检查，说明它有足够的 LMD-GHOST 支持，可以抵抗 sibling branch takeover。
    
-   这个检查要考虑很多边缘因素：proposer boost、攻击者预算 β、equivocation、empty slot discounting。
    
-   所以 FCR 复杂，不是因为目标复杂，而是因为要把 fork choice 可能被扭动的边界情况都算进去。
    

**实际意义**

-   钱包可以更可靠地显示“安全确认”，而不是交易一进块就乱标 confirmed。
    
-   CEX 可以降低“入金后立刻交易但交易又被 reorg”的风险。
    
-   PBS/Builder 场景里，builder 也能更快知道自己的 block 是否大概率不会被 reorg 掉。
...
今天这页讲的是一个还在研究中的方向：eODS = Enshrined Operator-Delegator Separation，也就是把现在链下自然存在的“出钱的人”和“跑节点的人”的分工，部分写进协议层。

eODS 想把 validator 角色继续拆开：operator 负责实际运行和服务，delegator 负责提供资本或轻量信号，让协议更好处理质押中心化、LST 风险、SSF 签名压力和抗审查问题。

现在流动性质押天然分成两类人：Delegators 出 ETH，Operators 跑 validator。

问题是典型的 principal-agent：delegator 是本金提供者，operator 是代理人；operator 作恶或失误被 slash，delegator 的钱也受损。

所以 LST 系统的可信度依赖一个假设：operator 是“good agent”，会正确验证、不双签、不恶意偏离协议。

eODS 的出发点就是：既然这个分工已经存在，协议要不要直接识别并约束它？

**eODS 是什么**

ePBS 拆的是 proposer-builder。

Execution Tickets 拆的是 validator-proposer。

eODS 拆的是 operator-delegator。

这三者都是在“拆 validator 的 bundled role”，让不同职责由不同市场角色承担。

页面把 eODS 和 SSF 联系起来，因为 SSF 下每 slot 聚合太多 BLS 签名会有技术压力。

解决思路是两层参与者：Heavy services 和 Light services。

Heavy services 是高复杂度层，大约少于 10,000 个参与者，每 slot 都参与，承担 finality/Gasper 这类重服务，高奖励，也可被 slash。

Light services 是低复杂度层，偶尔参与，硬件和技术要求低，做抗审查信号、inclusion lists、multiplicity gadgets 等轻服务，奖励低，可能不 slash 或只在短期 opt-in slash。

**Delegator 的意义**

页面引用 Vitalik 的问题：从协议角度看，delegator 到底有什么用？

如果 delegator 只是把钱交给 operator，然后什么都不做，协议可能还不如直接降低收益、减少中间层。

eODS 想让 delegator 做“有意义的事”：选择和约束 operator，或者参与轻量抗审查服务。

在 eODS 下，delegator 不一定参与 FFG finality 的经济安全，但可以提供额外信号，比如对 inclusion list、当前链头、抗审查机制表达看法。

**1D 和 2D**

单独的 1D-eODS 只是把 operator/delegator 分开，但如果两者仍然都 slash，改善不大。

如果加上 penalty capping，可以让 delegator 不被 slash，只 slash operator，但页面认为这仍然角色不够清晰。

更完整的是 2D-eODS，也叫 rainbow staking：同时区分 Heavy/Light 服务，并且每一层里又有 operator/delegator。

结果就是：Heavy operator + Heavy delegator 支撑 finality 和经济安全；Light operator + Light delegator 支撑抗审查和信号发现。

**Solo Stakers**

页面认为 solo staker 在这个新世界里仍然有重要价值。

第一是增强网络韧性：大型 operator 掉线时，solo staker 可以作为 fallback 继续帮助链前进。

第二是提供 preference entropy：更分散的 solo staker 能表达更多样的偏好，帮助协议看见被审查或被忽略的输入。

所以 solo staker 不一定在资本效率上胜过大型 LST，但在去中心化和抗审查上有独特价值。

而且不能只靠 social layer 和道德约束来防止 LST 中心化。

operator/delegator 分工已经是现实市场结构，eODS 只是把它协议化。

eODS 可以让 delegator 有更强的共识参与方式，而不是只持有 LST。

在 SSF 场景里，把 heavy validator 数量压到少于 10,000，可以减少每 slot 需要处理的 BLS 签名数量。

**和其他研究的关系**

和 Execution Tickets 配合：ET 拆 execution 和 consensus，eODS 拆 operator 和 delegator。

和 Inclusion Lists / Multiplicity Gadgets 配合：light services 可以生成抗审查列表，heavy services 负责验证 payload 是否满足约束。

和 EIP-7251 MAX\_EFFECTIVE\_BALANCE 配合：单个 validator 消息可以承载更多 stake，协议也更需要区分 operator stake 和 delegator stake。
...
由于我之前一直在cosmos生态，所以我非常熟悉celestia的运作

本质上celestia就是扩展的数据和数据可用性验证

那么什么是 PeerDAS？

全称是 Peer Data Availability Sampling，它是一个网络协议

主要处理的是rollup提交到Ethereum的blob数据

rollup需要把自己的交易数据发布到Ethereum，方便别人验证L2状态转换是否正确。

问题是如果每个节点必须下载所有blob数据，Ethereum想扩容的时候，普通节点的贷款和存储压力很快就会爆炸

所以PeerDAS不是要求每个节点下载所有数据，但仍然让网络能够确认这些数据是可用的

总共实现要分三步走：

Stage 0：EIP-4844

EIP-4844 只是 proto-danksharding，是通往 Danksharding 的中间步骤

引入blob，有blob分发子网，但不是真正的DAS，节点仍需要下载全部blob数据

Stage 1：1D PeerDAS

水平扩展blobs，映入Column subnets，节点通过peer sampling提高效率

这个时候节点只负责部分数据和采样部分数据

Stage 2：2D PeerDAS

完整 Danksharding 方向，增加 vertical blob extensions，使用更加轻量的 cell-based samppling

PeerDAS会把blob数据拆成更小的单位，叫columns

这些columns是数据采样的原子单位，然后网络会把节点分配到不同的 custody groups，每个group 负责特定的一组columns

PeerDAS 不让每个节点保存全部数据，而是让每个节点按规则保管一部分数据

把原始数据扩展成更多数据片段，使得即使丢了一部分，也能恢复原始数据。

PeerDAS 不是简单把 blob 切碎，而是先用纠删码增加可恢复性，再把列分发到网络

讲一下 Data Availability Sampling 这是PeerDAS的核心

节点拿到了足够多的随机样本，那么就使用概率的方法可以推断完整数据是可用的

数据可用性问题是：真实的数据到底发布了没，别人能拿到吗？

如果恶意的peoposer只发布了一小部分数据，藏起其他部分，那么L2或者其他参与者就可能无法验证状态转换

PeerDAS的思路是：非常多的节点随机抽样，抽样足够多，那么就能验证是否是正确的，用全网分布式随机抽样来获得高置信度的数据可用性判断

PeerDAS 是 Ethereum 为 Danksharding 和 rollup 扩容准备的数据可用性网络协议：它让节点通过抽样和验证部分数据列，来高概率确认完整 blob 数据可用，从而提高吞吐而不显著提高普通节点负担。

你现在最该记住的 6 句

PeerDAS 是 EIP-7594 引入的数据可用性采样网络协议。

它服务 rollup-centric roadmap 和 The Surge 扩容路线。

它把 blob 转成 columns，并让节点按 custody groups 负责部分 columns。

它用 Reed-Solomon 纠删码保证缺部分数据也能恢复。

它用 KZG commitments 验证采样数据确实匹配原始承诺。

它把数据可用性检查接入 validator fork-choice，确保节点只支持数据可用的 blocks。
...
PBS这页的核心问题是：出块这件事，是否应该继续由同一个validator同时负责 构造区块和提议区块，还是应该拆开？

PBS是什么？

在Ethereum当前系统重，validator同时做两件事情

创建区块和广播区块

而PBS - Proposer-Builder Separation 的核心就是把这两件事拆开

builder负责构造block，proposer负责在某个slot重选择一个block并提议出去

在PBS里，porposer不一定完整看见区块内容，而是从builider候选里面选最赚钱的那个

为什么PBS重要

第一点是去中心化

PBS之所以重要，是因为它可以降低册灰姑娘为validator所需要的计算负担

PoS 时代 Ethereum 的一个总体方向，就是通过模块化提升去中心化

PBS 的意义不只是提高收益，而是把 block production 模块化，从而降低 validator 侧门槛

PBS（Proposer-Builder Separation）总结

PBS 的核心思想，是把传统 validator 同时负责“构造区块”和“提议区块”的模式拆开。builder 专门负责收集交易、排序交易、构造最赚钱的 block；proposer 则在自己的 slot 里从候选 block 中选一个提交到网络。这样做的目标，是降低 validator 的计算门槛，让出块流程更模块化，也更符合 Ethereum 走向模块化和去中心化的长期方向。

当前主网广泛使用的并不是协议内 PBS，而是协议外方案，典型就是 mev-boost。现实中的参与链条通常是：searcher -> builder -> relay -> proposer。searcher 负责发现 MEV 机会并构造 bundles，builder 负责打包最优 block，relay 负责在 builder 和 proposer 之间转发、验证和撮合，最终 proposer 选收益最高的 block 提议到网络。

这种协议外 PBS 虽然有效，但问题也很明显。最大的风险来自 relay 和 builder 的中心化：少数 relay 掌握很大市场份额，会带来审查风险、信任问题和单点依赖。再加上 PBS 把出块过程拆给了更多外部参与者，也会增加失效模式和攻击面，比如组件不兼容、第三方服务异常导致 missed slots。

因此，Ethereum 社区正在研究更稳健的方案。ePBS 想把 PBS 直接做进协议，减少对外部 relay 的依赖；PEPC 想让 proposer 可以在协议层注册并强制执行更通用的外包承诺；Inclusion Lists 则是在 builder 强势的环境下，给 proposer 留下一部分强制交易纳入权，用来增强抗审查能力。

一句话总结：

PBS 是为了把 block building 专业化、把 proposer 负担降下来，但当前协议外实现带来了中心化和审查风险，所以社区正朝着 ePBS、PEPC 和 Inclusion Lists 这类协议内方案继续推进。
...
今天学习 [https://epf.wiki/#/wiki/research/scaling/scaling](https://epf.wiki/#/wiki/research/scaling/scaling)

首先我们要知道什么是扩展性

scalability 是 系统在工作负载增加时，仍能良好运行的能力

区块链的扩展性，是它在不提高节点运行者要求的情况下，处理越来越多操作，越来越多交易的能力

如果TPS提高了，但是节点要求也暴涨，那根本不能算好的扩展

为什么传统区块链很难无限提高吞吐

共识本身就有开销，每个区块被加进链的时候，都需要足够多的validator达成一致

这就引出了第一个限制：block latency ，一个有效区块被纳入链中所需要的时间

对Ethereum来说，理想slot的时间是12s，因为CL的规格里有 SECONDS\_PER\_SLOT= 12

但实际不可能永远完美，网络传播和验证也会带来偏差

不可能无限快的产生并确认区块，因为共识本身就有时间节拍的限制

再说说第二个核心参数：block size

Ethereum不是直接说一个区块多少多少字节，而是通过gas\_limit 间接限制区块能装多少计算与数据

每个操作都要消耗gas，。一个区块里的总gas不能超过gas\_limit，所以区块里的交易数量和复杂度都被限制了

EIP-1559说，协议允许区块gas使用量围绕gas\_target动态波动

这意味着Ethereum不是死板固定块大小，但也绝不会无限放大

你不可能无限往单个区块里塞计算和数据，因为节点必须跟得上梳理

TPS是由block latency和block size 共同决定的

块出的越快，TPS越高，块更大，TPS也可能更高

你当然可以构造一个高性能、低延迟、大区块的链，但那通常会更中心化。

系统变为少数高性能节点，高带宽内网级连接，巨大区块

但是代价就是普通人跑不起，去中心化下降，信任假设变重

所以TPS不能脱离去中心化成本单独看

还有一个被忽略的问题：state size

区块链不只处理交易，还要维护状态

data availability = 需要时，数据必须可访问

如果状态太大，就会导致存储需求过高，同步时间过长等，直接伤害去中心化

扩展性问题不仅是交易吞吐量问题，也是状态增长问题

因为TPS有限，而区块空间是稀缺资源，所以当交易需求增长的时候，费用会上升

而且经典区块链很难满足三件事：

security，decentralization。scalability

传统区块链系统很难在不牺牲去中心化或者安全性的情况下，做到面向大众的大规模扩展

在单链架构里，想要大幅度提高吞吐，通常就会碰到共识时间，区块大小，状态增长，节点成本等硬限制
...
不再强迫一条链同时吧所有事都在同一层做的最优，而是把职责拆开
...
L1扩容不是简单家gas limit 或者加大区块，必须配套其他协议，才能避免把节点门槛推高

L1扩容有空间，但不是无限的，必须和去中心化约束一起考虑

Layer 2 scaling

建立在L1纸上的方案，在不显著增加L1资源消耗的前提下，提高整体TPS

然而Ethereum 社区最终主要采用的是 multi-rollup-centric 路线

Rollup把很多L2交易打包成一个L1交易，数据仍然提交到L1，执行则放到链下或者链外环境。只把必要数据和状态承诺放回L1

把执行做压缩，把安全锚定留在 L1

Ethereum 的主扩容路线，不是无限做大 L1，而是让 L1 主要做 settlement + DA，让 rollups 承担大部分 execution。
...
Ethereum没有一个单一的路线图，但社区研究和开发长期围绕几类核心目标推进，这些目标合起来就构成了大家口中的roadmap

Ethrerum的开发哲学是：协议可以演化，只要收益足够大，风险可控，就愿意升级

没有官方路线图，没有单一权威机构决定一切

Ethereum roadmap 不是命令式计划，而是开放研究驱动下形成的长期共识方向

核心协议的研究和开发是公开，自由，可参与的

这句话的意思不是“谁都能今天提个想法明天就进主网”，而是：
...
所以 roadmap 不是封闭产品团队的内部排期，而是开放研发生态不断收敛出来的结果

Ethereum Protocol Roadmap 六大方向总结

The Merge

The Merge 这一类最初围绕 Ethereum 从 PoW 切换到 PoS，但现在已经扩展成对整个 PoS 共识系统的持续优化。已经完成的关键步骤包括 Beacon Chain 启动、执行层与共识层合并，以及提款功能上线。后续研究重点包括单 slot finality、单秘密 proposer 选举、validator 集合扩展，以及面向未来的抗量子签名。它的核心主题是：让 PoS 共识更快、更稳、更安全。

The Surge

The Surge 主要关注扩容，尤其是以 rollup 为中心的数据可用性扩容路线。EIP-4844 已经把 blob 引入协议，为 rollup 降低成本迈出了关键一步。后续目标包括完整的 danksharding、数据可用性采样（DAS）、更成熟的 fault proof / zkEVM 体系，以及更强的数据承诺方案。它的核心主题是：给 L2 和 rollup 提供足够便宜、足够安全的数据可用性空间。

The Scourge

The Scourge 关注的是扩张过程中出现的权力集中问题，尤其是 MEV、审查阻力和流动性质押中心化。这个方向既包括已经落地的 MEV-Boost 和更高的 validator 有效余额上限，也包括 ePBS、MEV-Burn、Inclusion Lists、Distributed Block Building、Preconfirmations 等研究。它想解决的是：Ethereum 在追求效率和收益时，不要变成更容易被少数 builder、LST 或大玩家操控的系统。核心主题是：在现实经济激励下，尽量保住去中心化和抗审查能力。

The Verge

The Verge 重点是让区块和状态验证变得更容易，尤其是对轻客户端和未来无状态方向更友好。页面里最典型的两项是 Data Availability Sampling 和 Verkle Trees，它们分别帮助降低数据验证和状态证明的成本。这个方向不是单纯为了“更快”，而是为了“更容易验证”。核心主题是：让更多节点能以更低资源成本安全验证链上信息。

The Purge

The Purge 关注的是协议膨胀问题，也就是历史和状态无限增长带来的节点负担。它的代表方向包括 History Expiry（如 EIP-4444）和 State Expiry，目标是让客户端不再被迫永久保存所有老数据和所有长期不活跃状态。这样可以降低磁盘压力、提高节点可运行性，并让系统更可持续。核心主题是：通过清理旧数据和状态，把协议负担控制在合理范围内。

The Splurge

The Splurge 是那些不一定最紧急，但对长期可用性、安全性和未来适应性很重要的增强项集合。页面里举的例子包括账户抽象、抗量子签名研究，以及形式化验证工具链。它不像前几类那样围绕单一技术瓶颈，而更像是对协议长期韧性和用户体验的补强。核心主题是：把一些重要但不容易归类的长期改进逐步补齐。
...
今天学习内容还没更新，暂时学习一下密码学

密码学给协议提供的是 可验证的 可依赖的约束
...
每笔Ethereum交易都要用ECDSA在secp256k1曲线上签名，对EOA来说，每次操作都离不开签名

这一块核心解决的问题就算看看交易是不是账户本人批准的，害怕别人伪造

EOA和ERC4337这种账户抽象模型不同，传统EOA的每次操作都要靠签名

ECDSA 在 Ethereum里面最核心的职责，就算给交易做授权证明

Keccak-256: Beyond "Just a Hash Function"

这一部分讲hash在Ethereum里不只是做摘要，而是协议结构本身的一部分

有两个用途：block hashing 和 state trie keys/node references

很多人吧hash函数理解为把数据压缩成一长串固定长度的字符串

但是在Ethereum里面，Keccak-256更像一个基础构建，区块标识靠它，trie里的引用靠它，状态结构能被高效追踪也靠它

Keccak-256 是Ethereum协议结构的骨架级别原语，而不是随便hash一下

BLS Signatures in PoS: Tradeoffs and Optimizations

这一部分讲的是PoS时代为什么CL用BLS，而不是继续用ECDSA

因为BLS支持签名聚合

成千上万哥validator的attestation可以压缩成一个 96-byte proof，这就是BLS在pos中最大的价值

ECDSA 负责交易授权，BLS 负责 PoS 大规模投票聚合。

Verkle Trees: A Shift in State Proofs

这一部分讲的是 为什么 Ethereum 想从现有状态证明结构继续往前演进。

现有的 storage merkle proof 太大，大约1KB，用KZG，把witness能压缩到大约200bytes

Verkle 不是“树换个名字”，而是状态证明大小和结构的一次重大升级方向。

Verkle 是为了把状态证明做得更小、更适合未来扩展，但它带来新的工程和密码学折中。

ZKPs in Practice: More Than Just Rollups

这一部分讲 ZKP在Ethereum中不只服务 rollup

EVM的opcode成本是变化的，这是zkEVM的难点之一

很多人会把ZKP简化为一句：拿来做rollup扩容

但并不是这样

ZKP 在 Ethereum 里的应用面更广，但要真正工程化非常难。

ZKP 在 Ethereum 里是一个跨扩容、验证、隐私的通用方向，但 EVM 本身对电路友好性并不好。

Post-Quantum Readiness: Not Just Theoretical

这一部分讲的是：量子计算对 Ethereum 现有密码学会带来什么威胁

量子计算会更直接威胁ECDSA，伪造签名更是容易

不要一夜之间把旧体系全扔掉，而是先做过渡性双轨方案。

后量子问题在 Ethereum 里不是纯理论话题，但真正迁移会非常重，尤其是密钥和签名体积问题。
...
1.  CL 的核心任务是维护 beacon chain，处理 beacon block 和 attestation，并和 EL 一起完成以太坊 PoS 共识。
    
2.  由于网络延迟、消息乱序和恶意行为，链上会同时出现多个分叉，所以必须用 fork choice rule 从 block tree 里选一个 head。如果后面发现另一条分支更优，就会发生 reorg。
    
3.  Ethereum 的 PoS 共识不是单一协议，而是 Gasper = LMD GHOST + Casper FFG。
    
    -   LMD GHOST 负责 liveness，保证链继续前进
        
    -   Casper FFG 负责 finality，防止长回滚
        
4.  Ethereum 在极端网络问题下更偏向保 liveness。也就是说，宁可先继续出块，也可能暂时失去 finality；所以它不是在所有情况下都绝对安全和绝对活。
    
5.  共识层的主要组件是：
    
    -   Beacon Node：和其他 CL 节点、EL、本地 validator 通信，运行共识逻辑
        
    -   Validator：质押 32 ETH 后参与提议和投票，只通过本地 beacon node 对外工作
        
6.  CL 和 EL 通过本地 Engine API 协作：
    
    -   不是 proposer 时，CL 收到 block 后把 execution payload 发给 EL 验证
        
    -   是 proposer 时，CL 让 EL 构造 execution payload，再把它装进 beacon block 广播
        

最后，Beacon Chain 的状态更新是 slot-driven 的，不是单纯“有块才更新”：
...
-   有 per-epoch  
    三层状态转换。所有节点如果从同一个 genesis state 出发、对同样的 block 跑同样的 state transition，就应该得到同样的 beacon state。

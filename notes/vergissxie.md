...
# Roadmap overview（路线图）

来源

[Roadmap overview](https://epf.wiki/#/wiki/research/roadmap)

## 学习定位

-   这是第 3 周最适合先读的一页，因为它不是单个机制，而是帮你把 Research 区所有主题放进一张总图里。
    
-   读这页的目标不是记住每个框，而是先建立“这些研究分别在解决哪一类问题”的空间感。
    

## 一句话总结

-   Ethereum roadmap 不是一张官方强制执行的工程排期表，而是一个由研究者、客户端团队、社区讨论长期共同塑造的演化方向图。
    

## 路线图的阅读方法

-   不要把 roadmap 看成线性路线，而要看成几条并行主线：
    
    -   `The Merge`：共识层升级与 PoS 相关后续优化。
        
    -   `The Surge`：扩容，尤其是 rollup-centric scaling。
        
    -   `The Scourge`：MEV、抗审查、质押中心化风险。
        
    -   `The Verge`：更轻量的验证与状态证明。
        
    -   `The Purge`：减少历史与状态负担。
        
    -   `The Splurge`：杂项但重要的长期增强。
        
-   第 3 周主要会落在 `The Surge`、`The Scourge` 和一部分 `The Verge`。
...
-   已完成的重点是 Beacon Chain、The Merge、本金与收益提取。
    
-   继续延伸的研究包括：
    
    -   `SSF (Single Slot Finality)`：把确认/最终性尽量压缩到一个 slot。
        
    -   `SSLE (Single Secret Leader Election)`：减少 proposer 暴露后的 DoS 风险。
        
    -   `更多 validator 与更高 MAX_EFFECTIVE_BALANCE`：重新平衡参与门槛与协议开销。
        

### 2\. The Surge

-   核心问题：如何扩容，同时不把节点硬件门槛推高到只剩大机构能跑。
...
-   关键路径：
    ...
    -   `rollup training wheels removal`
        
-   这一条主线和 [Scaling](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/Scaling)、[PeerDAS](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/PeerDAS)、[Light clients](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/Light clients) 紧密相连。
    

### 3\. The Scourge

-   核心问题：当排序权和质押权越来越值钱，协议如何避免被 MEV 与大规模资本结构拖向中心化。
...
-   关键研究：
    ...
    -   `eODS / staking economics`
        
-   这一条主线和 [MEV](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/MEV)、[PBS](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/PBS)、[Preconfirmations](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/Preconfirmations)、[eODS](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/eODS) 连成一组。
    

### 4\. The Verge

-   核心问题：怎样让验证更轻、证明更短、节点更容易同步。
...
research-roadmap-overview-full-roadmap2024.webp
...
-   我更建议这样看：
    ...
    -   纵向看：同一个机制会同时影响多个问题，比如 `ePBS` 既属于 MEV 轨道，也会影响扩容与抗审查。
        
-   你后面写笔记时，可以把每个主题都问一句：它更偏哪条 urge，它是在补哪个短板。
...
-   它会提醒你：很多“高级研究页”其实是共享上下文的。例如：
    
    -   `PeerDAS` 不是孤立的网络优化，而是 Surge 的关键拼图。
        
    -   `FCR` 虽然是确认规则，但会影响交易 UX、CEX 充值体验、PBS builder 风险。
        
    -   `eODS` 不是纯 staking 经济学，而是 Scourge 和 SSF 方向的交叉地带。
        

## 和已有笔记的关系

-   [Scaling](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/Scaling)
...
-   [eODS](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/eODS)
    
-   [Core development](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Development/基础部分/Core development)
    

## 我的理解

-   这页真正有用的地方，是让你建立一种“研究地图感”。
...
-   后面每读一页，我都会先问自己三件事：
    ...
    -   它和我已经学过的 CL / EL / PBS / Scaling 有什么连接。
<!-- DAILY_CHECKIN_2026-04-27_END -->

# 2026-04-25
<!-- DAILY_CHECKIN_2026-04-25_START -->

# Core development（核心开发流程）

来源

[Core development](https://epf.wiki/#/wiki/dev/core-development)

## 学习定位

-   这一页是把前面所有研究页重新拉回现实工程世界。
    
-   它回答的问题是：这些协议研究最后到底是怎样被不同客户端、研究者、测试者和协调者一起推进的。
    

## 一句话总结

-   Ethereum core development 不是某一个 repo 里的串行开发，而是多个客户端团队、研究者、测试团队和协调结构围绕同一协议规范持续协作的公共工程。
    

## 不要把 core development 理解成单团队开发
...
-   包括 EIP、consensus specs、engine API 变更等。
...
-   CL / EL 都有多个客户端，且往往使用不同语言和工程风格。
    

### 3\. Coordination

-   不同团队如何讨论、推进、测试、决定优先级。
    
-   没有这一层，研究不会稳定落地。
    

## client diversity 为什么是核心价值

-   如果整个网络只依赖一个客户端，实现 bug 会变成系统性风险。
...
research-core-space-core-devs.png

-   这张图很适合拿来提醒自己：核心开发不是“几个人写协议”，而是一整套基础设施共同维护一艘大飞船。
...
-   所以前面学的是“研究问题是什么”，这页学的是“研究怎样变成协议现实”。
    

## 和已有笔记的关系

-   [Consensus Layer (CL)](10 - Web3/Ethereum 协议/EPF/Consensus Layer \(CL\)/Consensus Layer \(CL\))
    
-   [Execution Layer (EL)](10 - Web3/Ethereum 协议/EPF/Execution Layer \(EL\)/Execution Layer \(EL\))
    
-   [Coordination (PM)](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Development/进阶部分/Coordination \(PM\))
...
-   以太坊协议开发并不是“先有 spec，再有人照着写代码”这么简单。
    
-   它更像一个长期公开协作系统，规范、实现、测试和协调是一起前进的。
...
[PBS](https://epf.wiki/#/wiki/research/PBS)

## 学习定位

-   这是第 3 周里最像“母题”的页面之一。
    
-   它不是一个单一方案，而是一整个研究簇的入口：`MEV -> mev-boost -> ePBS -> PTC / TBHL / PEPC / ET`。
    

## 一句话总结

-   PBS 的核心思想是把“提议区块的人”和“构建区块的人”拆开，让 proposer 不必亲自参与高强度、高专业化的 block building 竞争。
    

## 先抓核心矛盾

-   如果 validator 既要负责安全共识，又要负责构建最赚钱的区块，那么：
    
    -   专业化 builder 会在排序和 MEV 提取上更强；
        
    -   普通 validator 会被逼着加入越来越复杂的 MEV 基础设施；
        
    -   网络容易走向专业化、中心化和第三方依赖。
        
-   PBS 的目标就是承认“构块专业化”这件事已经发生，然后尝试把它纳入更可控的结构里。
    

## 在 CL 视角下，PBS 到底拆开了什么

-   slot 到来时，正常 proposer 负责把区块发布到链上。
    
-   但“区块内容怎么组成、交易按什么顺序排列、如何最大化区块价值”这部分工作，可以交给 builder。
    
-   proposer 最终做的是“选择哪一个 builder 提供的区块”。
...
-   把机会打包成 bundle，向 builder 出价。
    

### Builder

-   汇总 mempool、private orderflow、searcher bundle。
    
-   构造最赚钱的 block body。
    
-   在 PBS 结构下，它是排序权的专业经营者。
    

### Relay

-   在 today 的 mev-boost 体系里承担中介。
    
-   负责验证 builder 提交的区块、向 proposer 展示 header / bid、在签名后交付完整区块。
...
-   不一定知道候选区块内部全部内容细节，只会倾向选择最优报价。
    

## 为什么 PBS 重要

### 1\. 降低 validator 的能力门槛

-   如果每个 validator 都要自己参与复杂 MEV 竞争，小型参与者会明显吃亏。
    
-   PBS 让 proposer 可以只负责选择，而不一定自己构造最优 payload。
    

### 2\. 承认专业化分工

-   block building 已经是一个专业市场。
    
-   PBS 不是否认这件事，而是试图在协议边界内合理放置它。
    

### 3\. 让后续扩容更现实

-   在 full danksharding 等场景里，构块和传播负担会更重。
    
-   拆分 proposer / builder 有助于把更重的工作集中到更适合做它的人手里。
    

## 当前现实：mev-boost

-   今天的 PBS 主要是 `out-of-protocol PBS`，代表性实现是 `mev-boost`。
...
research-pbs-mev-boost-architecture.png

-   这张图最适合用来理解角色分层：
    
    -   searcher 不直接对 proposer 说话；
        
    -   builder 负责把机会组合成区块；
        
    -   relay 负责转发与验证；
        
    -   proposer 只拿“报价最好”的候选。
        
-   它也顺手暴露了问题：一旦 relay 变成关键基础设施，协议外中介就会变得过重。
    

## 图示理解 2：MEV-Boost / relay / builder 工作流

research-pbs-mev-boost-flow.png

-   这张图的重要点不是具体数值，而是趋势：
    
    -   builder 负责构建 payload，relay 负责转发与校验，proposer 负责选择和签名，这说明当代 PBS 已经形成了一条独立的外部生产链。
        
    -   一旦这种工作流在现实中跑得越来越重，协议就会被迫思考：哪些环节应该继续留在协议外，哪些环节需要内生化进 ePBS。
        

## 为什么大家会转向 ePBS

-   核心动机是把现在依赖 relay 的市场结构，尽量转成协议内更可验证、更少信任、更少单点故障的设计。
...
-   目标：把 proposer-builder separation 部分内生化到协议里。
    
-   关注点：
    
    -   builder 揭示内容时如何防止 proposer 作恶；
        
    -   如何减少 relay 依赖；
        
    -   如何兼顾 timeliness、审查阻力、可实现性。
        

### TBHL（Two-Block HeadLock）

-   用更明确的两块 / head-lock 机制去保护 builder，避免 proposer 看到内容后反悔或盗取价值。
    
-   重点是防 proposer equivocation 与 builder 保护。
    

### PTC（Payload Timeliness Committee）

-   通过额外委员会判断 payload 是否按时到达。
    
-   关注的是 ePBS 里“有效负载按时性”的可验证性。
    

### PEPC（Protocol-Enforced Proposer Commitments）

-   不把 proposer 的承诺限制为“整块外包”这一种形态。
    
-   更一般地说，proposer 可以在协议里注册某些必须兑现的承诺，协议负责 enforce。
    
-   所以它比狭义 PBS 更开放，更像“可信承诺框架”。
    

### ET（Execution Tickets）

-   继续把 validator / proposer / execution rights 的关系拆得更细。
    
-   它和 PBS 系列一起，构成 endgame block production pipeline 的延伸研究。
...
-   relay 与 builder 很容易形成头部集中。
    

### 2\. 审查风险

-   如果关键基础设施受监管压力，交易可能被系统性延迟或过滤。
...
-   更多中间环节意味着更多故障模式、更多可利用的边界。
    

## 和已有笔记的关系

-   [MEV](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/MEV)
    
-   [Preconfirmations](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/Preconfirmations)
    
-   [FCR](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/FCR)
    
-   [Scaling](10 - Web3/Ethereum 协议/EPF/Research 与 Development/Research/Scaling)
    
-   [Block Building](10 - Web3/Ethereum 协议/EPF/Execution Layer \(EL\)/进阶部分/Block Building)
    

## 我的理解

-   PBS 不是一个“功能点”，而是一种权力重分配。
    
-   你可以把它理解成：Ethereum 在承认区块构建已经专业化之后，试图决定这种专业化应该如何被限制、约束和协议化。

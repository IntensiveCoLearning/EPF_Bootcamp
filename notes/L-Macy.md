...
-   核心开发者大约 200+ 人，分布在约 20 个团队中（客户端开发、研究、测试、其他贡献者）。
    
-   开发高度分散，但必须通过共识与高效协作来推动协议变更。
    

主要协调平台：ethereum/pm 仓库

-   GitHub：[https://github.com/ethereum/pm](https://github.com/ethereum/pm)
    
-   核心用途：
    
    -   组织和记录 AllCoreDevs 会议（执行层 + 共识层）。
        
    -   存档会议议程、笔记、录音和行动项。
        
-   次要用途：
    
    -   分组讨论室（Breakout Rooms），讨论具体专题（EOF、账户抽象、EIP 等）。
...
-   AllCoreDevs 会议通常交替进行：
    
    -   一周执行层（ACDE）
        
    -   一周共识层（ACDC）
        
-   议程在 pm 仓库的 Issues 中公开。
    
-   会议直播在 Ethereum Protocol YouTube 频道，录音会存档。
    
-   Google 日历：订阅所有协议相关会议（链接可在 pm 仓库找到）。
    

其他重要资源

-   Protocol Support 团队：[https://ps.ethereum.foundation](https://ps.ethereum.foundation) （协调网络升级、帮助新贡献者）
    
-   EIPs 仓库：[https://eips.ethereum.org](https://eips.ethereum.org) （协议变更正式提案）
    
-   Ethereum Magicians：[https://ethereum-magicians.org](https://ethereum-magicians.org) （社区深度讨论）
    
-   客户端团队：Geth、Nethermind、Besu、Prysm、Lighthouse 等（通过 pm 仓库统一协调）
    

笔记心得

-   以太坊的协调依赖开源文化 + 公开透明会议，虽然分散但可控。
    
-   核心是自愿参与 + 粗糙共识（Rough Consensus），没有单一中心控制。
    
-   适合长期跟踪：每周看 pm 仓库 Issues + 听 AllCoreDevs 会议，就能掌握协议发展脉络。
<!-- DAILY_CHECKIN_2026-04-27_END -->

# 2026-04-26
<!-- DAILY_CHECKIN_2026-04-26_START -->

1\. Remix 是什么？为什么从 Remix 开始？Remix IDE 是以太坊官方推荐的浏览器/桌面端 Solidity 开发工具。  
优点：
...
-   支持插件（LearnEth 教程、RemixAI 智能助手等）
    
-   Desktop 版额外支持本地 Git、终端、离线开发
    

两种版本对比（你用的是 Desktop）：

-   在线版：[https://remix.ethereum.org（最稳定，推荐新手先用）](https://remix.ethereum.org（最稳定，推荐新手先用）)
    
-   Desktop 版：本地文件系统 + Native Terminal（你当前版本 1.1.6）
    

[remix-ide.readthedocs.io](http://remix-ide.readthedocs.io)

Remix 界面整体布局（带标注）：

-   左侧：图标面板（File Explorer、Compiler 等）
...
2\. 打开 Remix Desktop 并创建第一个工作区

1.  启动 Remix Desktop（你已经打开）
    
2.  左侧点击 File Explorers（文件夹图标）
    
3.  上方点击 + Create → New Workspace
    
4.  命名例如 MyFirstProject，点击 Create
    

提示：Desktop 版会自动把项目保存在本地文件夹，你可以直接用系统资源管理器打开。

[remix-ide.readthedocs.io](http://remix-ide.readthedocs.io)

* * *

3\. 编写第一个智能合约（HelloWorld.sol）

1.  在 File Explorers 中右键 workspace → New File
    
2.  文件名输入 HelloWorld.sol
    
3.  复制以下完整代码（已包含 constructor + 事件 + 权限控制）：
...
contract HelloWorld {
    ...
    // 构造函数：部署时自动执行
    constructor() {
        greeting = "欢迎来到 Remix！Hello, World!";
    ...
    function setGreeting(string memory _newGreeting) public {
        require(msg.sender == owner, "只有合约拥有者才能修改！");
...
4\. 编译合约（Solidity Compiler 面板）

1.  切换到左侧 Solidity Compiler（锤子图标）
    
2.  Compiler 下拉选择 0.8.28（推荐稳定版）
    
3.  勾选 Auto compile（自动编译）
    
4.  点击 Compile HelloWorld.sol
    

成功标志：出现绿色对勾 + ABI + BytecodeDesktop 版常见问题解决：

-   如果显示 “Downloading compiler...” 卡住 → 点击面板右上角刷新
    
-   版本列表中选择已下载的本地版本（绿色勾）
    

[ethereumremix.substack.com](http://ethereumremix.substack.com)

编译面板截图（版本选择 + 输出日志）

* * *

5\. 部署与交互测试（Deploy & Run Transactions）

1.  切换到 Deploy & Run Transactions（
    
2.  Environment 选择 Remix VM (Cancun)（推荐新手用本地模拟链）
    
3.  点击 Deploy（蓝色按钮）
    
4.  部署成功后，在 Deployed Contracts 区域展开 HelloWorld
    

立即测试：

-   点击 sayHello → 返回 “欢迎来到 Remix！Hello, World!”
    
-   输入新问候语（如 “你好，区块链世界！”）→ 点击 setGreeting
...
-   切换账号（Account 下拉选择 Account 2）→ 再调用 setGreeting 会报错（权限控制生效）
...
[docs.polkadot.com](http://docs.polkadot.com)

带标注的部署流程示例

* * *

6\. 必学插件：LearnEth（边玩边学）

1.  左侧 Plugin Manager（插件图标）
    
2.  搜索 LearnEth → Activate
    
3.  左侧出现教程图标 → 选择 Solidity Basics
    
4.  跟随教程一步步完成小测验（包含 HelloWorld、变量、函数等）
...
7\. Remix Desktop 专属高级功能

-   Native Terminal：菜单栏 Terminal → New Terminal（可直接跑 anvil 测试链）
    
-   Git 集成：右键 workspace → Initialize as git repo
    
-   RemixAI 助手（右侧面板）：直接问 “帮我生成一个 ERC20 代币合约”
    
-   本地文件同步：所有文件直接保存在 D:\\Remix-Desktop... 文件夹
...
-   Debugger：部署后点击函数旁边的 “Debug” 按钮 → 单步执行
    
-   Unit Testing：新建 test/HelloWorld\_test.sol，用 Solidity Unit Testing 插件运行
    
-   Transaction Recorder：自动记录所有交易历史，可导出
...
| 常见问题 | 解决办法 |
| --- | --- |
| 编译报错 “ParserError” | 检查 pragma 版本是否匹配 |
| 部署后函数无响应 | 检查函数是否标记 public / view / payable |
| gas 不足 | Deploy 时 Gas limit 改成 3000000 |
| 想用中文字符串 | Solidity 原生支持 UTF-8，直接写中文 |
| 想保存项目 | Desktop 版自动保存；在线版记得 Export Workspace |

Remix VM 是模拟环境，真实部署到测试网前一定要审计代码！

* * *

10\. 下一步学习路线（0→1 完成后的进阶）

1.  Day 2：变量类型、Mapping、Struct → 练习 SimpleStorage
    
2.  Day 3：事件 + 修饰符 → 练习 TodoList
    
3.  Day 4：ERC20 代币完整教程（我可以立刻给你）
    
4.  Day 5：用 ethers.js 写前端页面调用合约
    
5.  Day 6：部署到 Sepolia 测试网（需要 MetaMask + 测试 ETH）
...
1\. 当前以太坊区块生产流程 vs PBS传统方式（合并前/无 PBS）：

-   验证者（Validator）同时负责发现交易 → 打包交易 → 创建区块 → 广播区块。
    
-   通过 Gossip 网络收集交易，自己组装并传播。
    

PBS 机制：

-   将“区块构建”和“区块提议”两个角色明确分离。
    
-   区块构建者（Builder）：负责收集内存池交易、验证交易、优化排序（MEV 提取）、组装区块体（Block Body）。
    
-   区块提议者（Proposer）：只负责选择收益最高的区块体，加上区块头（Block Header）生成完整区块并广播。
    
-   提议者看不到区块具体内容，仅根据出价（支付给构建者的费用）选择最高收益区块。
    

2\. PBS 的重要性

-   降低参与门槛：大幅减少验证者所需的计算资源和硬件要求。
    
-   提升去中心化：吸引更多样化、更多元的验证者参与。
    
-   模块化愿景：符合以太坊“合并”后的模块化路线，让不同参与者专注于各自优势（构建者擅长 MEV，提议者专注共识安全）。
    
-   整体网络更强：外部依赖更少，专业化分工提升效率和抗审查能力。
    

3\. PBS 与共识层的关系

-   时隙（Slot）：12 秒一个基本时间单位。
    
-   纪元（Epoch）：32 个时隙（约 6.4 分钟），用于最终确认、更新验证者集等检查点。
    
-   每个时隙通过 RANDAO 随机选出一名区块提议者。
    
-   提议者负责提议区块，委员会验证者负责证明（Attest） 区块有效性。
    
-   PBS 在共识层之上运行，通过分离职责优化交易打包流程，同时不破坏共识安全。
    

4\. 角色详细分工区块构建者（Builder）的职责：

-   从交易内存池（Mempool）收集交易。
    
-   验证交易（Gas Limit、Nonce 等）。
...
-   MEV-Boost 是 PBS 的主流实现，已被大量验证者采用。
    
-   通过中继器（Relay） 作为构建者和提议者之间的信任桥梁。
    
-   后续还会涉及面临的挑战（如中心化风险、审查、MEV 提取公平性）、安全问题及解决方案。
    

  
PBS 把“谁来提议区块”和“区块怎么高效构建”分离，让验证者更轻量、去中心化程度更高，同时把 MEV 专业化处理，是以太坊迈向模块化、更高可扩展性和包容性的关键一步。
...
核心目标为了构建以太坊联合链（Ethereum Union / 互操作性强的基于以太坊的 Rollup 生态），实现不同 Rollup 和区块链的无缝协作。用户在不同链/状态间切换时，无需面对复杂壁垒（如桥接摩擦、不同排序机制）。  
核心解决方案：引入基于以太坊 L1 的排序机制（Based Sequencing），让 L1 直接驱动排序，实现共享排序与高互操作性。

[ethresear.ch](http://ethresear.ch)

Based Sequencing（基于 L1 的排序）

-   利用以太坊 L1 信标链（Beacon Chain） 来管理交易排序。
    
-   通过信标链的前瞻期（lookahead），提前知道即将上任的区块提议者（proposers）。
    
-   邀请这些提议者自愿参与排序服务：提交抵押品（collateral，通常通过再质押/opt-in slashing 机制）。
    
-   排序由 L1 提议者（结合 searchers/builders）permissionlessly 执行，继承 L1 的去中心化、抗审查性和活性。
    
    [ethresear.ch](http://ethresear.ch)
    

优势：无需独立排序器，经济对齐 L1，最大化简洁与安全性，支持跨 Rollup 原子性组合。预确认机制（Pre-confirmations / Preconfs）

-   大幅降低交易延迟：提供毫秒级（~100ms 量级） 的交易确认体验，接近中心化排序器的 UX，但保持去中心化。
    
-   即将上任的 L1 提议者（preconfer）向用户发出签名预确认承诺（signed preconf promises）。
    
-   用户支付预确认小费（preconf tips） 作为激励。
    
-   执行保证：承诺交易将被包含并执行，提供即时反馈（尤其是 DeFi、跨链场景）。
...
-   自愿加入：提议者主动 opt-in 成为预确认服务提供者，并提交额外抵押品。
    
-   违约惩罚：若未履行承诺（liveness fault：未出块；safety fault：违反承诺排序/执行），抵押品将被罚没（slashed）。
    
-   这提供经济可信承诺（credible commitment），确保预确认不是空谈。
...
1\. Web3 vs 传统金融攻击本质差异传统金融：像谍战片，需要 phishing、malware、内网渗透、权限横移、资金清洗等复杂链路，花几个月到一年准备。 Web3：代码公开（或 bytecode 可见），钱直接在合约里。攻击者先看规则、再找漏洞，往往只需利用一个业务逻辑缺陷就能“合法”把钱提走。 核心比喻：Web2 是金库在楼里（需突破权限）；Web3 是金库摆在广场中央，规则写在门上（研究规则即可）。

2\. Web3 常见致命漏洞类型（不是小 bug，而是提款权限）Reentrancy（重入攻击）：最经典。withdraw 时“先转钱、再更新余额”，攻击者用 fallback/receive 在中间再次调用，导致多次提款。本质是执行顺序错误。 Self-transfer：给自己转账时，from/to 是同一个地址，读-写顺序问题导致余额膨胀。循环 self-transfer + withdraw 可掏空合约。 函数可见性错误：internal 手滑写成 public。 Token 假设错误：默认所有 Token 都是标准 ERC20。实际可能遇到 ERC777（带 callback）、通缩型 Token（转账有手续费/税）、非标准行为，导致账面与实际资产不符。 整数精度/舍入（Rounding/Precision）：Solidity 无浮点数，除法丢精度，尤其在 AMM 等复杂模型中，蚂蚁搬家式攻击可逐步掏空池子。 边界条件：空市场、空池子、特殊输入、callback 重新进入等。

3\. 安全审计到底在干什么？审计 ≠ 挑格式或语法错误，而是阻止别人顺着业务逻辑合法提钱。完整审计流程：Threat Modeling：先理解资产、流程、权限、经济模型、最大损失场景（最重要一步）。 Static Analysis：工具初步扫描。 Manual Audit：人工深挖（attacker mindset），找工具找不到的逻辑/经济漏洞。 Checklist：行业血泪史沉淀（self-transfer、reentrancy、deflationary token、access control、emergency functions、upgrade path、precision 等）。 Dynamic Testing：实际跑边界 case（self-transfer、特殊 token、callback）。 Report：finding + 风险等级 + 影响 + 修复建议。 额外重点：中心化风险：emergencyWithdraw、admin 超强权限、hidden backdoor —— 技术能跑，但治理上危险（单点信任）。 设计风险：未来可能炸的 corner case，而非当下立即可 exploit 的洞。

4\. AI 在 Web3 审计中的作用（当前真实水平）能辅助：Threat Modeling、读大代码库、生成测试、跑 checklist、解释复杂逻辑、写报告。 真正价值：Benchmark System + Knowledge Base + Feedback Loop（可验证、可回归、可持续迭代）。 AI 卷走重复脏活，高价值工作仍靠人：发现复杂漏洞、理解系统本质、判断风险、负责最终结果。 CertiK 等机构已在构建多模块 AI 框架（planner + scanner + validator），已在真实事件上取得覆盖效果。

5\. 形式化验证（Formal Verification）—— 最高阶方法普通测试 = 手电筒照黑屋；Formal Verification = 把灯全打开。 核心工具：Symbolic Execution + SMT Solver。 关键概念：Specification（规约）、Invariant（不变量）。 示例：Balance transfer：证明 from==to 时余额不变 → 找出 self-transfer 反例。 AMM：证明池子乘积 invariant 始终成立 → 发现精度丢失导致的漏洞。 特别适合数学/经济模型问题。

6\. 入行与现实建议Bug Bounty / Audit Contest：现在越来越难，简单洞被卷，重点找高经济影响的复杂问题（rounding、fork 逻辑、空池子等）。 审计 vs 开发：开发岗位覆盖面更广（node、合约、前后端、infra 等），审计更“窄而精”。 适合人群：细心、负责、持续学习、批判性思维、attacker mindset、爱问“如果呢？”。 硬实力证明：真实 bug bounty 记录、audit contest 成绩、开源工具、漏洞分析文章。 真实工作状态：很多时候像急诊科——出事时要写 PoC、链上救援、考虑
...
1\. 共识基础升级（The Merge 已完成）

-   核心：从工作量证明（PoW）切换到权益证明（PoS）
    
-   已实现：2022年9月成功完成
    
-   关键成果：能源消耗降低约99.95%，网络更环保且安全
    
-   后续优化：追求单插槽最终性（SSF），目标交易在约12秒内彻底确认；未来可能进一步降低质押门槛
    
-   为什么重要：这是以太坊最重大的转型，为后续所有升级奠定节能且安全的共识基础
    

2\. 可扩展性与性能提升（The Surge 进行中）

-   核心：通过Rollups和数据分片技术，让以太坊更快、更便宜，目标实现超高吞吐量
    
-   已完成/进行中：
    
    -   EIP-4844（Proto-Danksharding）引入Blobs，大幅降低Layer 2（如Arbitrum、Optimism）费用
        
    -   近期升级（如Pectra、Fusaka）继续增强数据可用性
        
    -   2026年Glamsterdam升级：引入并行执行、区块级访问列表、ePBS（协议内置提议者-构建者分离），进一步提升L1吞吐量和效率
        
-   未来目标：完全Danksharding，提供海量数据空间
    
-   为什么重要：解决以太坊长期拥堵和高Gas费问题，让大规模应用成为现实
    

3\. 去中心化与抗审查保护（The Scourge）

-   核心：应对MEV（最大可提取价值）风险，防止少数大型节点或机构审查、操纵交易顺序
    
-   主要方案：
    
    -   MEV-Burn：把额外收益分配给所有ETH持有者
        
    -   Inclusion Lists（包含列表）/FOCIL：强制包含某些交易，增强抗审查能力（计划在2026年Hegota升级中进一步硬编码）
        
-   为什么重要：维护网络的中立性和公平性，避免中心化力量控制交易
    

4\. 验证门槛降低（The Verge）

-   核心：让任何人都更容易验证区块，大幅降低运行节点的硬件要求
    
-   关键技术：Verkle Trees（Verkle树），实现极小数据量的有效性证明，迈向“无状态”客户端
    
-   当前进展：2026年Hegota升级（预计下半年）将引入Verkle Trees，可将节点存储需求大幅降低（据称可减少90%），甚至支持轻量设备验证
    
-   为什么重要：提升参与度，让去中心化真正落地，而非只有大机构能运行节点
...
1\. 以太坊路线图演进路径（核心脉络）你提到的学习路径非常清晰，以太坊从早期到未来的核心升级逻辑如下：

-   PoW（工作量证明） → PoS（权益证明）  
    2022 年 The Merge（合并） 完成。从能源密集型的挖矿转向质押（Staking），大幅降低能耗，提升安全性与经济模型。当前以太坊已完全运行在 PoS 上，验证者约 48 万+。
    
-   Rollup 中心化扩展（The Surge 阶段）  
    以太坊不再依赖传统分片（Sharding），而是转向 Rollup-centric 路线。Layer 2 Rollups（Optimistic Rollups + ZK-Rollups）负责执行大量交易，Layer 1 只负责结算（Settlement） 和 数据可用性（Data Availability）。  
    关键里程碑：2024 年 Dencun 升级（EIP-4844 Proto-Danksharding） 引入 Blob 交易，大幅降低 L2 数据发布成本。目前 Blob 参数仍在逐步增加（2025-2026 年通过 Fusaka 等升级继续扩展）。
    
-   Verkle 树（The Verge 阶段）  
    替换传统的 Merkle-Patricia Trie（MPT） 为 Verkle Trees（向量承诺 + Merkle 树的结合）。  
    核心优势：证明（Witness）体积大幅缩小 → 实现 无状态客户端（Stateless Clients）。节点无需存储完整状态树即可验证区块，显著降低硬件门槛（未来甚至手机/手表可运行验证节点）。  
    当前进展（2026 年）：Verkle 测试网已运行多版，客户端实现仍在推进，预计在 2026 年下半年 Hegotá（或类似）升级 中有重要进展。部分路线图讨论转向二元树 + Poseidon 哈希（更 ZK/STARK 友好、量子抗性更好）。
    
-   减负 / 清理旧历史数据（The Purge 阶段）  
    通过 EIP-4444 History Expiry（历史过期）：节点不再需要永久存储超过 1 年 的旧历史区块数据（可选择性归档或通过 Portal Network 等分布式存储访问）。  
    这直接减少节点硬盘需求数百 GB，简化客户端代码（无需处理各种历史版本）。新节点可通过检查点同步（Checkpoint Sync）快速启动。  
    结合状态过期（State Expiry）：不活跃状态数据可“过期”，进一步减轻状态膨胀问题。  
    当前进展：已部分移除 Pre-Merge 数据，2026 年将继续推进。
    
-   ERC-4337 智能钱包（Account Abstraction，The Splurge 阶段）  
    无需修改协议底层，通过更高层实现账户抽象。引入 UserOperation 对象、独立 Alt-Mempool 和 Bundler（打包器）。  
    优势：
    
    -   支持智能合约钱包作为主账户（而非传统 EOA 外部拥有账户）。
        
    -   实现 Gasless（Paymaster 代付 Gas）、批量交易、一键恢复、社交恢复、无助记词钱包等。
        
    -   大幅提升用户体验（UX），让普通用户更容易进入 Web3。  
        当前已广泛采用，许多钱包和 dApp 已集成，是 Splurge（综合提升）中最重要的 UX 改进之一。
        

2\. 以太坊扩展性核心原则（今天重点）Layer 1 负责什么？ Layer 2 负责什么？

-   Layer 1（以太坊主网）：
    
    -   结算层（Settlement）：最终确认交易的有效性，提供经济安全性（约 110 亿美元质押资产）。
        
    -   数据可用性层（Data Availability）：通过 Blob（及未来 Danksharding）提供廉价、可靠的数据发布空间，保证 Rollup 数据可被任何人验证。
        
    -   不负责大量执行计算（避免成为瓶颈）。
        
-   Layer 2（Rollups）：
    
    -   执行层（Execution）：处理绝大部分交易计算（Optimistic 或 ZK 证明）。
        
    -   目标：整体生态达到 10,000 ~ 100,000+ TPS（当前 Rollup 已显著提升吞吐量）。
        
    -   未来趋势：Rollup 之间通过跨链意图框架（Intent）、ERC-7683 等实现更好互操作性，让多 Rollup 生态“感觉像一条链”。
        

整体策略：Rollup-centric + L1 持续轻度扩展（提高 Gas Limit、PeerDAS、ePBS 等），同时通过 Verkle + Purge 实现去中心化与可持续性。3. 整体路线图总结（2026 年最新视角）以太坊 2026 年协议优先级主要围绕三大轨道：

-   Scale（扩展）：继续增加 Blob 容量、提升 Gas Limit、推进 Verkle 与无状态。
    
-   Improve UX（用户体验）：ERC-4337 智能钱包、跨链意图、账户抽象深化。
    
-   Harden L1（强化 L1）：反审查、量子安全、单槽最终性（Single-Slot Finality）研究等。
...
-   L1 + 多 L2 形成统一、高效、可组合的生态。
    

-   重点掌握：Rollup 作为执行层 + L1 作为 DA+Settlement 的分工；Verkle 树如何通往无状态；History Expiry 如何“减负”。
    
-   实践方向：尝试在测试网上部署 Verkle 相关合约、集成 ERC-4337 钱包、分析不同 L2 的数据发布成本。
    
-   关注升级：2026 年 Glamsterdam（H1）和 Hegotá（H2）等 fork，追踪 ethereum.org/roadmap 和客户端开发进展。
...
-   执行层（Execution Layer）和共识层（Consensus Layer）客户端的开发与维护。
    
-   协议实现的测试、安全审计与验证。
    
-   相关工具链、同步调试工具、参考实现的建设。
    

客户端与团队生态以太坊采用多客户端架构：执行层和共识层分别由多个独立团队开发和维护。不同客户端在编程语言、性能侧重、特性上各有特点，但都严格遵循同一套公开的协议规范。除了客户端团队，还有专门的研究、测试和基础设施团队，共同维护底层技术栈。这种分散但协作的模式，确保了协议演进的稳健性。客户端多样性（Client Diversity）的意义以太坊刻意坚持客户端多样性原则：

-   提升网络稳定性和安全性（单一客户端 bug 或攻击不会导致全网崩溃）。
    
-   防止协议被单一组织或代码库主导，保持核心开发的去中心化。
    

协作方式与日常节奏尽管团队分散在不同组织、公司或非营利机构，核心开发者通过高度公开的协作机制紧密配合：

-   主要会议：All Core Devs（ACD）电话会议（执行层与共识层交替进行），以及各种专题 Breakout Room 和工作组会议。
    
-   议程与记录：托管在 ethereum/pm 仓库，任何人均可查看和跟进。
    
-   会外讨论：主要在 Ethereum R&D Discord、Eth Magicians 论坛、EthResearch 等公开渠道进行，降低参与门槛。
    

这些公开透明的流程，让协议升级和决策过程对社区高度可见。

“太空站”隐喻Hsiao-Wei Wang）在 2023 年 ETHGlobal Tokyo 的演讲《A Journey of an Ethereum Core Dev/Researcher》中，用空站（Space Station）比喻以太坊：

-   不同团队（规范作者、客户端实现者、研究人员、测试与基础设施团队）像太空站的各个模块，持续协作才能让整个“空间站”稳定运转。
    
-   强调核心开发是一项长期、系统性的工程，需要多角色长期配合，而非短期个人英雄主义。
    

总体特点核心协议开发是高度协作、公开透明、去中心化的长期系统工程。它不属于任何单一实体，而是由全球多个独立团队共同维护。这也是以太坊能长期保持韧性和创新活力的关键原因之一。
...
-   极致内卷：高薪老人 + 年轻人抢岗，流动性大，工作年限缩短。
    
-   应对：必须斜杠 + 多收入来源，构建个人品牌与核心能力。
    
-   行业转向：规范化，正规军（有交付能力者）胜出。流量 + 技术是杠杆。
...
1.  做KOL/KOC（最推荐起点）
    ...
    -   输出技术干货（如SSZ、Gasper、devp2p）+ 市场洞察。
...
2.  多跳槽 + 卡位赛道
    ...
    -   第一份工作当跳板，优化简历，争取小组长/增长岗，每次大胆谈薪。
...
3.  利用波动与副业
    ...
    -   猎头/就业辅导、离岸金融返佣（有粉丝号优势大）。
        
    -   整合资源创业（KOL+项目方，像18岁案例第一年百万）。
        

赚钱心态：流量杠杆 + 个人品牌 + 差异化服务（设计、视频、社群）。放下羞怯，目的性赚钱。3. 以太坊协议学习复习（第一周精华）两大层：

-   Execution Layer (EL)：交易执行、EVM、状态维护、mempool。客户端（Geth等）通过Engine API与CL交互。
    
-   Consensus Layer (CL)：Beacon Chain，管理验证者（32 ETH起）、最终性。质押超36M ETH，参与率97%+。
    

关键知识点：

-   RLP（EL序列化）：前缀+长度+数据，确定性高效。空字符串0x80，空列表0xc0。交易用RLP编码后通过JSON-RPC发送。
    
-   SSZ（CL序列化）：固定+偏移量，支持Merkleization（hash\_tree\_root），高效多重证明，适合轻客户端。
    
-   devp2p：P2P协议栈（发现+传输+应用层），GossipSub广播 + Req/Resp拉取。
    
-   Gasper共识：LMD GHOST（选链）+ Casper FFG（最终性）。Slot 12s，Epoch 6.4min。Attestations是主要负载。
    
-   验证者：Propose块、Attest证明。支持部分/完全提款（Capella后）。Slashing惩罚作恶。
    
-   EOF：EVM现代容器，部署时静态验证，提升安全与扩展性。
    
-   交易流程：构造 → 签名 → RLP → RPC发送 → mempool → EVM执行 → CL最终确认。

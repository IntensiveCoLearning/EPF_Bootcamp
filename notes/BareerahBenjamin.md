...
## 一、Kurtosis 与 Ethereum package 概览

-   Kurtosis 是一个为**多容器服务环境**设计的开发和测试平台，可以把一整套容器化服务（比如一条以太坊网络）打包、启动、销毁。
    
-   它在这里扮演“多容器测试环境的构建系统”，帮你一键起一套**本地以太坊 devnet**，并且可以稳定复现同样配置。
    
-   Ethereum package 是基于 Kurtosis 的一个预制包，用 Docker 或 Kubernetes 快速搭一个**可定制、可扩展、私有**的以太坊测试网，支持所有主流 EL / CL 客户端，并自动处理端口映射和服务连通性。
...
-   安装前提依赖：
    ...
    -   Docker：用于运行 Kurtosis 的容器。
        
    -   Git：用于 clone 仓库。
        
-   安装：按照 Kurtosis 官方安装指南操作（[docs.kurtosis.com/install）。](http://docs.kurtosis.com/install%EF%BC%89%E3%80%82)
    
-   安装验证：执行命令 `kurtosis version` 检查是否安装成功。
    
-   升级：参考官方的 Kurtosis 升级指南（upgrade 文档）。
    

* * *

## 三、Kurtosis Engine（引擎）

-   Kurtosis engine 是管理 devnet 基础设施的**核心服务**，负责实际拉起、管理容器和网络。
    
-   当你启动一个 devnet 时，engine 会自动启动，但也可以手动控制：
    
    -   启动：`kurtosis engine start`
        
    -   停止：`kurtosis engine stop`
        
    -   查看状态：`kurtosis engine status`
        

* * *

## 四、快速开始：一键起以太坊 devnet

-   官方建议的“最短路径”：使用 Ethereum package 的默认配置直接起一条 devnet。
...
-   启动后：
    ...
    -   终端会显示当前运行的 enclave 状态（里面有哪些服务、端口等）。
        
    -   通过 `kurtosis web` 打开 Kurtosis 的 web 界面，在浏览器里观察 devnet 的服务和状态。
        

* * *

## 五、Enclave 概念与多 devnet 管理

-   Enclave 是 Kurtosis 里的一个**隔离环境**，其中部署了一整套服务（比如一条 devnet）。
    
-   特点：
    
    -   可以创建多个 enclave，相当于同时起多条 devnet，互不干扰。
        
    -   注意本机资源，避免开太多导致性能问题。
        
-   常用命令：
    
    -   列出当前 enclaves：`kurtosis enclave ls`
        
    -   查看某个 enclave 详情：`kurtosis enclave inspect <enclave-name>`
        
        -   示例：`kurtosis enclave inspect my-testnet`
            
    -   删除某个 enclave：`kurtosis enclave rm -f <enclave-name>`
        
-   自定义 enclave 名称（推荐）：
    
    -   通过 `-enclave` 参数给新 enclave 命名，例如：
...
-   注意：这是**危险操作**，会删除所有 enclaves 和停止的 engine 容器，属于“全部清空”。
...
-   Devnet 的配置通过本地或远程的 YAML 文件定义，包括：参与者、网络参数、额外服务等。
    
-   如果某个参数没有在 YAML 中配置，就会使用默认值。
    
-   所有可配置参数的完整列表可以在 Ethereum package 仓库 README 的 configuration 部分找到。
...
-   参与者（共识 / 执行客户端类型与镜像）：
    ...
    -   可以自己在本地构建客户端 Docker 镜像，然后在配置里用 `el_image` / `cl_image` 指定。
        
-   网络参数：
    
    -   常见如 `seconds_per_slot`（slot 长度）、日志级别等。
        
-   额外服务：
    
    -   例如 dora（explorer）、prometheus\_grafana（监控面板）等。
...
## 八、定制 Ethereum package 本身

-   你可以直接 clone Ethereum package 仓库，在其基础上做开发、改逻辑、加新服务等。
    
-   运行你修改后的版本：
    
    -   在仓库目录下执行：`kurtosis run .`
        
    -   如需同时用自定义参数文件：`kurtosis run . --args-file ./custom.yaml`
...
Kurtosis 集成了多种配套工具，方便对 devnet 进行浏览、监控、断言测试等。

-   dora：轻量级 Beacon Chain 区块浏览器。
    
-   xatu：中心化的以太坊网络监控 / 数据流水线工具。
    
-   assertoor：用于编写“断言”（assertions）来验证网络行为是否符合预期的工具。
    

示例 YAML 片段（为某参与者加上工具）：
...
-   完整工具列表见 EthPandaOps 的 Kurtosis Deep Dive 文中 tooling 部分。
    
-   有些工具提供 web 界面，可以在 Kurtosis 的“user services list”中点击链接直接打开。
    
-   Dora 是最常用的 Beacon Chain 工具之一，文中也展示了其 web 界面效果。
...
-   查看服务日志：
    ...
    -   示例：`kurtosis service logs my-testnet cl-1-teku-geth -f`
        
-   将日志写入文件，方便后续分析：
    
    -   `kurtosis service logs <enclave-name> <service-name> > kurtosis.log`
        

## 2\. 通过 Docker

-   也可以用 Docker 自带的日志命令查看 Kurtosis 容器日志：
...
-   形式化方法（formal methods）是用**数学**来分析软件和硬件系统的一整套技术，起源可以追溯到柏拉图的“理念论”，到 19 世纪 Boole 的布尔逻辑和 Frege 的命题逻辑，逐步发展成今天的形式化推理体系。
    
-   在形式化方法中，\*\*形式化验证（formal verification）\*\*的核心问题是：系统是否正确满足它的规格（spec）？它通过把系统抽象成一个数学模型来证明或证伪“系统满足规格”。
    
-   “系统”指能执行其接口定义的所有功能的机制；“不变量（invariant）”是无论系统当前状态如何都必须成立的性质，例如自动售货机的不变量：没人可以免费拿走商品。
    
-   形式化验证就是检查：在所有可能运行过程中，这些关键不变量是否始终成立。
    
-   在安全评估里，它被评为 EAL 量表最高等级之一，说明对安全性有非常强的保障作用。
    

## 三种典型形式化验证类型

-   **模型检验 / 断言式检查（model checking / assertion-based checking）**：把系统建模为有限状态机，用命题逻辑验证安全性和活性（liveness）。
    
-   **时序逻辑（temporal logic）**：系统随时间变化，用时序逻辑描述“最终会发生”“一直发生”等时间相关性质。
    
-   **等价性检查（equivalence checking）**：比较两个实现是否对同一规格给出相同结果，用于验证优化后的版本是否语义等价。
...
-   Coq 是一个广泛使用的开源**证明管理系统 / 交互式定理证明器**。
    
-   它被用来形式化并验证 C 语言编译器 CompCert，后者被用于开发飞机、汽车、核电站等安全关键系统的软件。
    

## 2\. TLA+

-   TLA+ 是 Turing 奖得主 Leslie Lamport 设计的形式化规格语言，主要用于建模**并发和分布式系统**。
    
-   Amazon Web Services 使用 TLA+ 来验证其分布式系统的健壮性，确保复杂分布式协议在各种边界情况下一致安全。
    

## 3\. Alloy

-   Alloy 是一个用于软件建模的开源语言和分析器，强调轻量建模与自动分析。
    
-   例如曾用 Alloy 把某 Flash 文件系统设计与 POSIX 标准进行对照分析，检查设计是否满足标准要求。
    

## 4\. Z3

-   Z3 是 Microsoft Research 开发的**符号逻辑求解器（SMT solver）**。
    
-   它被广泛用于程序验证、编译器验证、测试、模糊测试以及各种优化问题中，也是很多现代形式化工具的底层求解引擎。
    

* * *

## 三、形式化验证示例：TLA+ 交通灯控制器

-   形式化验证流程通常从“**抽象系统**”开始：挑出与正确性相关的核心部分建模，其他次要细节全部忽略。
    
-   Dijkstra 把这种分层精化的过程比喻成一串珍珠项链：最顶层是最抽象的描述，往下每一层逐渐用更具体的概念解释上一层，最底层最终落到机器接口。
    

在页面的 TLA+ 例子中，建模了一个简化的交通控制器：

-   `Init`：初始状态没有车在等（`carsWaiting = 0`），信号灯是红灯（`greenSignal = FALSE`）。
    
-   `Arrive`：到车事件，如果 `carsWaiting < MaxCars` 就让等候车辆数加 1，否则保持不变。
    
-   `Depart`：离开事件，如果有车等待就 `carsWaiting` 减 1，否则保持不变。
    
-   `ChangeSignal`：如果有车在等，就把 `greenSignal` 变为 TRUE（绿灯）。
    
-   不变量 `carsWaiting <= MaxCars`：保证等待车辆不会超过最大容量，这是系统安全性的关键约束。
    

这个模型刻意忽略了很多现实细节（比如鸣笛、行人、传感器噪声），体现了“**高效抽象是一门艺术**”：只保留对目标不变量有影响的部分。
...
在以太坊里，形式化验证主要用来保证以下部分的安全与正确性：

-   协议层规格：执行层（EL）和共识层（CL）规范。
    
-   客户端实现：各个执行客户端 / 共识客户端是否正确实现规范。
...
-   Runtime Verification 团队：
    
    -   使用形式化方法验证了信标链规范和 Gasper 最终性机制，帮助确认 PoS 核心协议逻辑的正确性。
        
-   KEVM：
    
    -   基于 K 框架构建 EVM 的形式语义，用来验证 EVM 规格的正确实现情况。
        
-   在 Eth2 状态转换组件中，形式化验证还帮助发现了一个微妙的数组越界运行时错误，说明其在测试套件中的实用价值。
...
-   等价性检查在软件优化中非常重要：
    ...
    -   比如对智能合约做 gas 优化后，可以用形式化方法确认“优化后的版本与原版在语义上等价”，确保优化没有引入新 bug。
...
-   常见工具：
    ...
    -   Certora Prover、halmos 等形式化验证工具，可以在部署前识别合约中的安全性问题。
        
-   实例：Runtime Verification 对以太坊 2.0 的存款合约做了形式化验证，并发现一个细微 bug（后来有 GitHub issue 记录）。
    
-   Solidity 自身：
    
    -   Solidity 语言从早期就高度重视形式化验证，编译器内置 SMT-based 检查器（SMTChecker），基于 SMT 和 Horn solving 来证明或反例搜索。
        
    -   官方有多场相关演讲，介绍如何使用 SMTChecker 来为合约增加自动化安全证明。
...
-   形式化验证本身**复杂且耗时**，往往需要专门工具和技能，这也是工业界推广的阻力之一。
    
-   它只保证“模型是正确的”，不直接保证“实际代码实现是正确的”：
    
    -   从实现到模型的抽象 / 翻译步骤，如果出错，仍然可能引入漏洞。
        
-   形式化验证严重依赖“**抽象是否到位**”：
    
    -   如果把关键细节抽掉了（建模时忽略），证明出来的安全性可能不真实，反而造成安全幻觉。
        
-   因此工程实践中通常会把形式化验证与**仿真 / fuzzing**等方法结合：
    
    -   fuzzing 用大量随机输入在“真实实现”上找 bug，弥补模型抽象与现实实现之间的缺口。
        

在测试哲学上，Dijkstra 的名言很好地总结了它的位置：**测试可以证明“存在 bug”，却永远证明不了“没有 bug”；形式化验证试图在模型层面逼近“没有 bug”的保证。**
...
-   以太坊客户端（各个实现）需要持续进行多层次测试，以保证网络的**安全性**和**稳定性**。
    
-   在去中心化网络中，所有客户端必须正确通信、行为一致，并且对交易结果有相同理解，否则就会出现分叉或网络中断等严重问题。
    
-   如果在某一次状态转移中出现差异，就可能导致网络分裂，产生最终性失败（finalization failure），给用户带来很大风险。
    
-   为了达到稳定性，以太坊客户端必须通过一套标准化测试用例的严格测试。
    

## 二、测试的目标：对齐执行层和共识层规范

-   测试主要是验证客户端是否符合\*\*执行层（Execution Layer，EL）**和**共识层（Consensus Layer，CL）\*\*的规范。
    
-   通过这些测试，可以保证所有客户端以**相同方式解释和执行交易**，对链上状态有一致认识。
    
-   系统化的测试也是一种“主动找 bug 的工具”，提前发现会导致**链分叉**或**共识不一致**的问题。
...
-   Testing & Security Overview 视频（YouTube）：从宏观层面介绍以太坊测试与安全的整体图景。适合先入门再深入工具和细节。
    

## 2\. 通用测试套件（Common test suite）

-   pytest：Python 测试框架，在很多以太坊测试工具/代码中作为基础测试框架使用。
    
-   Ethereum Tests：面向所有实现的公共测试集（通用测试用例仓库），用来验证不同客户端的一致性。
    
-   Hive：以太坊端到端测试框架（test harness），专门用于做客户端之间的互操作性和整体集成测试。
    

## 四、执行层测试（Execution layer tests）

主要针对 EVM、交易执行逻辑和执行层 API 等。

-   Execution Spec Tests：专门为执行客户端设计的测试用例，基于执行层规范，对执行逻辑进行单元/集成层面的验证。
    
-   FuzzyVM：EVM 的 differential fuzzer（差分模糊测试工具），通过随机生成输入并对比不同实现的输出来发现错误。
    
-   retesteth：测试生成工具，可根据规范或场景自动生成多种测试，用于覆盖更广泛的执行路径。
    
-   EVM lab utilities（evmlab）：一组 EVM 相关实验/测试工具，用于分析和调试执行行为。
    
-   Go evmlab：受 evmlab 启发的 Go 语言版本实验室，用于在 Go 环境中做 EVM 相关实验和测试。
    
-   Collection of execution APIs：执行层相关 API 的集合仓库，用于规范和测试 Execution APIs 的实现情况。
    

## 五、共识层测试（Consensus layer test）

主要针对信标链、验证者行为和共识流程。

-   Consensus Spec Tests：基于共识层规范的测试，用来验证共识客户端逻辑（状态转换、fork choice、验证者职责等）。
    

## 六、链级测试工具（Testing chains）

这些工具用于搭建、测试和监控各种测试网或本地链。

-   Assertoor：以太坊测试网测试工具，可在测试网上自动执行各种检查和断言，验证网络在不同场景下的行为是否正常。
    
-   Ethereum package（Kurtosis）：用于部署测试网的工具包，官方推荐给初学者用来快速搭建和操作测试链环境。
    

## 七、监控与可视化（Dashboards）

-   Hive test results：Hive 测试结果的仪表盘，可视化展示不同客户端在各种测试中的表现（是否通过、错误统计等）。
...
-   记录了以太坊历史上具有代表性的**主网/测试网事故**，包括网络中断、最终性问题、DoS 漏洞等。
    
-   这些事故通过**复盘与总结**，推动协议与客户端在安全性、健壮性上的持续改进。
    
-   想要更全面的列表和分析，可以参考 EthStaker 的 Incidents 页面（外部资源）。
    

* * *

## 二、近期事故概览（Recent Incidents）

## 1\. Holesky 最终性问题（Pectra 升级后，2025-02-24）

-   发生环境：Holesky 测试网在进行 Pectra 升级后出现问题，区块无法被正常最终确定（finalized）。
    
-   根本原因：很多执行层（EL）客户端配置了**错误的存款合约地址**，导致部分客户端认为区块无效，部分客户端认为区块有效。
    
-   直接后果：
    
    -   一些客户端拒绝“无效”区块，另一些客户端继续接受这些区块。
        
    -   网络因此出现**分裂（network split）**，不同节点对链头和状态的认知不一致。
        
-   经验教训：
    
    -   升级过程中的**配置一致性**极其关键，尤其是涉及存款合约这类共识关键参数时。
        
    -   测试网能够提前暴露主网升级潜在风险，因此在主网升级前必须进行充分的测试网演练。
        

* * *

## 2\. Blob 传播问题（Dencun 升级后，2024-03-27）

-   发生环境：Dencun 升级之后，一些区块由特定 block builder 构建，区块中附带的 blobs 传播出现问题。
    
-   根本原因：这些区块中的 blob 在 p2p 网络中**传播速度过慢**，导致某个客户端实现错过了一些 slot 的区块信息。
    
-   直接后果：
    
    -   受影响客户端在部分 slot 上无法及时获取完整区块数据，从而**错失出块/验证时机**。
        
    -   这体现出 EIP-4844（blob 交易）引入后，网络层和数据传播策略的复杂性变高。
        
-   经验教训：
    
    -   升级引入新数据结构（如 blobs）时，**网络传播性能**和客户端实现的鲁棒性必须被重点测试。
        

* * *

## 3\. 主网 DoS 潜在漏洞（Merge 至 Dencun 时间段，披露于 2024-03-21）

-   时间与范围：漏洞存在的时间区间从以太坊合并（The Merge）一直到 Dencun 硬分叉之间。
    
-   漏洞核心：攻击者可以构造一个特殊区块：
    
    -   区块整体大小超过 5MB 规定上限。
        
    -   区块内包含多笔交易，每笔交易大小都小于 128KB。
        
    -   区块内所有交易的总 gas 仍然低于 3000 万 gas 上限。
        
-   影响机制：
    
    -   多数节点由于区块尺寸规则问题会拒绝该区块，而部分节点可能仍接受这样的区块。
        
    -   结果就会产生**链分叉**：少数节点形成少数派链，导致 proposer 奖励丢失、网络混乱。
        
-   安全意义：
    
    -   说明简单的“大小、gas 限制”组合可能存在**边界条件绕过**，需要从实现和规范两侧强化。
        

* * *

## 4\. 主网最终性问题（Mainnet Finality Incident，2023-05-11）

-   现象：主网上出现了**区块产出中断**，导致新交易长时间无法达到最终性（finality）状态。
    
-   持续时间：这种状态持续了两天，期间最终性显著延迟，网络进入 inactivity 惩罚模式。
    
-   结果：
    
    -   网络最终在**无需外部干预**的情况下自动恢复，这体现了以太坊当前共识设计在异常情况下的自恢复能力。
        
-   教训：
    
    -   即便协议设计了自动恢复机制，客户端实现和监控体系仍需要能够快速定位最终性问题的根因，以缩短影响时间。
        

* * *

## 5\. Reth 主网状态根不匹配（State Root Mismatch，2025-09-01）

-   涉及对象：Reth 客户端（Rust 实现的执行层客户端）。
    
-   根本原因：Reth 在处理 trie 更新（状态树更新）时存在 bug，导致本地 trie 表中存储了错误信息。
    
-   直接后果：
    
    -   随着区块推进，这些错误逐步累积，最终导致节点在某些后续区块上计算出的**状态根（state root）错误**。
        
    -   这会让受影响节点认为自己与网络其余部分的链状态不同，可能拒绝正确区块或构建错误链头。
        
-   经验教训：
    
    -   状态树（trie）更新逻辑是执行客户端的关键部分，需要**极高测试覆盖率**和差分测试（多客户端对比）。
        

* * *

## 三、历史事故（Historical Incidents）

## 1\. 少数派链分裂（Minority Split，2021-08-27）

-   事故描述：Geth 在处理 `datacopy` 操作后，将数据写回内存时出现错误实现细节。
    
-   技术原因：
    
    -   预期行为：在 `datacopy` 完成后，应把数据写入一个新的内存位置。
        
    -   实际行为：实现中错误地覆盖了原始数据位置，导致原数据被破坏（corrupted）。
        
-   后果：
    
    -   使用有 bug 版本 Geth 的节点会在某些交易上计算出错误结果，从而与其他客户端**分叉**。
        
    -   出现“少数派链”：一部分节点停留在错误链上，形成 minority split。
        
-   教训：
    
    -   单个主流客户端中的实现 bug 就足以在主网上产生**链级别分裂**，因此鼓励客户端多样性及公共测试集非常重要。
...
-   简要背景：The DAO 是早期以太坊上非常重要的一个去中心化投资基金智能合约项目。
    
-   漏洞与攻击：
    
    -   The DAO 合约中存在可被利用的漏洞（重入相关），攻击者借此多次重复提取资金。
        
    -   结果是约 360 万 ETH 被转移，造成重大资金损失。
        
-   链级影响：
    
    -   社区通过硬分叉方式“回滚”这起事件，产生了两条链：**Ethereum Classic (ETC)** 和现在的 **Ethereum (ETH)** 主链。
        
-   教训：
    
    -   智能合约层的漏洞也可以演变为**协议层治理与共识**的重大事件，测试不仅是客户端代码，还包括应用合约的安全性。
        

* * *

## 3\. 上海 DoS 攻击（Shanghai DOS Attacks，2016）

-   发生场景：Devcon2 在上海期间，以太坊网络遭遇了一系列 DoS 攻击。
    
-   技术根因：攻击者利用 **EVM 操作码定价过低** 的问题（尤其是 `EXTCODESIZE`），构造消耗执行资源但 gas 费用偏低的交易，导致节点处理区块极其缓慢。
    
-   后果：
    
    -   区块处理时间显著拉长，网络出现严重拥堵和性能下降。
        
-   协议层应对：
    
    -   随后通过 **Tangerine Whistle** 和 **Spurious Dragon** 两次硬分叉，重新调整相关操作码的 gas 定价，防止类似攻击。
        
-   教训：
    
    -   gas 定价不仅影响用户成本，也是**网络抗 DoS 能力**的重要参数，设计不当会被攻击者放大利用。
...
（这一部分是从页面列举的不同事故中抽象出来的共性要点，方便你整体记忆。）

-   配置与参数一致性：
    
    -   像存款合约地址这样的关键配置，一旦在客户端之间不一致，就会直接引发**网络分裂或最终性问题**。
        
-   协议与实现的边界条件：
    
    -   大小限制、gas 上限、trie 更新逻辑等地方的边界条件容易被忽略，需要专门的测试与形式化思考。
        
-   客户端多样性与差分测试：
    
    -   Minority split、Reth 状态根错误等案例说明，多客户端生态 + 差分测试可以更快发现单一实现的系统性 bug。
        
-   网络与数据传播层：
    
    -   在 Dencun/Blob 引入后，网络传播性能问题会直接影响客户端同步和出块，必须在升级前后重点压测。
        
-   应用层安全与协议层治理：
    
    -   DAO 攻击显示，应用合约漏洞也会演化成**全网治理与价值观**层面的事件，不能只看底层协议代码的安全。
...
-   Hive 是以太坊生态里的一个**端到端测试框架（testing harness）**。
    
-   它可以在一张本地“迷你网络”里同时拉起多个不同的以太坊客户端，然后在统一参数和场景下跑各种测试与仿真（simulation）。
    
-   核心目的：在相同条件下对比多个客户端的行为，检查是否有任何一个客户端在特定场景下出错或表现异常。
...
-   同时测试多客户端：
    ...
    -   Hive 可以在同一条测试网络中启动多个不同实现（比如 Geth、Nethermind、Besu 等，具体列表在官方 [clients.md](http://clients.md) 中）。
        
    -   所有客户端在同样的网络拓扑、相同参数、相同测试场景下运行，极适合发现**跨客户端一致性问题**。
        
-   端到端 / 集成测试：
    
    -   Hive 测的不是单个函数或模块，而是整个客户端在**真实网络协议交互**过程中的表现。
        
    -   包括 p2p 交互、共识流程、区块传播、交易执行等整体行为。
        
-   场景化与仿真（simulations）：
    
    -   Hive 提供了丰富的“模拟场景”（simulators），可以定制各种网络状态和协议场景，例如：延迟、分叉、重组、升级前后行为等。
        
    -   这些场景里通常包含很多专门设计的**边界条件 / edge cases**，用于触发潜在实现 bug。
...
-   升级与分叉前的关键工具：
    ...
    -   在主网进行硬分叉或重大升级（例如 The Merge、Dencun、Pectra 等）之前，开发者会用 Hive 在一张模拟网络中测试所有主流客户端在新规则下的行为。
        
    -   通过统一测试，可以更早发现：
        
        -   某个客户端在新规则下无法同步或出块
            
        -   客户端对新 opcode、新字段、新边界条件的处理不一致
            
        -   升级过程中产生的链分叉风险
            
-   降低测试复杂度：
    
    -   对个体开发者而言，如果手动搭一个多客户端测试网非常麻烦；Hive 把这件事自动化、标准化了。
        
    -   用一句话概括：Hive 让“同时测试所有客户端”变成一件**可重复、成本低**的事情。
        

* * *

## 四、Hive 的 Simulators（仿真器）

-   Simulators 定义了 Hive 要跑的具体“测试场景”：包括网络拓扑、节点数量、客户端组合以及要执行的测试步骤。
    
-   官方仓库提供了一系列现成的 Hive Simulators，可以直接拿来跑，或者在它们基础上做修改。
    
-   这些 Simulators 中往往包含：
    
    -   正常同步场景（full sync、snap sync 等）
        
    -   网络分叉、重组、延迟等异常网络情况
        
    -   特定协议功能测试（比如处理特定交易类型、特定 EVM 操作、升级前后行为）
...
-   安装与运行：
    ...
    -   对 Hive 感兴趣、想自己跑测试的人，可以根据官方文档安装 Hive（commandline 使用方式）。
        
    -   典型流程会包括：准备 Docker 环境、拉取客户端镜像、选择 simulator、执行测试命令等。
        
-   自己写 Hive 模拟器：
    
    -   官方有一份 _Hive Simulator Programming Guide_，详细介绍如何编写自己的 Hive simulator。
        
    -   适合以下场景：
        
        -   你想复现历史上的某次主网事故 / 边界条件
            
        -   你设计了新的协议提案（EIP / 共识变更），想在多客户端环境下提前检验其行为
            
        -   你在维护某客户端，想针对性构造场景验证 bug 修复

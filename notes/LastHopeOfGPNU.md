...
**Consensus Layer (CL)** 是 **以太坊 PoS 共识机制** 在协议层面的核心组成。它定义了如何在分布式网络中达成一致、维护链的线性历史，并处理分叉、状态更新等问题。
...
-   实际上是由两个共识机制组合而成：
    ...
    -   **LMD GHOST** → 提供 _活性（liveness）_
        
    -   **Casper FFG** → 提供 _最终性（finality）_
        
-   两者协同工作，弥补单一共识在活性 vs 安全方面的权衡。
...
-   _安全_ = 不会出现冲突决定（即不会两个分叉同时被认为有效）  
    由于通信不可靠，不可能同时完美满足两者，Gasper 通过组合设计达成可用折中。
...
-   节点基于本地视图对分支打分，选择最高分的 _head block_；
...
-   当前主流规则包括：
    ...
    2.  **Casper FFG → 以最高 justified checkpoint 选择**
        
    3.  **LMD GHOST → 按累计投票最重子树选择**
        
-   节点收到新投票/区块后可能导致 reorg（链重组）。
    

## Consensus 层组成

-   **Beacon Node**：运行共识客户端，实现链广播、区块验证等；
    
-   **Validator**：参与者节点，负责提议和打包区块；
    
-   节点需要与本地 Execution Layer (EL) 客户端联动，通过 **Engine API** 交换数据；
    
-   共识客户端还运行自己的 P2P 网络协议栈。
...
3.  把交易作为执行 payload 发给 EL
    
4.  执行并校验状态
    
5.  把结果发回 CL
    
6.  CL 添加区块并广播 attestations（投票）
...
4.  CL 将哈希和交易打包入 Beacon block
    
5.  广播区块，等待 attestations
    
6.  达到足够 attestations 后进入 finalized 状态。
    

## 状态迁移函数

整个网络共有一个 **state transition function**，定义如何加入区块后更新链状态：

> 新状态 = state\_transition(旧状态, 区块)

函数对 slot/区块/epoch 等阶段回调。它是**纯函数**，同样输入必定生成同样输出，任何不一致说明共识失败。
...
-   **客观（Objective）信息**：可以完全验证正确性，不依赖信任。  
    _例子：_ 在 PoW 时代，从创世块(genesis)开始按父哈希逐层向后验证区块，可以确认整个链历史。
    
-   **主观（Subjective）信息**：无法完全通过验证逻辑确认正确性，需要一定程度的信任。多数系统信息属于这一类。
    

## 同步机制演进背景（Merge 后）

以太坊合并 (The Merge) 引入了分离的：

1.  **共识层（Consensus Layer, CL）**：处理验证者投票、最终性投票机制。
    
2.  **执行层（Execution Layer, EL）**：执行交易与 EVM 逻辑。
    

在 CL 中最新的最终性头（finalized head）是节点同步的目标，而这个目标必须来自共识机制，而不是像 PoW 中那样单靠数值难度即可验证。

## “弱主观性（Weak Subjectivity）” 的缘由

### 1) 为什么传统从 genesis 递归验证不再安全？

在 CL 中，由于验证者可以退出网络，其投票权消失；但这些退出验证者后续仍可以 _重新参加旧的分叉_，对历史区块产生新投票，这会造成所谓的 **历史再组织（equivocation / alternate history）** 可能性。  
这意味着节点不能单纯只依靠链的历史哈希和签名来确保链是可被信任的。

## 弱主观性同步（Weak Subjectivity Sync）

为保证节点同步到正确的链，需要采用与 PoW 不同的策略：

### **核心风险**

-   **当前同步目标（sync target）不可完全客观验证**：因为可能有历史被攻击者重写的风险。换言之，仅凭链数据无法确认该目标属于**真实最终链**。
    

### 因此需要：**弱主观性检查点（Weak Subjectivity Checkpoint）**

-   **检查点必须通过可信渠道提前获得**，不依赖网络随机对等体 (peer-to-peer)。
    
-   这是一个被认为具有足够共识（finality）的区块状态，用作验证起点。
...
3.  更新目标头（target header），与执行链（EL）信息对齐。
    
4.  **乐观导入（optimistic import）新头**，即使执行层可能稍后验证其有效性。
    
5.  一旦执行层完全同步，标记 CL 所有 Slot 为已验证 —— 节点视为 **fully synced**。
    

这个过程必须依赖共识最终性与外部可信信息，而非仅靠链上哈希连接性。

## 与执行层同步的区别

| 方面 | 执行层（Execution Layer） | 共识层（Consensus Layer） |
| --- | --- | --- |
| 验证方式 | 可由本地重放 EVM 交易验证历史 | 需要验证者投票达成的最终性 |
| 信任锚点 | 创世块 + 直接父哈希链 | 弱主观性检查点（外部传入） |
| 历史否认风险 | 极低（PoW 计算难度） | 存在（可通过退出验证者改变历史态度） |
| 是否可独立验证完全链 | 是 | 否 |
...
**Consensus Layer（CL）网络堆栈负责共识客户端之间的点对点网络交互**，关键包括：

-   对等节点发现（Peer Discovery）
...
CL 网络使用模块化协议栈（libp2p + 相关协议）实现这些能力。

## 网络协议组件与功能结构

### **1）libp2p – 核心 P2P 框架**

CL 网络使用 **libp2p** 作为底层对等网络协议，它提供：

-   多种传输支持：TCP、QUIC 等
    
-   安全通道（通过 libp2p‑noise）
...
每个对等连接实际上是一个 libp2p 多层协议栈。

## libp2p 协议栈组成

| 协议层 | 主要职责 |
| --- | --- |
| Transport | TCP / QUIC 等物理/逻辑连接 |
| Security | 加密与对等身份确认（Noise） |
| Multiplexing | 多路复用单连接上的多个数据流 |
| Application | 实际消息逻辑（pubsub、gossipsub、req/resp等） |
...
-   生成和传播 **ENR（Ethereum Node Record）**
    
    -   包含节点身份、地址、端口、CL 子网信息等
        
-   其他节点可通过 discv5 查询与其建立连接。
...
-   节点通过 libp2p‑noise 建立加密安全信道
    
-   使用 _multiaddr_ 统一描述网络地址（包含 IP/端口/协议信息）
    
-   多路复用器（如 yamux）允许一条 TCP/QUIC 连接承载多个逻辑会话。
...
1.  **Gossip（基于 pubsub/gossipsub）**
    
    -   用于块数据、attestation 等广播式消息
...
这些机制覆盖大部分 P2P 数据传输需求，不同于传统链的直接单播。

## 编码与压缩

-   **SSZ**：Consensus Layer 默认编码格式
...
-   **Phase 0 Networking 规范** 是 CL 网络初始设计
    
-   之后的 fork 对 CL 网络做了具体调整
...
-   Ethereum 共识层（Proof‑of‑Stake）的核心链，负责协调验证者、处理 attestations（共识投票）与区块Finality等。
    

**Beacon API（信标 API）**

-   Consensus Layer 客户端（Beacon Node）对外暴露的 **REST/JSON API**。
    
-   主要用于读取共识层状态、查询链数据、获取验证者任务等。
    
-   Validator Clients（验证者客户端）依赖它来获取共识任务与提交签名数据。
    

> 它不同于 Ethereum 的 Execution API —— Beacon API 专注共识层（验证、状态、投票、Finality 等），而 Execution API 管理交易执行和 EVM 状态。

## 数据结构与容器

文档摘录了 Beacon Chain 关键容器类型，包括但不限于：

### BeaconState

存储整条信标链当前状态，包括：

-   slot、fork、validators 列表
    
-   balances（余额列表）
    
-   最新区块头、历史根
    
-   sync committee 当前/下一个
    
-   参与度、Finality checkpoint
    
-   最新 execution payload header 等（与执行层协作）
...
-   proposer、attestations、slashings、deposits、voluntary\_exits 等
    
-   execution\_payload（与 EVM 交互）
    
-   blob commitments（blob/KZG commitments，长期存储与数据可用性用）
...
1.  **链状态查询**
    ...
    -   当前 slot/epoch、validator 状态、balances、Finality checkpoint 等。
        
2.  **Validator Duty（验证者任务）服务**
    
    -   查询 proposer/attester 分配
        
    -   获取 block 模板
        
    -   提交 signed blocks/attestations 回网络。
...
4.  **数据分析与监控**
    ...
    -   构建 staking dashboard、Block Explorer、性能监控、SLASH 条件检查。
        

## 常见 API Endpoints

虽然该文档（笔记源）没有一一列出，但以 BlockPI 等资料为例汇总常见 Beacon API Endpoints：

| 类别 | 示例 Endpoint |
| --- | --- |
| 链信息 | /eth/v1/beacon/genesis |
| 状态查询 | /eth/v1/beacon/states/{state_id}/validators |
| Block 头/体 | /eth/v1/beacon/blocks/{block_id} |
| Committees / RANDAO | /eth/v1/beacon/states/{state_id}/committees |
| Sync 状态 | /eth/v1/node/syncing |

注意：state\_id 常见取值有 `head`、`finalized`、slot/epoch 具体 ID 等。

## 规范目的与安全建议

-   Beacon API 规范旨在 **实现各共识客户端间的一致性**（例如 Prysm, Teku, Lighthouse 等）。
    
-   默认用于本机 [localhost](http://localhost)，不应暴露对公网，否则可能遭受 DoS 等攻击。
...
-   构造一种可高效验证、可生成轻量证明的数据结构，使节点（特别是轻客户端）能仅通过少量数据验证整个状态。
    
-   在以太坊共识层，用于对 Beacon 状态、容器等复杂结构计算唯一哈希标识。
...
-   Merkleization：构建 Merkle 树并派生根哈希的过程（从序列化数据到树形结构）
    
-   Hash Tree Root：对 SSZ 对象（基本类型和容器）的 Merkle 根，用于标识对象
    
-   SSZ：Simple Serialize，规范化以太坊共识层数据的序列化格式
    

## 基本 Merkle 化流程（32 字节粒度）

1.  序列化
    
    -   将数据根据 SSZ 类型系统转为字节流，每个字段或元素按规范编码。
        
2.  拆分成 Chunk
    
    -   每 32 字节为一个叶节点，若不足 32 字节，右侧填充 0x00。
        
3.  构建二叉哈希树
    
    -   叶子按两两组合哈希，向上递归生成父节点。
        
    -   如果数量不是 2 的幂，用全 0 chunk 补齐保持平衡。
        
4.  根哈希输出
    
    -   最终形成唯一单个哈希值（Merkle 根、Hash Tree Root）。
        

## 容器与复合类型的处理规则

-   对于复合类型（container / list / vector）：
    
    -   分别计算每个字段的 Hash Tree Root，
        
    -   再对这些根进行一次 Merkle 化组合，形成整个对象的根。
        

这种“分段哈希 + 再 Merkle 化”保证局部变化必然影响最终根哈希。

## Generalized Index 与多重证明（Multiproofs）

-   Generalized Index
    
    -   唯一标识树中每个节点（包括叶子与内部节点），根为 1，按层级推导位置编号。
        
-   Multiproofs
    
    -   针对特定叶子元素构造包含必要哈希节点的最小 Merkle 证明，
...
-   输入的 Leaf 数量若非 2 的幂，需要用 zero-value chunks 填充，确保树平衡。
    
-   复合结构 (container 等) 的 Merkle 根由级联哈希子结构计算。
    
-   轻客户端可基于 Merkle 根 + 证明验证数据完整性和包含性，无需全状态数据。

...
<!-- DAILY\_CHECKIN\_2026-05-01\_START -->

今天讀了 ePBS 和 PeerDAS 兩篇。ePBS 接續昨天的 PBS，從「extra-protocol 的 MEV-Boost」推進到「寫進協議層」。PeerDAS 則是 Danksharding 的網路層實現，讓節點不用下載全部 blob 就能驗證資料可用性。兩者都是 Fusaka 和後續升級的重點。
...
昨天讀了 MEV-Boost 是目前約 90% 的 block 在用的 extra-protocol PBS 方案。問題是它依賴少數幾個受信任的 relay——這些 relay 不在協議保護範圍內，出問題只能靠社會層（social layer）處理。

ePBS 要做的事很明確：把 PBS 的機制直接嵌入 Ethereum 的共識層，讓 proposer-builder 的互動由協議本身保證，而不是靠外部軟體。

核心動機三點：

\- 去中心化：減少對中心化 relay 的依賴

\- 安全性：block construction 遵循跟其他共識活動相同的安全規則，而不是跑在鏈外

\- 效率和公平：用協議層的透明拍賣取代黑箱的 relay 仲介

反對意見

ePBS 不是沒有爭議。反對方的核心論點：

複雜度和技術風險——往協議裡加新東西就是加攻擊面。網路性能——可能影響 latency 和 block propagation。Bypassability——即使 enshrine 了，validator 和 builder 可能還是繼續用外部方案（比如改良版的 MEV-Boost），那 ePBS 就變成沒人用的空殼。Premature optimization——應該先解決更緊迫的問題（SSF、censorship resistance）。

支持方的回應：ePBS 的好處超過複雜度成本，可以設計成不犧牲網路效率，而且即使被 bypass 也能作為可靠的 fallback。長期來看 ePBS 是 MEV burn 等進階機制的前提。

Minimum Viable ePBS 的設計原則

一個最小可信的 ePBS 系統需要滿足一堆安全屬性：

\- Honest Builder Publication Safety：按時發布 block 的 builder 不會被忽視

\- Builder Reveal Safety：builder 揭示 block 內容和 bid 時，不會被 proposer 的 equivocation（同時簽兩個 block）坑

\- Unconditional Payment：builder 只要做了工作就能拿到報酬，不管 block 最終有沒有被 include

\- Proposer Safety：proposer 不會因為參與 ePBS 而損失收入

\- Permissionlessness：任何人都可以當 builder 或 proposer，不需要中央授權

\- Censorship Resistance：不能審查特定 block 或交易

\- Roadmap Compatibility：跟未來的升級（Danksharding、SSF 等）相容

同時還要保證：proposer 沒有動機在 off-protocol 賣 block（否則 ePBS 就被 bypass 了）。

設計方案

TBHL（Two-Block HeadLock）：把一個 slot 拆成兩個 phase。第一個 phase proposer commit 到某個 builder 的 bid，第二個 phase builder reveal block 內容。Headlock 機制防止 proposer equivocation——一旦 proposer commit 了，就不能反悔換另一個 builder。

PTC（Payload-Timeliness Committee）：一組 validator 組成 committee，投票決定 block 的 payload 是否及時到達。這解決了一個核心問題——在 ePBS 下，proposer 不 build block，那誰來判斷 builder 是否按時提交？PTC 把這個判斷分散給一組 validator，避免單點信任。

PEPC（Protocol-Enforced Proposer Commitments）：比 ePBS 更通用的框架。proposer 可以註冊任意的 commitment（用 EVM 邏輯表達），builder 只要滿足這些 commitment 就可以提供 block。違反 commitment 直接讓 block 無效——這比 MEV-Boost 的 optimistic model 強得多。PEPC 是 ePBS 的概念擴展和泛化。

Inclusion Lists（EIP-7547）也是 ePBS 的互補機制——proposer 保留「強制 include 某些交易」的權力，確保 builder 不能隨意審查。

開放問題

Mike Neuder 提出了幾個 ePBS 的核心未解問題：

Bypassability 的意涵是什麼？如果大部分人繼續用外部方案，ePBS 到底有多大用？但即使被 bypass，ePBS 也可以作為可靠的 fallback，降低對外部 relay 的依賴。

在 L2 主導的未來，L1 ePBS 還重要嗎？隨著更多活動遷移到 L2，L1 的 MEV 可能會減少。但 ePBS 建立的基礎設施和原則可能對跨層的 MEV 管理仍然重要。

ePBS 的優先級應該排在哪？跟 SSF、censorship resistance 這些升級相比，ePBS 的緊迫性還在爭論中。
...
PeerDAS 是 EIP-7594 定義的網路協議，屬於 Surge 的核心部分。它的定位是從 EIP-4844 走向 Full Danksharding 的中間步驟。

演進路線分三個 stage：

\- Stage 0（EIP-4844，已上線）：引入 blob，但節點還是要下載所有 blob data，沒有 sampling

\- Stage 1（1D PeerDAS）：水平擴展 blob 成 column，引入 column subnet，啟用 peer sampling

\- Stage 2（2D PeerDAS / Full Danksharding）：加上垂直擴展，cell-based sampling，支援分散式重建，達到最大擴展性

PeerDAS 要解決的問題很直白：blob 數量增加後，讓每個節點都下載全部 blob data 不現實。需要一個方法讓節點只抽樣驗證就能確認資料可用。

核心機制

Data Partitioning and Custody：blob 被切成更小的單位叫 column。網路用確定性函數（基於 node ID）把節點分配到 custody group，每個 group 負責保管特定的 column set。節點有最低保管門檻；願意存更多 column 的節點成為 super-node，持有所有 column，增加系統冗餘。

Erasure Coding：用 Reed-Solomon erasure code 編碼 blob——把資料分成多個 column 並加入 parity column。這意味著即使部分 column 丟失，只要收集到超過 50% 的 column 就能完整重建原始資料。

資料分發：編碼後的 column 透過 GossipSub 傳播。Proposer 先把 column 分發給一部分節點，這些節點再轉發給各自的 peer。每個節點訂閱跟自己 custody group 對應的 subnet。如果 gossip 沒收到某個 column，可以用 request/response 協議主動去其他節點拉。

Data Availability Sampling：節點從 peer 那裡隨機請求 column 的樣本。透過概率方法——如果抽到足夠多的樣本都是有效的，就可以高信心推斷完整資料是可用的。如果發現缺失的 column，可以主動發請求去復原。

KZG Commitment 驗證：用 KZG commitment 確保抽樣到的 column 跟原始資料一致。節點不需要下載完整 data 就能用密碼學方式驗證 column 的正確性。這防止了篡改和資料損壞。

資料重建和冗餘管理：節點持續 sample 自己負責的 column。當累積超過 50% 的 column 時，用 Reed-Solomon 解碼重建完整資料，然後把恢復的 column 重新分發到網路上。這個機制讓系統在部分 subnet 故障或資料暫時不可用時仍能自癒。

Fork Choice 整合：validator 的 fork choice rule 被修改來強制執行 DA 檢查。新提議的 block 透過 gossip subnet 收到的 column 來判斷 DA；舊 block 則透過 peer sampling 結果來驗證。這確保 validator 只會為有可驗證資料的 block 投票。

\---

小結

ePBS 和 PeerDAS 分別對應路線圖的 Scourge 和 Surge，但有深度交集：Full Danksharding 需要 PeerDAS 來實現 DA sampling，同時也需要 ePBS/PBS 來確保只有 builder 需要處理完整的大 block，普通 validator 只做 sampling 就好。

換個角度看：PeerDAS 解決的是「怎麼讓大量 blob data 在網路裡高效傳播和驗證」，ePBS 解決的是「在 block 變大之後，怎麼讓出塊過程還能保持去中心化」。兩者是 scaling 的一體兩面。

<!-- DAILY\_CHECKIN\_2026-05-01\_END -->
...
<!-- DAILY\_CHECKIN\_2026-04-29\_START -->

今天讀了 Scaling 和 MEV/PBS 兩篇，前者是以太坊擴展性的整體框架和 rollup-centric roadmap 的邏輯，後者是 MEV 的演進以及 PBS 的現狀和未來方向。
...
任何區塊鏈系統的擴展性都受兩個基本參數制約：block latency（出塊間隔，以太坊固定 12 秒）和 block size（受 gas limit 限制，目前上限 30M gas，target 15M）。這兩個參數直接決定 TPS。

想提高 TPS 最直接的方法是加大 block size 或縮短出塊時間，但這意味著節點需要更高的硬體規格和頻寬來驗證和傳播區塊，跑節點的門檻升高，願意跑的人變少，網路就中心化了。

這就是 Blockchain Trilemma：安全性、去中心化、擴展性，三者很難同時最大化。天真地拉高 TPS 必然犧牲去中心化或安全性。

!\[Blockchain Trilemma\]([https://epf.wiki/wiki/research/img/scaling/blockchain-trilemma.png](https://epf.wiki/wiki/research/img/scaling/blockchain-trilemma.png))

另一個限制因素是 data availability——狀態資料必須可存取，才能讓新節點在無信任假設下重建最新狀態。但狀態持續膨脹會導致儲存需求和同步時間越來越誇張。以太坊的做法是讓讀寫 state 的 gas cost 特別貴，用經濟手段抑制狀態增長。

模組化區塊鏈

面對三難困境，現代區塊鏈設計走向了「分而治之」——把系統拆成功能模組，各自獨立優化：

\- Consensus：節點之間就出塊和交易排序達成一致

\- Execution：執行交易、推進狀態

\- Data Availability：確保資料在需要的時候可被查詢

\- Settlement：保證交易的最終性和不可逆性

這種模組化設計降低了系統複雜度，各個模組可以獨立演進和升級。

L1 vs L2 擴展

L1 擴展：在協議本身層面提高 TPS。包括提高 gas limit、降低 opcode 的 gas cost（讓同一個 block 能塞更多交易）、改進共識機制等。但 gas limit 提高會同時拉高節點硬體門檻。

!\[L1 Scaling\]([https://epf.wiki/wiki/research/img/scaling/layer-1-scaling.png](https://epf.wiki/wiki/research/img/scaling/layer-1-scaling.png))

L2 擴展（Rollup-centric roadmap）：以太坊社群主流的擴展路線。以太坊 L1 做 settlement 和 data availability 層，把執行任務委派給上層的 rollup。Rollup 把多筆 L2 交易打包成一筆 L1 交易，執行在鏈下完成，資料和狀態承諾存到 L1。

!\[L2 Scaling\]([https://epf.wiki/wiki/research/img/scaling/layer-2-scaling.png](https://epf.wiki/wiki/research/img/scaling/layer-2-scaling.png))

Rollup 本質上是交易執行的壓縮——把計算負擔從 L1 搬走，但仍然繼承 L1 的安全性，因為資料和 state transition commitment 都存在 L1。

歷史上 rollup 把資料塞在 calldata 裡，但這導致歷史儲存暴增。EIP-4844 引入 blob 交易，提供獨立的、臨時的資料空間和獨立的 gas market，大幅降低 rollup 的 DA 成本。

兩種 Rollup

Optimistic Rollup：預設所有交易都是有效的（樂觀假設），提交到 L1 之後有一個 challenge period，任何人都可以提交 fraud proof 來挑戰。挑戰成功則 challenger 獲得獎勵、operator 被 slash。代表項目：Arbitrum（BoLD 爭議解決協議）、Optimism（Cannon fault prover）。

ZK Rollup：用零知識證明來數學保證交易批次的正確性。Proof 提交到 L1 驗證，通過就確認所有交易有效。不需要 challenge period，finality 更快。兩種主要的 proof 系統：zk-SNARK（目前更廣泛採用）和 zk-STARK（可擴展性更好、不需要 trusted setup）。

ZK Rollup 的「ZK」其實有點誤導——主要好處是 succinctness（proof 遠小於原始交易資料），而不是 zero-knowledge（隱私）。
...
Maximal Extractable Value——透過策略性地排序、包含或排除區塊內的交易，能從出塊中榨取出超過標準區塊獎勵和 gas fee 的價值。

MEV 的演進：PoW 時代叫 Miner Extractable Value，礦工有權決定交易順序。早期的 MEV 提取方式是在 public mempool 裡互相搶——出更高的 gas price 讓自己的交易排前面，這段混亂時期叫 Priority Gas Auction（PGA），Flashboys 2.0 論文記錄了這個時代。

Flashbots 作為開放的 R&D 組織出現，目標是讓 MEV 的工具和知識民主化。Merge 之後 miner 消失，validator 接管了出塊角色，MEV 的提取機制不變但參與者換了。

常見的 MEV 策略：frontrunning（搶跑）、sandwich attack（三明治攻擊）、backrunning（跟跑）、arbitrage（套利）。

PBS（Proposer-Builder Separation）

目前以太坊的 validator 同時負責選交易、組裝區塊和 propose 區塊。PBS 把這些職責拆開：

\- Builder：專門從 mempool 撈交易、排序、組裝成 block body，目標是最大化區塊的價值

\- Proposer：從多個 builder 提交的 block 中選一個最賺錢的 propose 出去，不需要知道 block 的具體內容

PBS 為什麼重要：降低 validator 的計算門檻（block building 的優化交給專業 builder），讓更多人能當 validator，提高去中心化程度。

Supply chain：Searcher → Builder → Relay → Proposer

1\. Searcher：掃描 mempool 找 MEV 機會，構造 bundle（有序的交易列表 + bid），提交給 builder

2\. Builder：從多個 searcher 收集 bundle，組裝成完整的 block，提交給 relay

3\. Relay：從多個 builder 收集 block，驗證有效性，把最高出價的 block 轉給 proposer

4\. Proposer（Validator）：從 relay 收到 block，選最賺的一個，簽名 propose 到網路

!\[MEV-Boost Architecture\]([https://epf.wiki/images/mev-boost-architecture.png](https://epf.wiki/images/mev-boost-architecture.png))

MEV-Boost

目前 PBS 是 extra-protocol 的，透過 Flashbots 的 MEV-Boost 實現。約 90% 的 Ethereum block 是透過 MEV-Boost 構建的。問題是依賴一小組受信任的 relay，引入了中心化風險和審查風險。

安全隱患：relay 出問題可能導致 missed block（不會危害以太坊完整性，但影響 validator 收入和用戶體驗）。客戶端可以在外部 builder 失敗時回退到本地構建。

審查風險：少數幾個 dominant builder 可能被強迫或勾結排除特定交易，削弱以太坊的 permissionless 性質。目前就算大多數 builder 選擇審查，也只能延遲交易的 inclusion，不能完全阻止。

研究方向

ePBS（Enshrined PBS）：把 PBS 寫進協議層。好處是降低對第三方 relay 的依賴、減少中心化風險、增強安全性。透過 Two-Block HeadLock（TBHL）等機制實現。

PEPC（Protocol-Enforced Proposer Commitments）：比 ePBS 更通用——讓 proposer 可以註冊任意的 commitment（用 EVM 表達），外部 builder 只要滿足 commitment 就可以提供服務。Block 只有在滿足所有 commitment 的情況下才算有效。這比 MEV-Boost 的 optimistic model 更強——違反 commitment 直接讓 block 變非法。

Inclusion Lists（EIP-7547）：讓 proposer 可以指定一組「必須被 include」的交易。Builder 如果想讓自己的 block 被 propose，就必須包含這些交易。解決的問題：在 PBS 下 proposer 把出塊權外包給 builder，但仍然保留強制 include 特定交易的權力，防止審查。

\---

小結

Scaling 和 MEV/PBS 是以太坊路線圖裡 Surge 和 Scourge 的核心。Scaling 透過 rollup-centric roadmap 解決 TPS 問題——L1 專注做 DA 和 settlement，L2 做 execution。MEV/PBS 則解決出塊過程中的權力集中問題——把 building 和 proposing 分開，用市場機制分配 MEV，同時透過 inclusion list 等機制保持抗審查能力。

兩者的交集在 data availability：rollup 需要 DA 來保證安全（fraud proof / validity proof 都需要原始資料），而 DA 的實現（Danksharding、PeerDAS）又需要 PBS 來確保只有 builder 需要處理完整的大 block，降低 validator 的負擔。

<!-- DAILY\_CHECKIN\_2026-04-29\_END -->
...
<!-- DAILY\_CHECKIN\_2026-04-27\_START -->

今天讀了 Ethereum Protocol Roadmap 這篇，把整個協議的未來發展方向整理了一遍。這篇不是在講某個具體的技術機制，而是站在更高的視角看以太坊想往哪走、為什麼要往那走。

\---

Infinite Garden 哲學

以太坊不是一個有終點線的零和遊戲，而是一個要持續玩下去的無限遊戲。協議需要在安全性、擴展性和可持續性上不斷升級，直到最終 ossification（骨化，即核心協議不再大幅改動）。在那之前，持續演進是常態。

沒有所謂的「官方路線圖」，也沒有任何人或組織能單方面決定協議的方向。核心 R&D 完全公開，任何人都可以參與討論和提案。所謂的「路線圖」其實是社群、開發者和研究者在持續討論中形成的大致共識。

Vitalik 在 2023 年 12 月畫了一張知名的路線圖總覽，把各個研究方向歸類成六個「urge」。
...
\- Beacon Chain 上線（2020.12）：作為獨立鏈啟動，引導 validator 入場

\- The Merge（2022.9）：EL 和 CL 合併，PoW 正式退役，能耗降低 99.988%

\- 啟用提款（Shapella）：validator 可以提取質押的 ETH

待研究的方向：

Single Slot Finality（SSF）：目前 finality 需要約 12.8 分鐘（2 個 epoch），SSF 的目標是在一個 slot（12 秒）內就完成 finality。好處是交易確認時間從分鐘級降到秒級、L2 bridging 更快、多區塊 MEV reorg 被消除、共識機制複雜度降低。挑戰是怎麼在一個 slot 內處理幾十萬 validator 的簽名。

Single Secret Leader Election（SSLE / EIP-7441）：目前 proposer 的身份在出塊前就公開了，容易被 DoS 攻擊。SSLE 讓只有被選中的 validator 自己知道自己被選中，直到它 publish block 的那一刻。

Enable more Validators：怎麼在 validator 數量持續增長的情況下高效協調共識。EIP-7514（限制每個 epoch 的 churn limit）和 EIP-7251（提高 MAX\_EFFECTIVE\_BALANCE 到 2048 ETH）是兩個已經在推進的方案。

Quantum-safe signatures：用抗量子的密碼學方案（lattice-based 或 STARK-based）替換現有的 BLS/ECDSA。這不是急事，但需要提前研究。跟我之前在 PQC 方向做的東西直接相關。
...
\- Proto-Danksharding（EIP-4844）：引入 blob 交易，rollup 的資料不再永久存在鏈上，改用臨時的 blob storage。L2 成本大幅降低。

待研究：

Full Danksharding：Proto-Danksharding 的完整版，為 rollup 提供大規模的資料空間。

Data Availability Sampling（DAS / EIP-7594 / PeerDAS）：讓節點不需要下載全部 blob data 也能驗證資料可用性——透過概率抽樣就夠了。這是 Fusaka 的重點。三個作用：確保 rollup 的資料在 EIP-4844 之後仍然可驗證、確保 block producer 公開了所有資料、在 PBS 下只有 builder 需要處理完整 block。

Removing Rollup Training Wheels：讓 rollup 真正去中心化。包括 Optimistic Rollup 的 fault prover（Arbitrum BoLD、Optimism Cannon）、ZK-EVM 的成熟化、以及 rollup interoperability（L1 sequencer / based sequencing）。

Quantum-safe and Trusted-Setup-Free Commitments：用不需要 trusted setup 且抗量子的 commitment scheme 取代 KZG。

\---

The Scourge（抗審查與 MEV）

已完成：

\- MEV-Boost：把 MEV 市場化，大部分 MEV 現在流向 validator 而不是被 searcher 獨吞。但這是 extra-protocol 的方案。

\- EIP-7251（提高 MAX\_EFFECTIVE\_BALANCE）：減少 validator 數量但不減少總質押量，降低網路通訊開銷。

待研究：

ePBS（Enshrined PBS）：把 proposer-builder separation 寫進協議層。目前 MEV-Boost 是 extra-protocol 的，依賴中心化的 relay。ePBS 的目標是在協議內實現 PBS，防止交易審查、讓小 validator 不被機構 builder 淘汰、為 Danksharding 鋪路。

MEV Burn：把 builder 支付的 MEV 費用 burn 掉而不是給 proposer，讓所有 ETH 持有者受益而不只是 validator。

Execution Tickets（ET）：建立一個 permissionless 市場，讓買家購買 propose execution payload 的權利。Attester-Proposer Separation：beacon proposer 不關心 execution payload，execution proposer 從市場中選出。協議對市場有 introspection。

Inclusion Lists（IL）：讓 proposer 保留一定的權力來強制 include 交易，防止 builder 審查特定交易。相關研究有 Multiplicity gadgets 和 COMIS。

Preconfirmations：讓用戶在交易被 include 之前就能拿到「會被 include」的承諾。Builder 公開承諾 include 超過某個 priority fee 的交易，並給用戶一個 receipt。

Distributed Block Building：把 builder 的角色拆分——交易排序演算法、block construction 的計算資源、extra builder services（如 preconfirmation）可以分別去中心化。

Application Layer MEV Minimization：在應用層面減少有害的 MEV（frontrunning、sandwich attack）。

Staking Economics 方面：降低節點運行成本（Verkle tree + SNARK 減少 SSD 需求、加快同步）、研究 validator set 上限、對抗 LST 中心化問題（enshrined LST、rainbow staking、two-tiered staking 等方案）。

\---

The Verge（驗證簡化）

Data Availability Sampling（跟 Surge 重疊）：讓 light client 可以用概率抽樣驗證 blob 資料。

Verkle Tree Commitments（EIP-7736）：用 vector commitment 取代 Merkle proof，proof size 從 O(log n) 降到接近 O(1)。讓 light client 更輕、stateless client 變得可行。這跟之前在 data-structures 那篇讀到的 Verkle tree 是同一個東西，現在看到它在路線圖裡的位置了。

\---

The Purge（瘦身）

History Expiry（EIP-4444）：客戶端自動刪除超過約一年的歷史 block 和 receipt，控制磁碟用量。之後就不能從 genesis full sync 了。

State Expiry（EIP-7736）：在 Verkle tree 基礎上，把長期未被存取的 state leaves 移除，需要的時候用 proof 復活。目標是把 active state 壓縮到 20-50 GB。

\---

The Splurge（其他）

Account Abstraction（EIP-4337）：已出貨。UserOperation mempool + bundler + paymaster，讓智能合約錢包不需要共識層改動就能實現 social recovery、sponsored tx、batched ops。EIP-7702 是後續在協議層的補充。

Quantum-Safe Signatures：跨 Merge 和 Splurge 的主題，研究 hash-based、lattice-based、STARK-based 的多簽方案來替換 BLS 和 ECDSA。

Formal Verification Tooling：用 Coq、SMT、TLA+、Dafny、Isabelle/HOL 等工具證明協議客戶端和智能合約的正確性。
...
\- Merge：共識機制的持續優化（SSF、SSLE、量子安全）

\- Surge：擴展性（Danksharding、DAS、rollup 去信任化）

\- Scourge：MEV 治理和去中心化（ePBS、IL、preconfirmation、staking economics）

\- Verge：驗證輕量化（Verkle tree、DAS for light clients）

\- Purge：協議瘦身（history expiry、state expiry）

\- Splurge：UX 和長期安全（AA、量子安全、formal verification）

一個有意思的觀察：很多升級之間有深度依賴關係。比如 Full Danksharding 需要 DAS，DAS 的效果受益於 ePBS，ePBS 又需要 Inclusion Lists 來保持抗審查性。State Expiry 依賴 Verkle Tree，Verkle Tree 的遷移本身就是個巨大的工程挑戰。這不是一張可以平行推進的 checklist，而是一張有大量前後依賴的 DAG。

另外，量子安全這個主題在 Merge、Surge 和 Splurge 裡都有出現——簽名方案要換（BLS → post-quantum）、commitment scheme 要換（KZG → trusted-setup-free + quantum-safe）、用戶層的 ECDSA 也遲早要遷移。這正好跟我之前做的 PQC 研究方向接上了。

<!-- DAILY\_CHECKIN\_2026-04-27\_END -->
...
<!-- DAILY\_CHECKIN\_2026-04-24\_START -->

今天讀了 CL Architecture 和 CL Clients 兩篇。Architecture 那篇把 fork choice、reorg、safety vs liveness、Gasper 的組合邏輯、以及 CL 的 state transition function 講得很完整。Clients 那篇是各個 CL 客戶端的概覽和 client diversity 的重要性。

\---

Fork Choice

幾千個節點分散在全球，網路延遲、掉線、惡意行為都會讓不同節點看到不同的 block tree。Fork choice rule 的工作就是讓所有誠實節點最終選中同一條鏈。

!\[Block Tree\]([https://epf.wiki/images/cl/blocktree.svg](https://epf.wiki/images/cl/blocktree.svg))

Fork choice rule 選的不是「一條鏈」，而是選一個 head block——鏈頭。Head block 確定了，一路沿 parent 回到 genesis 的那條唯一路徑就是 canonical chain。第一條規則：被選中的 block 和它所有祖先必須是合法的，任何非法 block 直接忽略，建在非法 block 上的 block 也一樣。

三種 fork choice rule 的比較：

\- PoW（Ethereum 舊版 / Bitcoin）：heaviest chain rule，累計算力最多的那條鏈。常被誤稱 longest chain 但不精確。另外一個常見誤解：以太坊 PoW 時期並沒有用 GHOST（雖然白皮書寫了），Vitalik 後來確認是因為擔心攻擊而沒有實作，直接用了更簡單的 heaviest chain rule。

\- Casper FFG：跟隨包含最高 justified checkpoint 的那條鏈，永遠不 revert finalized block。

\- LMD GHOST：Greediest Heaviest Observed SubTree——累計每個 block 及其後代的 validator 投票權重，選最重的子樹。同時也遵守 Casper FFG 的規則。

Reorg（重組）

收到新 block 或新的 attestation 時，節點會重新跑 fork choice rule。通常新 block 是當前 head 的 child，直接接上去。但有時候新資訊會讓另一條分支的 block 變成 head。

!\[Reorg Before\]([https://epf.wiki/images/cl/reorg-0.svg](https://epf.wiki/images/cl/reorg-0.svg))

假設節點目前認為 F 是 head，chain 是 A ← B ← D ← E ← F，block C 存在但在側鏈上。

!\[Reorg After\]([https://epf.wiki/images/cl/reorg-1.svg](https://epf.wiki/images/cl/reorg-1.svg))

當 block G（建在 C 上面）出現，如果 fork choice rule 判定 G 比 F 更好，節點就要 reorg：回退 D、E、F 的狀態，回到 B 之後的狀態，再套用 C 和 G。新的 chain 變成 A ← B ← C ← G。

1-2 個 block 的短 reorg 很常見（網路延遲造成的），長 reorg 應該很罕見——除非鏈正在被攻擊或 fork choice 有 bug。

Safety vs Liveness

Safety：壞事不會發生（不會 finalize 衝突的 checkpoint、不會雙花）。

Liveness：好事終究會發生（鏈永遠能繼續出新 block、不會死鎖）。

CAP Theorem 告訴我們：分散式系統不可能同時滿足一致性、可用性和分區容忍性。以太坊在兩者之間選擇了偏向 liveness——網路分區時，各邊會繼續出 block 但不會達到 finality。如果分區持續夠久，兩邊可能各自 finalize 不同的歷史，形成兩條不可調和的獨立鏈。

這個取捨在日常運作中不是問題，只有極端的網路災難才會暴露。

Gasper = LMD GHOST + Casper FFG

LMD GHOST 提供 liveness：確保鏈持續出塊，但容易 fork，沒有形式化的 safety 保證。

Casper FFG 提供 safety：定期 finalize，防止長距離的 reversion。

兩者組合：LMD GHOST 讓鏈持續向前走，Casper FFG 在後面穩定地蓋章確認。即使 Casper FFG 暫時無法 finalize（比如太多 validator 離線），鏈還是能繼續跑——只是暫時沒有 finality。

\---

CL 架構

!\[Network\]([https://epf.wiki/images/cl/network.png](https://epf.wiki/images/cl/network.png))

以太坊節點 = EL client + CL client，可選地再加上 validator client。Beacon node 不一定要跑 validator（圖中綠色節點才是有 validator 的）。

元件職責

Beacon Node：跑 CL client 軟體，跟其他 beacon node 通訊、維護 beacon chain、處理 attestation。跟本地的 EL client 以及可選的 validator client 溝通。

Validator Client：只跟本地的 beacon node 溝通。當 beacon node 通知它「該你 propose 了」或「該你 attest 了」，它簽名然後透過 beacon node 廣播出去。Validator client 不直接跟網路上的其他節點說話。

!\[CL Layers\]([https://epf.wiki/images/cl/cl.png](https://epf.wiki/images/cl/cl.png))

EL 和 CL 並行跑，透過 Engine API（本地 RPC）溝通。CL 給 EL 下指令，EL 把交易打包好傳回給 CL 放進 beacon block。兩者共享一個 ENR 但各自有獨立的金鑰（eth1 key 和 eth2 key）。

Control Flow

不是出塊者時：收到 block gossip → 預驗證 → 把 execution payload 送給 EL → EL 執行交易、驗證狀態 → EL 回傳驗證結果 → CL 把 block 加到鏈上、做 attestation 並廣播。

是出塊者時：收到出塊通知 → 呼叫 EL 的 create block → EL 從 mempool 撈交易、執行、生成 block hash → CL 把交易和 hash 包進 beacon block → CL 廣播 → 其他節點驗證並 attest → 足夠的 attestation 之後 block 加到 chain head → 最終被 justified 和 finalized。

State Transition

CL 的 STF 跟 EL 的概念一樣但結構不同。EL 是 block-driven（有 block 才更新），CL 是 slot-driven（時間到了就更新，不管有沒有 block）。

S' ≡ f(S, B)

拆成三層：

\- Per-slot transition f\_s(S)：每個 slot 都跑，不管有沒有 block

\- Per-block transition f\_b(S, B)：有 block 的時候跑

\- Per-epoch transition f\_e(S)：每個 epoch 結束時跑（獎懲結算、committee 重新洗牌、finality 檢查等重活都在這裡）

Pyspec 裡的 state\_transition 函數很清楚：

1\. process\_slots：從當前 state 的 slot 推進到 block 的 slot（中間可能有空 slot），每跨一個 slot 跑一次 per-slot transition，碰到 epoch 邊界就跑 per-epoch transition

2\. verify\_block\_signature：驗證 proposer 的 BLS 簽名

3\. process\_block：處理 block 裡的所有內容

4\. 驗證最終的 state\_root 跟 block header 裡宣告的一致

任何環節觸發未處理的 exception（failed assert、array out of range、uint64 overflow/underflow）都算非法。

\---

CL Clients

目前 production-ready 的 CL 客戶端有六個，再加上兩個還在開發中的：

| Client | 語言 | 開發者 | 特色 |

|--------|------|--------|------|

| Lighthouse | Rust | Sigma Prime | 安全性和效能導向，支援 cross compilation、slasher、doppelganger protection |

| Prysm | Go | Prysmatic Labs | 易用性和穩定性導向，用 gRPC + BoltDB + libp2p，最完整的安全實踐文件 |

| Teku | Java | ConsenSys | 企業級，完整的 REST API、外部 key management、適合大規模部署 |

| Nimbus | Nim | Status | 極致輕量，能跑在手機和 Raspberry Pi 上，內建 validator client |

| Lodestar | TypeScript | ChainSafe | 快速原型開發和瀏覽器相容，提供 BLS 和 SSZ 的 JS library，有 light client 和 prover 實作 |

| Grandine | Rust | Grandine Developers | 效能極致化，利用平行化和高效資源利用，低延遲高吞吐 |

| Caplin | Go | Erigon | 內嵌在 Erigon 裡的 CL，讓 Erigon 可以不需要外部 CL 獨立運作 |

| LambdaClass | Elixir | LambdaClass | EPF4 期間起步，還在積極開發中 |

Client Diversity 為什麼重要

目前大多數節點跑 Prysm 或 Lighthouse。如果某個客戶端佔了 supermajority（>2/3），那個客戶端出 bug 就可能導致錯誤的 block 被 finalize，然後所有跑那個客戶端的 validator 都會被 slash。這不是理論上的風險——Prysm 和 Lighthouse 都出過跟其他客戶端不一致的 bug。

少數派客戶端出 bug 的後果小很多：最多就是那些 validator 暫時離線、錯過一些 attestation 獎勵，不會被 slash。

所以從個人和網路健康的角度來看，跑少數派客戶端（Teku、Nimbus、Lodestar、Grandine）是更安全的選擇。

\---

小結

CL Architecture 講的是共識層的「大腦」：fork choice 決定跟哪條鏈、Gasper 組合了 liveness 和 safety、state transition function 定義了狀態怎麼隨 slot/block/epoch 演進。CL Clients 講的是這些規範的「肉身」——六個不同語言的實作，各有側重，共同維護網路的去中心化和韌性。

一個有趣的觀察：EL 的 STF 是 block-driven 的（σ(t+1) = Π(σ(t), B)），CL 的 STF 是 slot-driven 的（即使沒有 block 也要更新狀態）。這個差異反映了兩層的本質不同：EL 關心的是交易的執行結果，CL 關心的是時間的推進和共識的形成。

<!-- DAILY\_CHECKIN\_2026-04-24\_END -->
...
今天參加了**Satoshi Salon #1**這場現下活動，從 cypherpunk 思想源頭出發，討論比特幣作為貨幣、主權與技術秩序的全球實驗：它從何而來、改變了什麼、今天如何重新理解、下一個十年走向何方，以及台灣比特幣社群的方向。東西方兩條建設路徑（台灣研究者視角 × 美國協議層開發者視角）的對照是這場的主軸。印象比較深兩塊。 Cypherpunks Taiwan 的歷史。另外是 Dan 的 Payjoin，透過打亂 common input ownership heuristic 來強化鏈上隱私，算是協議層隱私工程蠻具體的做法。回到家已經很晚了，今天先暫時叛變到BTC陣營，明天重新做回E衛兵。

...
目前合约运行在所有节点这个设定很浪费资源....也许可以改成随机分配2-10个节点(这个数量有些激进,可以增加多点)同时运行再将结果广播到网络...当然也不是完全随机;比如负荷重的节点随机因子就小,负荷轻随机因子就大... 如何保证节点诚实?可以增加专门的审计团队...
...
以太坊的二层网络通过打包提交缓解主网压力,但随着数据量的持续增多.也许可以发明新的3层 4层 5层…但运行的代码依然需要全部节点…如果某一天,虽然还远没有到来…主网代码增加到运行都会显著卡顿的程度. 是不是应该删除没用的代码? 这样又会和去中心化理念冲突…或者实行竞价?价高者得??
...
把第一天缺失的都补上了...之前只是零星地接触了散落在各地的知识,这下终于可以系统地一整套学习完整的历史理念等等之类的了...接下把重心都学习这个,其他语言的学习先缓缓...

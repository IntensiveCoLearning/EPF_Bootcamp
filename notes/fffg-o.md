...
经典密码学通过利用某些数学问题的内在困难性来保护信息，包括：大数分解、离散对数、图同构、最短向量问题等，这些问题属于 Hidden Subgroup Problem（HSP）相关研究范围。

这些问题的核心在于：在不知道“秘密”或私钥的情况下，确定大群中某个秘密子群的结构在计算上不可行。公钥密码算法正是利用这种单向的“陷门函数”来获得安全性。

RSA 的安全性依赖于大素数分解问题；ECDSA 的安全性依赖于椭圆曲线离散对数问题。随着密钥大小增加，经典计算机解决这些问题会变得指数级更困难，因此在经典计算模型下被认为不可行。

### 2\. 量子计算带来的威胁

量子计算机利用量子力学原理，能够采用新的计算方式。某些量子算法相较于经典算法，可以以指数级效率解决经典密码学依赖的问题。

如果大规模量子计算机被建成，它们将能够破解目前广泛使用的许多公钥密码系统。Shor 算法是量子计算最著名的应用之一，可用于整数分解。

后量子密码学的目标，是开发即使面对强大量子计算机也仍然安全的新算法。

### 3\. 时间线与风险判断

多数专家认为，到 2030 年以前，公钥密码学面临的威胁概率低于 5%；但到 2050 年，风险预计会显著上升到约 50%。目前最先进的量子计算机拥有少于 2000 个物理量子比特，而在理想的一小时窗口内破解 Bitcoin 加密大约需要 3.17 亿个物理量子比特。这些预测具有一定主观性，也可能无法反映并非公开的真实进展。高级威胁行为者可能比公众更早获得强大量子计算能力，并采用 retrospective decryption 等策略。

### 4\. 后量子风险对 Ethereum 的影响

Ethereum 账户由两层密码系统保护：私钥通过椭圆曲线乘法生成公钥；公钥再通过 keccak256 哈希生成 Ethereum 地址。

直接的后量子威胁是：量子攻击者可能逆向 ECDSA 所依赖的椭圆曲线乘法，从而暴露私钥。这会使所有外部拥有账户（EOA）面临量子攻击风险。如果从公钥映射到 Ethereum 地址的哈希函数仍然安全，那么从地址提取私钥仍然困难，但页面仍将其描述为“有漏洞”。实际中多数用户的私钥来自 BIP-32 的一系列哈希计算，这会让暴露私钥的计算成本更高。

### 5\. Ethereum 后量子紧急方案设想

EthResearch 上有一个关于后量子紧急情况下硬分叉的进行中提案，核心行动包括：

1.  回滚从明确发生大规模盗窃的第一个区块之后的所有区块。
    
2.  禁用传统基于 EOA 的交易。
    
3.  添加一种新的交易类型，使智能合约钱包可以发起交易，例如 RIP-7560 的一部分，如果当时尚不可用。
    
4.  添加一种新的交易类型或 opcode，允许用户提供 STARK 证明，证明其知道：私有原像 `x`、批准哈希函数列表中的某个哈希函数 ID、以及公共地址 `A`，并满足 `keccak(priv_to_pub(hashes[i](x)))[12:] = A`。该 STARK 还接受该账户新验证代码哈希作为公共输入。
    

如果证明通过，账户代码会切换为新的验证代码，之后该账户可以作为智能合约钱包使用。

这种方式并不完美。部分用户仍然会损失资金，因为不可能回滚攻击事件之后的所有区块，而且可靠地检测网络上的量子攻击非常困难。一个大型交易所钱包被量子计算机盗走时，人们可能首先认为是交易所安全失败；一个依赖离散对数假设的智能钱包被盗时，人们可能首先怀疑智能合约漏洞；攻击者也可能避开高知名度目标，慢慢盗取大型 EOA 的资金，以至于没人意识到发生了量子攻击。此外，支撑 EIP-4844 的 KZG commitment schemes 也需要升级，以防止欺诈性承诺。

### 6\. 研究与标准化

后量子密码学是一个活跃研究领域。多个组织正在进行新后量子算法的原型设计、开发和标准化工作。

**NIST 后量子密码学标准化**

NIST Post-Quantum Cryptography standardization 进行了多年国际竞赛，用于评估和标准化抗量子密码算法。2024 年 8 月，NIST 发布了第一组最终 PQC 标准，作为 Federal Information Processing Standards（FIPS）。
<!-- DAILY_CHECKIN_2026-05-02_END -->

# 2026-05-01
<!-- DAILY_CHECKIN_2026-05-01_START -->

**Keccak256**一种**密码学哈希函数**，将任意输入映射为**固定 256-bit 输出**
...
**BLS（Boneh–Lynn–Shacham）签名** 一种基于椭圆曲线配对（pairing）的数字签名方案

-   权益证明协议使用数字签名来识别参与者并追究其责任。
    
-   信标链（以太坊）中的验证者使用 BLS 签名参与共识、签署区块、发布证明等。
    
-   BLS 签名可以聚合在一起，从而可以高效地进行大规模验证。
    
-   签名聚合使得信标链能够扩展到数十万个验证者。
    
-   以太坊执行层的常规交易签名仍然使用 ECDSA。但是，账户抽象钱包也可以使用 BLS。
    

BLS 特征的核心是通过椭圆曲线上的配对进行双线性映射的概念。一个关键组成部分是配对函数 e_e_ ，它定义在由椭圆曲线导出的两个组之间：

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/EPF_Bootcamp/main/assets/fffg-o/images/2026-04-30-1777546078937-image.png)

对于所有整数 G1_G_1​ 中的 PP,Q 和 aaa ，双线性定义为：

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/EPF_Bootcamp/main/assets/fffg-o/images/2026-04-30-1777546101411-image.png)

此外，它还必须满足加法分配律。

传统的 ECDSA 签名（例如比特币或以太坊交易中常用的签名）严重依赖于随机数生成的随机性，并且需要逐一验证所有相关的公钥，这会带来大量的计算资源消耗。在专利到期后，Schnorr 签名成为一种替代方案，它允许一定程度的聚合，但仍然缺乏 BLS 签名所具备的全部效率。

采用双线性配对的 BLS 签名能够有效抵御某些密码攻击，并生成更短的签名。与 Schnorr 签名不同，BLS 签名不依赖随机数生成来保护签名，因此本质上更能抵御与随机性相关的漏洞。

BLS 的一个主要优势在于能够将多个签名聚合为一个简洁的签名。这在涉及多个交易或签名者的场景中尤为有用，可以大幅减少区块链空间和验证所需的计算能力。

在区块链领域，数字签名通常利用椭圆曲线群。以太坊主要采用基于 secp256k1 曲线的 ECDSA 签名，而信标链协议则采用基于 BLS12-381 曲线的 BLS 签名。与 ECDSA 不同，BLS 签名利用了某些椭圆曲线的独特特性——“配对”。这使得多个签名可以聚合，从而提高共识协议的效率。虽然 ECDSA 签名的处理速度更快，但 BLS 签名的聚合能力为区块链的可扩展性和共识效率带来了显著优势。

BLS 数字签名流程中包含 4 个组成数据。

-   密钥：协议中的每个参与者，特别是验证者，都拥有一个密钥，也称为私钥。该密钥对于签署消息和维护验证者在网络中操作的机密性至关重要。
    
-   公钥：公钥直接通过加密方法从私钥派生而来。虽然公钥与私钥相关联，但若不付出极大的计算努力，就无法从私钥逆向工程得到公钥。它作为协议中验证者的公开身份标识，所有参与者均可访问。
    
-   消息内容：在以太坊中，消息由字节串组成，其结构和用途将在后续章节中详细探讨。首先，请将这些消息理解为区块链协议中处理的基本数据单元。
    
-   签名：签名是加密过程的结果，该过程将消息与私钥结合。此签名唯一地标识消息是由私钥持有者撰写的。通过使用相应的公钥验证签名，可以确认消息源自特定的验证者，并且在签名后未被篡改。
...
**ECDSA（Elliptic Curve Digital Signature Algorithm），** 一种基于**椭圆曲线密码学**的数字签名算法

基于椭圆曲线：y2=x3+ax+by^2 = x^3 + ax + by2=x3+ax+b

安全性来源：椭圆曲线离散对数问题（ECDLP）**正向计算容易，逆向困难**
...
eODS 为 execution layer 的数据/对象结构体系（抽象层），一套描述 **以太坊执行层（Execution Layer）状态与数据结构** 的模型
...
2\. 可证明数据结构（Proof-friendly structures）
...
eODS 并不是具体实现，而是 一种规范层（spec-level abstraction）

类似：

-   数据模型（Data Model）
    
-   状态机定义（State Machine Spec）
...
**FCR（Fast Confirmation Rule，快速确认规则）** 是一种让以太坊节点判断“某个区块是否可以被认为不会离开规范链”的算法。它输出的结果很简单：区块是 **confirmed** 或 **not confirmed**。它的目标是在不等待最终性 finalized 的情况下，尽早给出更安全的确认信号。FCR 不是最终性，但它是在特定网络假设下，比等 N 个区块深度更有理论保证的快速确认规则。  
以太坊当前最强的确认机制来自 **Gasper / FFG finalization**。它非常安全，即使在异步网络条件下也能工作，但速度较慢：最佳情况约 13 分钟，平均约 16 分钟。最终确定的区块不会与任何诚实验证者看到的规范链冲突，除非超过三分之一验证者被罚没。

FCR 依赖同步网络条件，但可以在最佳情况下把确认时间降低到 **12 秒左右**，显著快于 FFG finalization。EPF Wiki 总结它的关键性质是：FCR 不会确认一个非规范链上的区块，并且具备形式化保证。已确认的区块，在诚实节点视角中不会被 reorg 掉。已确认区块不会向后倒退，除非触发 reset-to-finalized。这类 reset 可以被理解为：系统检测到假设可能不再成立，于是回退到更保守的已 finalized 状态。

FCR 的安全性依赖两个重要假设。

假设一：网络同步性

诚实验证者在某个 slot 发出的 attestations，其他诚实验证者能在该 slot 结束前收到。EPF 页面把这描述为健康网络条件下的 synchrony assumption。

Ethereum consensus specs 也强调：FCR 适用于相信该同步假设的用户；如果该假设被打破，已确认区块可能在没有恶意行为、也没有 slashing 的情况下被 reorg。

假设二：拜占庭验证者比例有上限

FCR 假设任意 committee 中恶意 stake 的最大比例为 β，EPF Wiki 给出的范围是 **20–25%**，且可配置。当前 specs 中的配置项 `CONFIRMATION_BYZANTINE_THRESHOLD` 为 25，表示假设拜占庭验证者比例最大为 25%。
...
PeerDAS是一种以太坊网络协议，目标是在不要求每个节点下载全部 blob 数据的情况下，通过点对点抽样、纠删码和 KZG 承诺来确认数据可用性**。** 它由 EIP-7594 引入，服务于以太坊 rollup-centric 扩容路线。

以太坊扩容的关键瓶颈之一是 **数据可用性（Data Availability, DA）**。L2 rollup 需要把交易数据发布到 L1，以便其他人能验证状态是否正确。早期 rollup 使用 calldata，成本高且效率低；EIP-4844 引入 blob，降低了 rollup 发布数据的成本。

但 EIP-4844 仍有一个问题：**每个节点仍然需要下载全部 blob 数据**。随着 blob 数量增加，节点带宽和存储压力也会上升，这会伤害去中心化。PeerDAS 的核心价值就是：在提高 blob 吞吐量的同时，避免单个节点负担线性增长

| 阶段 | 含义 | 节点负担 |
| --- | --- | --- |
| Stage 0：EIP-4844 | 引入 blob 和 blob 分发子网，但没有 DAS | 节点仍需下载全部数据 |
| Stage 1：1D PeerDAS | 水平扩展 blob，引入 column subnets，节点进行 peer sampling | 节点只需处理部分列 |
| Stage 2：2D PeerDAS / Full Danksharding | 加入垂直扩展和 cell-based sampling | 更强重建能力和更高扩展性 |
...
**mev-boost 是以太坊 PoS 时代的一种“协议外 PBS 实现”。**

它把原本由验证者同时承担的两件事拆开：

| 角色 | 负责什么 |
| --- | --- |
| Builder / 构建者 | 构建最有价值的区块 |
| Proposer / 提议者，也就是验证者 | 从多个候选区块中选择报价最高的区块并提议上链 |

Flashbots 对 mev-boost 的定义是：它是验证者运行的开源中间件，用来接入竞争性的 block-building 市场，也是 PoS Ethereum 上 proposer-builder separation，也就是 PBS，的初始实现。
...
| 阶段 | PBS 前 | PBS 后 |
| --- | --- | --- |
| 构建区块 | 验证者自己做 | Builder 做 |
| 优化 MEV | 验证者自己做 | Builder 竞争 |
| 选择区块 | 验证者自己决定 | 验证者选最高报价 |
| 提议区块 | 验证者 | 验证者 |

这就是 PBS 的关键思想：专业化可以发生在 block building 上，但不要让这种专业化吞噬 block proposing 和 staking 的去中心化。

MEV-Boost 工作流程

-   用户或 searcher 提交交易、bundle 或 orderflow。
    
-   Builder 根据交易、bundle、MEV 机会构建完整区块。
    
-   Builder 把区块提交给 relay。
    
-   Relay 检查区块有效性、支付金额、gas limit 等。
    
-   Relay 只把最高有效 bid 的 header 发给 mev-boost / proposer。
    
-   Proposer 选择最高报价 header 并签名。
    
-   Relay 收到签名后释放完整 payload。
    
-   Proposer 将完整区块广播到以太坊网络。
...
区块链扩容的核心目标，是在不显著提高节点运行成本、不削弱去中心化和安全性的前提下，处理更多交易；以太坊当前的主流方案是让 L1 负责安全、结算和数据可用性，让 Rollups 负责大部分执行。

区块链可扩展性可以理解为：系统处理越来越多操作，尤其是更多交易的能力，同时不提高网络节点运营者的负担。这里的重点不是单纯追求更高 TPS，而是 **“更多交易 + 不显著提高节点门槛”**。如果只是把节点硬件要求堆高，那不是理想的扩容。

以太坊的吞吐量主要受两个参数影响：

1.  Block latency，一个有效区块被纳入链中的时间，越短，理论 TPS 越高
    
2.  Block size，单个区块能容纳的数据和计算量，越大，能放入更多交易
    

以太坊使用基于 PoS 的 Gasper 共识协议，理想情况下每个 slot 是 12 秒，但现实中可能因为验证者错过出块而有差异。区块大小方面，以太坊用 gas limit限制每个区块能包含的总计算量；EIP-1559 又引入了动态费用机制，让区块在短期拥堵时可以弹性变化，但仍有上限。
...
如果为了高 TPS 牺牲节点可运行性，系统可能变快，但会更中心化。如果只追求去中心化和安全性，吞吐量又会受限。原文指出，经典区块链系统很难在不牺牲某些去中心化水平的情况下实现大规模采用。

现代区块链扩容采用一种“分而治之”的模块化设计，把区块链拆成几个功能层：

| 模块 | 作用 |
| --- | --- |
| Consensus 共识 | 节点对区块构建和交易排序达成一致 |
| Execution 执行 | 执行交易，更新状态 |
| Data Availability 数据可用性 | 保证数据在需要时可以被获取 |
| Settlement 结算 | 保证交易最终性和不可篡改性 |

模块化的好处是：不同组件可以分别优化，而不是让一条链同时承担所有任务。这种方式降低了系统整体复杂度，也方便引入新的扩容方案。

**Layer 1 扩容**

L1 扩容是直接改进以太坊主链本身，例如：
...
-   调整 gas limit 或操作 gas 定价。
    

但 L1 扩容必须谨慎，因为提高区块容量或降低某些操作成本，都可能增加节点负担。提高 gas limit 确实能让以太坊容纳更多计算和数据，但也会提高节点运营者的要求。

**Layer 2 扩容**

L2 扩容是在 L1 之上构建系统，把大量交易放到链下或上层执行，同时仍依赖以太坊 L1 的安全模型。常见方案包括：
...
以太坊社区当前主要采用的是 **multi-rollup-centric approach**，也就是多 Rollup 中心路线。
...
以太坊的协议演进理念是：**愿意持续升级，但对不值得冒险的变更保持谨慎**。随着研究和工程经验积累，社区不断围绕网络的安全性、可扩展性、可持续性提出改进方向。虽然**没有一个中心化权威来规定路线图**，但通过公开讨论、研究和开发协作，逐渐形成了大家默认认可的“路线图”。
...
-   **Scourge**：抗审查、去中心化、MEV 风险
    
-   **Verge**：更轻量的验证
    
-   **Purge**：减少历史和状态负担
    
-   **Splurge**：其他重要但不紧急的增强项
    

Merge:已于20220915完成，从pow到pos。后续重点在于让pos更快，更稳，更安全。

Surge：已完成引入blob，让rollup不再永久储存在以太坊上，而是放入临时空间，显著降低rollup成本，仍在研究danksharding，DAS,Removing Rollup Train Wheels

Scourge: 形成协议外的MEV市场，大量收益流向验证者。

Verge：不必让所有人都保存和验证全部状态，也能安全验证链。
...
-   Beacon Node :维护信标链，处理区块，状态，验证者信息，参与网络通信，提供api
    
-   Validator Client:提议区块，进行证明，参与共识投票
    
-   Beacon Chain: 存储区块，状态，验证者集合
    
-   P2P Networking:libp2p , 用于节点发现，区块传播，证明传播
    
-   Engine API: cl通知el构建区块，el返回执行结果
    

完整cl客户端：

1.  Sync 同步模块，从网络同步区块和状态
    
2.  Fork Choice： 从多个分叉中选择正确的，使用LMD-GHOST算法
    
3.  State Transition：区块改变系统状态
    
4.  Block Processing：验证区块合法性
    
5.  Validator Duties Scheduler 安排验证者何时执行任务
...
| 客户端 | 使用的语言 | 开发者 | 目前状态 |
| --- | --- | --- | --- |
| Lighthouse | Rust | Sigma Prime | 正式 |
| Lodestar | TypeScript | ChainSafe | 正式 |
| Nimbus | Nim | Status | 正式 |
| Prysm | Go | Prysmatic Labs | 正式 |
| Teku | Java | ConsenSys | 正式 |
| Grandine | Rust | Grandine Developers | 正式 |
| Caplin | Go | Erigon | 测试 |
| LambdaClass | Elixir | LambdaClass | 测试 |

**一、Prysm**

1.  基本信息  
    1.1 使用 Go 语言开发  
    1.2 当前（历史上）使用最广泛的 CL 客户端
...
第二周还剩一个完成，昨日第三周开启，也就是还差两天的进度，偏偏这周有一堆不得不去做的事，连挤出一点时间写笔记都很难。
...
今日学习 Weak Subjectivity

原本的pow模式导致修改区块链历史是很困难的，所以具有客观性，因为可以轻易地验证到创世区块。

合并后，以太坊转向了pos机制，让创世同步变的不够安全，引入了弱主观性。

当验证者退出时，无法参与未来的区块投票，但是可以重新为过去的区块投票，只要有很多验证者退出并重新投票，就可能制造一条分叉链，已经在链头的节点不会受到影响，但是新的同步节点并不知道验证者的历史状态，可能把假链当成真链，所以信标链采用反向同步，用来解决这个问题。

今日还是有点水，还剩cl 架构和cl clients 没学，本周其他的事太多了

...
-   Glamsterdam / DevNet 路线
    
-   ePBS 相关 PR
    
-   Hegota 候选提案
    
-   Amsterdam repricing 更新
...
-   原计划中的 `ePBS DevNet2` 也不再单独推进。
    
-   下一步直接瞄准 `Glamsterdam DevNet0`。
    
-   团队倾向先把现有核心改动并入，再在 interop 中继续消化问题。
...
-   它主要用于解决 `checkpoint sync` 的现实问题。
    
-   虽然它引入了 execution requests 的重复数据，但会议结论是：
...
-   当前 execution requests 既在 envelope 中传播，也出现在 block 中。
    
-   争议点在于：
    
    1.  这样会造成重复传播。
        
    2.  但去掉 block 中的数据，又会影响 STF 的纯函数形式和 checkpoint sync。
        
-   会议没有否定优化方向，但决定：
    
    1.  不在这一轮立即处理。
        
    2.  后续继续研究是否能只在 gossip 层做 commitment + expansion。
        

### 4\. Glamsterdam DevNet1

-   `DevNet1` 范围暂未确定。
    
-   倾向把新增内容作为 interop 期间的 `stretch goals`。
...
-   ePBS 把 payload 和 block 拆开后，`checkpoint sync` 和数据依赖变复杂。
    
-   为了让 STF 继续保持“只吃 state + block”的纯函数形式，请求数据被放回 block。
    
-   这样做的副作用是重复传播 execution requests。
...
-   如果现在移除 block 中的 requests，会重新把 checkpoint sync 问题带回来。
    
-   一旦这么做，还要继续修改 RPC、gossip 对象和客户端流程，复杂度会升高。
...
-   这更像是一个“数据流设计问题”，而不是 STF 签名哲学问题。
    
-   可以考虑：
    
    1.  beacon block 里只放 commitment
        
    2.  节点利用前一个 slot 的 payload 在本地展开数据
...
-   但要处理客户端缺失前一个 payload 的边缘情况。
    

### Nico

-   从现实数据看，最坏情况不像 1.5MB 那么常见。
    
-   当前 gas 限制下，实际最大规模约为 489KB。
    
-   最近两周主网的 execution requests 平均大小约为 70 字节。
    
-   因此，这个问题存在，但未必是当前最高优先级。
...
-   `BAL DevNet4` 与 `ePBS DevNet2` 原本都想在下周五推进。
    
-   然后在 interop 第一日做 `Glamsterdam DevNet0`。
...
-   如果先做两个分离 DevNet，再快速合并，大家没有足够时间消化问题。
...
-   把精力集中到 `Glamsterdam DevNet0`。
    
-   这样路径更简单，也更适合直接进入 interop 节奏。
...
-   用于澄清 genesis / builder workflow 相关问题。
    
-   需要 rebase 到 `5094` 之上。
    
-   希望尽快并入，以便 Justin 的版本发布。
    

### Meta EIP 调整

-   把 networking EIPs 单独归类进 Amsterdam 的 meta EIP。
    
-   部分内容是 optional。
    
-   会议认为这更多是格式和范围整理，整体可以接受。
    

### Partial sales / 可选项

-   Barnabas 希望把 partial sales 相关 EIP 也列入 Amsterdam meta EIP，作为可选项和 stretch goal。
...
-   Barnabas 想把 `8045` 从“slash 后不能 proposal”扩展成“slash 后不能执行任何 duty”。
    
-   Potuz 认为这可能引入额外状态依赖问题。
    
-   本次未定，留到 interop 期间继续讨论。
    

## 五、BAL 笔记

-   Potuz 认为 BAL 在 CL 侧的改动很小。
    
-   他的理解是：
    
    1.  主要只是 payload 结构变化
        
    2.  再加一些 engine API 调整
        
-   不应影响 beacon state，也不应大改 `process_execution_payload`
...
-   引入 correlated downtime penalties。
...
-   惩罚曲线不是持续高压，而是在停机开始阶段更明显。
    

## 预期效果

-   对 solo stakers 和更分散的运营者影响较小。
...
-   stokes 建议最好准备一个 PoC，降低未来纳入 Hegota 的风险。
...
-   解决 validator deposit front-run 漏洞。
    

## 问题

-   第一笔有效 deposit 会永久设置 withdrawal credentials。
    
-   在委托质押场景中，validator key 持有人理论上可抢跑资金方的存款。
    

## 方案

-   引入一个可选的 preregistration 机制。
    
-   在真正存款前，先在共识层注册 validator key 与预期 withdrawal credentials。
    
-   资金方在 deposit 前检查 preregistration 是否存在且匹配。
...
-   stokes 关注 preregistration 本身是否也会被 front-run。
...
-   Maria 简要更新了 Amsterdam repricing breakout 的结果。
    
-   state growth EIP 的若干开放问题已处理。
    
-   下一版规范会更稳定。
    
-   state gas 的收费与退款逻辑做了重构：
...
-   直接筹备 `Glamsterdam DevNet0`
    
-   继续准备 BAL 的 engine API / CL 改动
    

### 待继续研究

-   execution requests 去重优化
    
-   `8045` 的扩展版本
    
-   `DevNet1` 的 stretch goals 范围
    
-   `EIP-7716` 的 PoC 与参数设计
